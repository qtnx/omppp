...
import { Agent, type AgentEvent, type AgentMessage, type AgentTool, ThinkingLevel } from "@oh-my-pi/pi-agent-core";
...
describe("Agent", () => {
	...
	it("continue() should process all queued follow-up messages in one turn when followUpMode is all", async () => {
		...
		for (const message of grouped) agent.followUp(message);
