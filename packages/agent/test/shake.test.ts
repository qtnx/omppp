...
describe("collectShakeRegions — positive savings", () => {
	...
	test("shake skips a normal tool result no larger than its placeholder", () => {
		...
		expect(tokenizer.countMessage(entry.message)).toBeLessThanOrEqual(16);

		expect(collectShakeRegions([entry], tokenizer, RESCUE_SHAKE_CONFIG)).toHaveLength(0);
	});
	...
	test("keeps only positive-saving normal tool results in a mixed batch", () => {
		...
		const regions = collectShakeRegions([tiny, large], tokenizer, RESCUE_SHAKE_CONFIG);
		expect(regions).toHaveLength(1);
...
describe("collectShakeRegions — useless results", () => {
	test("a tiny useless tool result inside the protect window yields a region; identical plain result does not", () => {
		...
		expect(tokenizer.countMessage(flagged.message)).toBeLessThanOrEqual(16);
	...
	test("error results never bypass the window even when flagged", () => {
		const tiny = messageEntry(toolResultMessage("search", "ok", { useless: true, isError: true }));
		const large = messageEntry(toolResultMessage("search", "boom\n".repeat(50), { useless: true, isError: true }));
		const protectRecent = cfg({ protectTokens: 1_000_000 });
		expect(collectShakeRegions([tiny], tokenizer, protectRecent)).toHaveLength(0);
		expect(collectShakeRegions([large], tokenizer, protectRecent)).toHaveLength(0);
	});
});
