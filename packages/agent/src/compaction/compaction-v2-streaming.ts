...
export function buildCompactionV2Request(
	...
	const body: OpenAICodexCompactionBody = {
		...
		...(reasoning || model.useResponsesLite
			? {
					reasoning: model.useResponsesLite ? { ...reasoning, context: "all_turns" } : reasoning,
...
async function attemptCompactionV2Streaming(
	...
	options: {
		...
		// Live stream progress for the indeterminate compaction indicator.
		onProgress?: (u: CompactionProgressUpdate) => void;
		providerSessionState?: Map<string, ProviderSessionState>;
		codexCompaction?: CodexCompactionContext;
		preferWebsockets?: boolean;
	...
	if (shouldUseCodexProviderTransport(model)) {
		...
		return collectCompactionV2Events(eventStream, request, options.onProgress);
	}
	...
	return collectCompactionV2Output(response, request, options.onProgress);
}
...
async function collectCompactionV2Events(
	...
	for await (const event of events) {
		...
		progressEvents++;
		progressBytes += Buffer.byteLength(stringifyJson(event) ?? "", "utf8");
