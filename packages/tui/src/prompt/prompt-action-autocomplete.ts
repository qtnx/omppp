...
/** Host-supplied `$skill:` catalog entry. */
interface DollarMentionSkill {
	name: string;
	description: string;
	filePath: string;
	baseDir: string;
...
/** Host-supplied `$agent:` catalog entry. */
interface DollarMentionAgent {
	name: string;
	description: string;
}
...
import { subsequenceMatch, subsequenceScore, type WorkspaceAutocompleteRoot } from "../autocomplete";
...
function buildDollarMentionItems(
	...
	for (const skill of options.skills) {
		...
		if (!subsequenceMatch(query, searchable)) continue;
		candidates.push({
			value,
