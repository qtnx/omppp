...
export class TUI extends Container {
	...
	// Legacy child-tree hosts have no frame provider to declare immutable history.
	// Keep their stable prefix in native scrollback and pin a focused editor's row
	// inside the mutable window, matching the pre-provider renderer contract.
	#legacyChildFrame: readonly string[] = [];
	#legacyChildCommittedRows = 0;
	#legacyHistoryBatchId = 0;
	// Once a legacy tree rewrites or collapses its committed prefix, it no
	// longer proves which future rows are final. Keep its history frozen and
	// repaint the current tail rather than appending stale/mutable rows.
	#legacyChildHistoryFrozen = false;
	...
	/**
	 * Fallback frame for legacy child-tree hosts (tests, simple embeds).
	 *
	...
	#renderChildrenFrame(width: number, height: number): void {
		...
		if (!this.#legacyChildHistoryFrozen && composed.length <= this.#legacyChildCommittedRows) {
			this.#legacyChildCommittedRows = commonPrefix;
			this.#legacyChildHistoryFrozen = true;
		}
		...
		const canCommit =
			!this.#legacyChildHistoryFrozen &&
			(this.#legacyChildFrame.length === 0 || commonPrefix >= this.#legacyChildCommittedRows);
		const commitTo = canCommit
			? Math.max(this.#legacyChildCommittedRows, preferredTop)
			: this.#legacyChildCommittedRows;
		const historyRows = composed.slice(this.#legacyChildCommittedRows, commitTo);
		const viewportTop = this.#legacyChildHistoryFrozen
			? preferredTop
			: Math.max(this.#legacyChildCommittedRows, preferredTop);
		const viewport = composed.slice(viewportTop, viewportTop + height);
		const history =
			historyRows.length > 0
				? { id: ++this.#legacyHistoryBatchId, kind: "append" as const, rows: Array.from(historyRows) }
				: undefined;
		this.#emitPlanFrame(width, height, viewport, history, undefined);
		this.#legacyChildFrame = Array.from(composed);
		this.#legacyChildCommittedRows = commitTo;
	}
