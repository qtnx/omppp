...
/** Duo phase is live for any phase other than inactive/suspended (fork `duo/state` port). */
function isDuoPhaseLive(phase: string | undefined): boolean {
...
/**
 * Fields the PR segment renders. `StatusLineHost.lookupPullRequest` must request
 * exactly these: {@link normalizePrStatus} drops any it does not receive, which
 * silently blanks the draft/merge-state/review indicators.
 */
export const PR_STATUS_FIELDS = "number,url,state,isDraft,mergeStateStatus,reviewDecision";
...
function normalizePrStatus(value: unknown): PrStatusInfo | null {
	...
	if (typeof pr.number !== "number" || !Number.isInteger(pr.number)) return null;
	if (typeof pr.url !== "string" || pr.url.length === 0) return null;

	const normalized: PrStatusInfo = { number: pr.number, url: pr.url };
	if (typeof pr.state === "string") normalized.state = pr.state;
	if (typeof pr.isDraft === "boolean") normalized.isDraft = pr.isDraft;
	if (typeof pr.mergeStateStatus === "string") normalized.mergeStateStatus = pr.mergeStateStatus;
	if (typeof pr.reviewDecision === "string") normalized.reviewDecision = pr.reviewDecision;
	return normalized;
}
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	#usageRefreshAbortController: AbortController | undefined;
	...
	#abortUsageRefresh(): void {
		this.#usageRefreshAbortController?.abort();
		this.#usageRefreshAbortController = undefined;
	}
	...
	/**
	 * Explicit Git/repository cache invalidation for a HEAD/cwd change. Unlike
	 * generic repaint invalidation, this aborts in-flight resolves before the
	 * next render refetches from disk.
	 */
	...
	/**
	 * Generic repaint invalidation passes `preserveActiveRequests`: it clears
	 * resolved cache values but lets a live resolve finish, avoiding abort/restart
	 * churn while still forcing synchronous HEAD reads to refresh.
	 */
	#invalidateGitCaches(preserveActiveRequests = false): void {
		...
		// jj label/status share the git segment's lifecycle. A real HEAD move must
		// abort/reset them; a generic repaint preserves active work just like the
		// reftable branch resolve above.
		this.#resetJjRequests(preserveActiveRequests);
		this.#cachedJjBranch = null;
		this.#jjBranchLastFetch = 0;
		this.#cachedJjStatus = null;
		this.#jjStatusLastFetch = 0;
		if (!preserveActiveRequests) this.#jjCacheGeneration++;
	}
	...
	#resetJjRequests(preserveActiveRequests = false): void {
		if (preserveActiveRequests) return;
	...
	async #runUsageRefresh(session: TSession): Promise<void> {
		...
		this.#usageRefreshAbortController = controller;
		const signal = AbortSignal.any([controller.signal, AbortSignal.timeout(STATUS_USAGE_REFRESH_TIMEOUT_MS)]);
		...
		try {
			...
			if (this.#usageRefreshAbortController === controller) this.#usageRefreshAbortController = undefined;
