...
import { summarizeLoopCondition, type LoopLimitRuntime } from "./loop";
import type { StatusWorkspaceRoot } from "./host";

/** Compact `/loop` limit form; duration limits carry no iteration count (fork `loop-limit` port). */
function describeLoopLimitRuntime(limit: LoopLimitRuntime): string | undefined {
	if (limit.kind === "duration") return undefined;
	const initial = limit.kind === "iterations" ? limit.initial : limit.initialIterations;
	const remaining = limit.kind === "iterations" ? limit.remaining : limit.remainingIterations;
	return `${remaining} of ${initial} ${initial === 1 ? "iteration" : "iterations"} remaining`;
}
...
/** Deepest workspace root containing `inputPath` (fork `workspace-roots` port). */
function findWorkspaceRootForPath(
	inputPath: string,
	workspaceRoots: readonly StatusWorkspaceRoot[] | undefined,
): StatusWorkspaceRoot | null {
	if (!workspaceRoots || workspaceRoots.length === 0) return null;
	const resolvedPath = path.resolve(inputPath);
	let bestMatch: StatusWorkspaceRoot | null = null;
	let bestLength = -1;
...
export function __setStatusLinePathRootOverridesForTests(overrides: StatusLinePathRootOverrides | null): void {
	...
	PROJECT_DIR_CLASSIFICATIONS.clear();
}
...
function getNormalizedScratchRoots(): readonly string[] {
...
function classifyProjectDir(projectDir: string): ProjectDirClassification {
	...
	for (const normalizedRoot of getNormalizedScratchRoots()) {
...
function renderPrimaryMode(ctx: SegmentContext): RenderedSegment {
	...
	if (plan && (plan.enabled || plan.paused)) {
		...
		return { content: color === "accent" ? accentFg(ctx, color, content) : theme.fg(color, content), visible: true };
	}
...
const pathSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		showScratchIcon = !workspaceRoot && classifyProjectDir(projectDir).scratch && stripPrefix;
...
const prSegment: StatusLineSegment = {
	...
	render(ctx) {
		...
		const label = withIcon(
			theme.icon.pr,
			status ? `#${statusValue(ctx, `${pr.number}`)} ${status}` : `#${statusValue(ctx, `${pr.number}`)}`,
		);
		const content =
			!ctx.startupPlaceholder && TERMINAL.hyperlinks && isSafePrUrl(pr.url)
