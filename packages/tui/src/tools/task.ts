...
export function renderAgentProgress(
	...
	let rowLine = row.line;
	// Fork `/workflow` frames ask for the textual resolved-model and agent-name
	// badges the shared tree row does not carry; every other row renders as-is.
	if (renderProgressOptions?.showResolvedModelBadge && progress.resolvedModel) {
		rowLine += `${theme.sep.dot}${theme.fg("dim", truncateToWidth(replaceTabs(progress.resolvedModel), 30))}`;
	}
	if (renderProgressOptions?.showAgentName && progress.agent) {
		rowLine += `${theme.sep.dot}${theme.fg("dim", truncateToWidth(replaceTabs(progress.agent), 30))}`;
	}

	lines.push(truncateTaskRow(rowLine, maxWidth, ""));
	...
	if (progress.reviewGate) {
		...
		lines.push(gateLine);
		if (gate.current) {
			for (const line of renderAgentProgress(gate.current, "", "  ", expanded, theme, spinnerFrame, options)) {
				lines.push(`${continuePrefix}${line}`);
			}
		}
	}
...
function renderAgentResult(
	...
	const icon = aborted
		...
		: reviewBlocked || needsWarning
			? theme.status.warning
			: success
				? theme.styledSymbol("status.done", "text")
				: theme.status.error;
	const iconColor = reviewBlocked || needsWarning || mergeFailed ? "warning" : success ? "success" : "error";
...
export interface TaskItem {
	...
	isolated?: boolean;
	/** Explicit model selector or fallback chain for this spawn; overrides the agent's model. */
	model?: string | string[];
	/** Hard wall-clock ceiling for this spawn, in seconds. `0` = unlimited. */
	max_runtime_seconds?: number;
	/** Run the review gate for this spawn regardless of the agent's own policy. */
	self_review?: boolean;
	/** Legacy fork alias for {@link name}. */
	id?: string;
	/** Legacy fork alias for the short task label. */
	description?: string;
	/** Legacy fork alias for the agent role/type. */
	role?: string;
	/** Legacy fork alias for {@link task}. */
	assignment?: string;
}
...
export interface TaskParams {
	...
	isolated?: boolean;
	/** Explicit model selector or fallback chain (flat form); overrides the agent's model. */
	model?: string | string[];
	/** Hard wall-clock ceiling for the flat-form spawn, in seconds. `0` = unlimited. */
	max_runtime_seconds?: number;
	/** Run the review gate for the flat-form spawn regardless of the agent's own policy. */
	self_review?: boolean;
	/** Legacy fork alias for {@link name}. */
	id?: string;
	/** Legacy fork alias for the short task label. */
...
/** Terminal outcome of the fork review gate that wraps an implementation subagent. */
...
/**
 * Aggregated review-gate result attached to a {@link SingleResult}. Present only
 * when the gate was enabled for the task. The `iterations` array preserves the
 * full review-fix history; `finalReview` mirrors the last iteration's structured
...
/** Live review-gate activity for a settled implementer while reviewer/fixer subagents run. */
...
/** Which leg of the review gate a subagent run belongs to. */
export type SubagentRunPhase = "work" | "review" | "fix";

/** Why a subagent run stopped early. */
export type SubagentAbortReason = "signal" | "terminate" | "timeout" | "budget";

/** Settled state of a subagent run. */
...
export interface AgentProgress {
	...
	outputTokens: number;
	/**
	 * Sum of provider-reported assistant request durations. Throughput displays
	 * divide `outputTokens` by this, not by `durationMs`, so queueing and tool
	 * time never deflate the rate.
	 */
	modelMs?: number;
	/**
