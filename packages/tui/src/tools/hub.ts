...
export interface IrcDeliveryReceipt {
	...
	/** True when a parked recipient was revived first; independent of `outcome` (fork AgentRegistry revival). */
	revived?: boolean;
	error?: string;
}
...
function renderSendResult(
	...
	if (receipts.length === 1) {
		...
		if (receipt.revived && receipt.outcome !== "revived") meta.push(theme.fg("accent", "revived"));
	} else {
			...
			...renderTreeList<IrcDeliveryReceipt>(
				{
					...
					renderItem: receipt => {
						...
						const revived =
							receipt.revived && receipt.outcome !== "revived" ? ` ${theme.fg("accent", "revived")}` : "";
						const error =
							receipt.outcome === "failed" && receipt.error
								? ` ${theme.fg("error", `${theme.format.dash} ${receipt.error}`)}`
								: "";
						return `${theme.fg("toolOutput", receipt.to)} ${badge}${revived}${error}`;
					},
				},
				theme,
			),
