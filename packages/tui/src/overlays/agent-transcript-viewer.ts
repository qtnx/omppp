...
import type { AgentLifecycleLike, AgentRecordLike } from "./agent-hub-types";
...
export interface AgentTranscriptViewerDeps<TRecord extends AgentRecordLike = AgentRecordLike> {
	agentId: string;
	...
	/**
	 * Revive+prompt path for messageable local agents. Lazy to avoid touching the
	 * global, and may resolve to `undefined` while the agent ref is not yet live.
	 */
	lifecycle?: () => AgentLifecycleLike<TRecord> | undefined;
	ui: TUI;
...
export class AgentTranscriptViewer<TRecord extends AgentRecordLike = AgentRecordLike> implements Component {
	...
	readonly #deps: AgentTranscriptViewerDeps<TRecord>;

	constructor(deps: AgentTranscriptViewerDeps<TRecord>) {
		this.#deps = deps;
	...
	get #sendable(): boolean {
		...
		// Resolve the thunk: a provisional agent has a lifecycle factory that still
		// returns nothing, and its transcript stays read-only until the session exists.
	...
	#submit(text: string): void {
		...
		const getLifecycle = this.#deps.lifecycle;
		if (!getLifecycle) return;
		void (async () => {
			try {
				// Revives a parked agent; returns the live session for running/idle.
				// The factory yields nothing while the agent ref is not live yet.
