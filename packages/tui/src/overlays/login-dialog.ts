 ...
 * on this host's tailnet address (fork port of `utils/oauth-tailnet.ts`).
 */
const TAILNET_SHORTCUT_LABEL = "Tailscale shortcut (other devices):";

/** Prompt the user to swap `localhost` for the tailnet host when the callback lands elsewhere. */
function tailnetCallbackHint(tailnetLaunchUrl: string): string {
