...
export class Markdown implements Component, NativeScrollbackCommittedRows, NativeScrollbackReplay {
	...
	#renderContentLines(
		...
		for (let i = start; i < end; i++) {
			...
			const tokenRowStart = rowOffset + contentLines.length;
			...
			if (tableSpecs !== undefined) {
				for (let specIndex = tableSpecStart; specIndex < tableSpecs.length; specIndex++) {
					...
					if (token.type === "table") {
						...
						relativeStart = 0;
						relativeEnd = Math.min(tokenLines.length, spec.lineCount);
					} else {
						// Container renderers express nested table spans relative to their
						// returned lines. Preserve that exact span through final wrapping.
						if (spec.startRow < 0 || spec.endRow <= spec.startRow) continue;
						relativeStart = Math.min(tokenLines.length, spec.startRow);
						relativeEnd = Math.min(tokenLines.length, spec.endRow);
					}
	...
	#appendFinalized(
		...
		for (const rawLine of renderedLines) {
			...
			lineOffsets?.push(out.length - outputStart);
		}
		return prevWasOsc66;
	}
