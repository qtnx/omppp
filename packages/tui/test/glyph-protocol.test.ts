...
/**
 * Every multiplexer marker the probe gate reads. The probe never runs inside a
 * multiplexer pane, so the suite must clear all of them — otherwise it passes
 * only on a direct terminal and fails for anyone running it from tmux, screen,
 * zellij, cmux/wmux, or a Herdr pane.
 */
...
const originalMultiplexerEnv: Record<string, string | undefined> = Object.fromEntries(
...
describe("glyph protocol probe", () => {
	...
	beforeEach(() => {
		...
		// `TERM=tmux*`/`screen*` classifies as a multiplexer on its own.
	...
	afterEach(() => {
		...
		for (const key in originalMultiplexerEnv) restoreEnv(key, originalMultiplexerEnv[key]);
