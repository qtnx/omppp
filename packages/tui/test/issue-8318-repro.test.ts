...
describe("issue #8318: scaled OSC 66 headings survive repaint and resize", () => {
	// A resize under Herdr (or Warp, or an explicit override) takes the in-place
	// path and skips the ED3 full repaint these assertions inspect. Neutralize the
	// ambient terminal identity so the direct-terminal repaint path is exercised
	// on any dev machine, not just a bare CI shell.
