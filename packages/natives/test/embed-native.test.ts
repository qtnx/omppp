...
describe("native addon embedding", () => {
	it("rejects a longer release sentinel that starts with the expected version", async () => {
		...
		try {
			...
			await Bun.write(path.join(nativeDir, "pi_natives.win32-arm64.node"), "binary__piNativesV1_11_10");
			...
			).rejects.toThrow("does not contain the @oh-my-pi/pi-natives@1.11.1 native ABI sentinel `__piNativesV1_11_1`");
