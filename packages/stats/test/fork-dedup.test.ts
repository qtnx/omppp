...
describe("stats sync deduplicates forked-session entries", () => {
	...
	it("collapses pre-existing duplicate rows on init (one-shot migration)", async () => {
		...
		// hand-inserted dupes, then plant duplicate provider, user, and
		// reminder rows that share (entry_id, timestamp) under different
		// session_files — exactly the shape `SessionManager.fork()` leaves behind.
		const database = new Database(getStatsDbPath());
		database.prepare("DELETE FROM meta WHERE key = ?").run("fork_dedupe_v2");
		const ts = Date.now();
		...
		`);
		const sharedReminder = ["/tmp/fork-dedup/project", ts, "gpt-5.4", "openai", "openai-responses"];
		insertReminder.run("/tmp/parent.jsonl", "rem01ab", ...sharedReminder);
		insertReminder.run("/tmp/fork.jsonl", "rem01ab", ...sharedReminder);
		...
		`);
		const sharedDelegation = ["/tmp/fork-dedup/project", ts, "gpt-5.4", "openai", "openai-responses", 1, 2, 3];
		insertDelegation.run("/tmp/parent.jsonl", "del01ab", ...sharedDelegation);
		insertDelegation.run("/tmp/fork.jsonl", "del01ab", ...sharedDelegation);
		...
		const overall = getOverallStats();
		const verificationDb = new Database(getStatsDbPath());
		try {
			const reminderCount = verificationDb
				.prepare("SELECT COUNT(*) AS count FROM system_context_reminders WHERE entry_id = ?")
				.get("rem01ab") as { count: number };
			const delegationCount = verificationDb
				.prepare("SELECT COUNT(*) AS count FROM delegation_reminders WHERE entry_id = ?")
				.get("del01ab") as { count: number };
