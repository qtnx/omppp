...
interface SummaryFold extends TraceSessionSummary {
	modelSet: Set<string>;
}
 ...
 */
export async function listSessionSummaries(limit = 100, q?: string): Promise<TraceSessionSummary[]> {
	const sessionsDir = getSessionsDir();
	...
	let rows: TraceSessionSummary[] = folds.map(({ modelSet: _modelSet, ...summary }) => summary);
	if (q?.trim()) {
