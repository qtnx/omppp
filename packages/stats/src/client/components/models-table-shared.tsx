...
/** System/delegation reminder cells used by the primary model statistics table. */
export function ModelReminderMetricCells({ model }: { model: ModelStats }) {
	return (
		<>
			...
			<div key="delegation-reminders" className="text-right text-[var(--text-secondary)] font-mono text-sm">
...
/** Expanded reminder-rate rows used by the primary model statistics table. */
export function ModelReminderDetailRows({ model }: { model: ModelStats }) {
	return (
		<>
			<div className="flex items-center justify-between">
				<span>System reminders</span>
				<span className="font-mono text-[var(--accent-cyan)]">
					{model.systemContextReminderCount.toLocaleString()} ({(model.systemContextReminderRate * 100).toFixed(1)}
					%)
				</span>
			</div>
			<div className="flex items-center justify-between">
				...
				<span className="font-mono text-[var(--accent-cyan)]">
					{model.delegationReminderCount.toLocaleString()} ({(model.delegationReminderRate * 100).toFixed(1)}%)
				</span>
			</div>
		</>
	);
