...
describe("AuthStorage forceRefresh + rotateSessionCredential", () => {
	...
	test("rotateSessionCredential(codex server_is_overloaded) parks the throttled account for minutes and rotates", async () => {
		...
		await authStorage.keys.get(PROVIDER, "block-sibling");
		const exhausted = await authStorage.limits.markReached(PROVIDER, "block-sibling", { retryAfterMs: 30_000 });
	...
	test("rotateSessionCredential parks long retry-after 429s until the hinted retry time and selects a sibling", async () => {
		...
		expect(rotated).toBe(true);
		const second = await authStorage.keys.get(PROVIDER, sessionId);
		expect(second).not.toBe(first);
