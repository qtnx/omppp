...
describe("Google transports drop Gemini-unsupported sampling fields (#12784)", () => {
	it("public Gemini API omits minP/repetitionPenalty but keeps supported knobs", async () => {
		...
		// Gemini 3 on the official endpoint auto-selects the Interactions API, whose
		// `generation_config` does accept min_p/repetition_penalty. This case pins the
		// generateContent transport that #12784 is about, so opt out explicitly.
		await drain(streamGoogle(geminiModel, context, { apiKey: "k", fetch, useInteractionsApi: false, ...sampling }));
