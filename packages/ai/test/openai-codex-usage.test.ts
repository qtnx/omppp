...
describe("openai-codex usage parser", () => {
	...
	it("backs off Spark and chat under separate scopes", () => {
		...
		expect(codexRankingStrategy.blockScopes?.({ modelId: "gpt-5.3-codex" })).toEqual(["chat", "shared", "main"]);
		...
		expect(codexRankingStrategy.blockScopes?.()).toEqual(["chat", "spark", "shared", "main"]);
	});
});

describe("codexRankingStrategy.scopeLimits", () => {
	const credential = {
		type: "oauth" as const,
	...
	it("gates non-spark models on the shared primary/secondary pool only", async () => {
		...
		const ids = codexRankingStrategy.scopeLimits?.(report, { modelId: "gpt-5.3-codex" }).map(l => l.id);
		expect(ids?.sort()).toEqual(["openai-codex:primary", "openai-codex:secondary"]);
	});

	it("gates spark models on the spark pool only", async () => {
		const report = await reportFrom(makePayload());
		const ids = codexRankingStrategy.scopeLimits?.(report, { modelId: "gpt-5.3-codex-spark" }).map(l => l.id);
	...
	it("ignores an absent model id and gates on the shared pool", async () => {
		const report = await reportFrom(makePayload());
		const ids = codexRankingStrategy.scopeLimits?.(report, undefined).map(l => l.id);
		expect(ids?.sort()).toEqual(["openai-codex:primary", "openai-codex:secondary"]);
	});
