...
describe("AuthStorage credential block persistence", () => {
	...
	it("backfills refresh leases for a v5 auth database", async () => {
		const legacyLeaseExpiresAt = Date.now() + 3_600_000;
		...
		legacyDb
			.prepare(
				"INSERT INTO auth_credentials (provider, credential_type, data, identity_key, refresh_lease_until) VALUES (?, ?, ?, ?, ?)",
			)
			.run(
				PROVIDER,
				"oauth",
				JSON.stringify({ access: "legacy-access", refresh: "legacy-refresh", expires: legacyLeaseExpiresAt }),
				"account:legacy",
		...
		try {
			expect(migratedStore.getCredentialRefreshLeaseExpiresAt(1)).toBe(legacyLeaseExpiresAt);
			expect(migratedStore.tryAcquireCredentialRefreshLease(1, "test-owner", legacyLeaseExpiresAt + 3_600_000)).toBe(
				false,
			);
			expect(readAuthSchemaVersion(dbPath)).toBe(7);
			expect(tableExists(dbPath, "auth_credential_refresh_leases")).toBe(true);
		} finally {
			migratedStore.close();
		}
	});

	it("initializes a refresh lease for a v5 auth database without a legacy lease", async () => {
		const legacyDb = new Database(dbPath);
		legacyDb.run(`
			CREATE TABLE auth_schema_version (
				id INTEGER PRIMARY KEY CHECK (id = 1),
