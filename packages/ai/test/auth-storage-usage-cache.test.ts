...
function writeStaleUsageCache(store: ObservableStore, row: StoredAuthCredential, report: UsageReport): void {
	...
	// The key carries the provider's usage cache version; read it from the
	// provider so a version bump cannot silently turn these seeds into misses.
	const key = `usage_cache:report:${claudeUsage.claudeUsageProvider.cacheVersion}:${row.provider}:default:oauth${account}${email}`;
	store.cache.set(key, {
...
describe("AuthStorage usage cache: last-good failure fallback", () => {
	...
	it("cold-fetches instead of replaying a report stored under the previous Anthropic cache version", async () => {
		const previousVersionKey = "usage_cache:report:anthropic:default:oauth|account:account-1|email:a@example.com";
		...
		const freshReport = { ...base, metadata: { ...base.metadata, source: "fresh-v2-fetch" } };
		...
		expect(reports[0]?.metadata?.source).toBe("fresh-v2-fetch");
	});
...
describe("AuthStorage usage cache: cache-first credential selection", () => {
	it("does not block getApiKey on a stale usage refresh when last-good reports exist", async () => {
		...
		try {
			const keyPromise = storage.keys.get("anthropic", "rank-session");
...
describe("AuthStorage usage cache: terminal refresh failure", () => {
	...
	it("suppresses last-good fallback when an expired OAuth access token has a definitive refresh failure", async () => {
		...
		const cacheKey = `usage_cache:report:${claudeUsage.claudeUsageProvider.cacheVersion}:anthropic:default:oauth|account:account-3|email:expired@example.com`;
	...
	it("preserves last-good fallback for transient (non-definitive) refresh failures", async () => {
		...
		const cacheKey = `usage_cache:report:${claudeUsage.claudeUsageProvider.cacheVersion}:anthropic:default:oauth|account:account-2|email:b@example.com`;
		cache.set(cacheKey, {
