...
/**
 * Inference headers want a session id on every request. Callers that share a
 * `providerSessionState` map across turns (agent loops, tests) but pass no
 * `sessionId` must still look like ONE session to the providers: Anthropic
 * keys its control state (strict-tools learning, prefix planning, cache
 * diagnostics) and GitLab Duo keys its workflow state on `sessionId`, so a
 * fresh id per call would reset that state every turn. Generate the id once
 * per state map; only stateless one-off calls get a per-call id.
 */
const inferredSessionIds = new WeakMap<Map<string, ProviderSessionState>, string>();
...
function withInferenceSessionId(options?: SimpleStreamOptions): SimpleStreamOptions {
	...
	if (!states) return { ...options, sessionId: crypto.randomUUID() };
	let sessionId = inferredSessionIds.get(states);
	if (!sessionId) {
		sessionId = crypto.randomUUID();
		inferredSessionIds.set(states, sessionId);
	}
...
export async function completeSimple<TApi extends Api>(
	...
	if (model.transport === "pi-native") {
		const requestOptions = withTransportFetch(model, sessionOptions);
...
function normalizeMandatoryReasoningOptions<TApi extends Api>(
	...
	const dropsThinkingWhenReasoningEffort =
		model.compat !== undefined &&
		"dropThinkingWhenReasoningEffort" in model.compat &&
		model.compat.dropThinkingWhenReasoningEffort === true;
	// GLM-5.3's OpenAI-compatible endpoints advertise `defaultLevel: max` for
	// the UI, but omitted wire effort must still start at the lowest tier. The
	// compat policy's drop-thinking marker identifies this mandatory low/high/max
	// dialect without changing defaults for models that rely on their declared
	// level (for example Grok).
	const effort = explicitlyDisabled || dropsThinkingWhenReasoningEffort ? floor : (defaultLevel ?? floor);
...
function mapOptionsForApi<TApi extends Api>(
	...
	switch (model.api) {
		...
		case "devin-agent": {
			...
			const effort = options?.disableReasoning ? undefined : resolveOpenAiReasoningEffort(devinModel, options);
