...
export function isHarmonyLeakMitigationTarget(model: Model): boolean {
	...
	if (compat !== undefined && "harmonyLeakMitigation" in compat) return compat.harmonyLeakMitigation === true;
	// Models built outside the catalog (mocks, ad-hoc specs) carry no compat; the
	// Codex transport is the only one that leaks Harmony scaffolding.
