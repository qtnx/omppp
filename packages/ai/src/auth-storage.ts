...
/**
/**
 * Grace a stale usage entry gets to refresh before ranking falls back to its
 * last-good value. Deliberately microtask-scale: a provider answering from its
 * own cache still re-ranks on current usage, while a slow network endpoint
 * never blocks `getApiKey` behind a refresh it does not need.
 */
...
// Keep the fork's configurable acquisition wait path while durable leases use
// the upstream 15-second default and five-second renewal cadence.
...
export class AuthStorage {
	...
	> = new Map();
	/** Tracks in-flight credential uses by provider/session for lifecycle accounting. */
	...
	#getCredentialBlockedUntil(
		...
		if (provider === "openai-codex" && !scopes.includes("shared")) {
	...
	/**
	 * Usage-limit backoff scope for a model. Prefer the current ranking-strategy
	 * context API, but keep the legacy model-only hook as a compatibility fallback
	 * for fork strategies that still expose it.
	 */
	#getBlockScope(
		strategy: CredentialRankingStrategy | undefined,
		context: CredentialRankingContext,
	): string | undefined {
		return strategy?.blockScope?.(context) ?? strategy?.backoffScope?.(context.modelId);
	}
	...
	/**
	 * Cache-first read used by credential ranking. Returns the last known report
	 * — fresh or last-good — without awaiting a refresh; `undefined` means
	 * nothing is cached and the caller must fetch. Ranking must never block
	 * `getApiKey` on a stale usage refresh while a last-good report exists.
	 */
	#peekUsageReportForRanking(
		...
		// The broker hook and api-key path bypass `#usageCache`, so there is
		// nothing to peek at and the caller must take the awaited path.
		if (credential.type !== "oauth" || this.#store.getUsageReport) return undefined;
		const cacheKey = this.#buildUsageReportCacheKey(this.#buildUsageRequestForOauth(provider, credential, baseUrl));
		const entry = this.#usageCache.getStale<UsageReport | null>(cacheKey);
	...
	async #usageReportForRanking(
		...
		if (!peeked) return this.#getUsageReport(provider, credential, options);
		if (peeked.fresh) return peeked.value;
		const refresh = this.#getUsageReport(provider, credential, options).catch(() => null);
	...
	#getScopedUsageLimits(
		...
		return (
			...
			strategy.selectGatingLimits?.(report, context.modelId) ??
			report.limits
		);
	}
	...
	#credentialBlockRouting(
		...
		// API keys are selected from the provider/type bucket; only OAuth pools
		// participate in model-scoped ranking and therefore scoped backoff.
		const scopedTarget = credentialType === "oauth";
		const defaultBlockScope = scopedTarget ? this.#getBlockScope(strategy, rankingContext) : undefined;
		const blockScope = blockScopeOverride ?? defaultBlockScope;
		const requestBlockScopes = scopedTarget
			? credentialBlockScopesForRequest(provider, strategy, rankingContext, defaultBlockScope)
			: [];
	...
	#compareUsageRankedCandidates(
		...
		const priority = this.#compareUsageRankedCandidateHeadroomPriority(left, right, planRequirement);
	...
	async #rankOAuthSelections(args: {
		...
		// Fetch every candidate's report in parallel before ranking. The timeout
		// preserves the hot-path bound while ensuring a cold cache cannot bypass
		// known exhausted or explicitly allowed accounts.
		const usageTimeout = Math.max(5000, this.#usageRequestTimeoutMs * 1.5);
		const usagePromise: Promise<Array<UsageRankingResult<OAuthCredential> | null>> = Promise.all(
			args.order.map(async idx => {
				const selection = args.credentials[idx];
				if (!selection) return null;
				...
				if (
					...
					usage = await this.#usageReportForRanking(args.provider, selection.credential, {
						...args.options,
						timeoutMs: this.#usageRequestTimeoutMs,
					});
					usageChecked = true;
	...
	async #stickyCredentialShouldYield(
		...
		for (const selection of credentials) {
			...
			if (this.#isCredentialBlocked(provider, providerKey, selection.index, blockScope)) continue;
	...
	async #resolveOAuthSelectionImpl(
		...
		await Promise.all(
			candidates.map(async candidate => {
				...
				try {
					...
					const storedCredentialId = this.#getStoredCredentials(provider)[candidate.selection.index]?.id;
					if (
						storedCredentialId !== undefined &&
						this.#syncOAuthSelectionFromStore(provider, candidate.selection, storedCredentialId) &&
					...
					if (isDefinitiveFailure) {
						...
						const attemptedCredential = candidate.selection.credential;
						...
						if (
							(outcome === "peer-rotated" || outcome === "cas-lost") &&
							credentialId !== undefined &&
							this.#syncOAuthSelectionFromStore(provider, candidate.selection, credentialId) &&
							!authCredentialEquals(attemptedCredential, candidate.selection.credential)
						) {
							// Resume only after reloading a demonstrably different credential.
							// A bare CAS loss leaves the stale rejected token in place, so the
							// final candidate pass must skip it rather than retrying refresh.
							return;
						}
		...
		const allowBlockedFallback = options?.allowBlockedOAuthFallback !== false;
		for (const pass of passes) {
			if (pass.allowBlocked && !allowBlockedFallback) continue;
	...
	async #refreshOAuthCredentialWithLease(
		...
		const tryAcquireCredentialRefreshLease = this.#store.tryAcquireCredentialRefreshLease?.bind(this.#store);
		const renewCredentialRefreshLease = this.#store.renewCredentialRefreshLease?.bind(this.#store);
		const releaseCredentialRefreshLease = this.#store.releaseCredentialRefreshLease?.bind(this.#store);
		if (!tryAcquireCredentialRefreshLease || !renewCredentialRefreshLease || !releaseCredentialRefreshLease) {
			return this.#refreshOAuthCredentialUnshared(provider, credential, credentialId, signal);
		}

		const owner = crypto.randomUUID();
		if (tryAcquireCredentialRefreshLease(credentialId, owner, Date.now() + this.#oauthRefreshLeaseTtlMs)) {
	...
	async #refreshOAuthCredentialHoldingLease(
		...
		let leaseRenewalError: unknown;
		const leaseRenewalStopped = Promise.withResolvers<void>();
		const leaseRenewal = (async () => {
			while (!stopLeaseRenewal) {
				await Promise.race([Bun.sleep(OAUTH_REFRESH_LEASE_RENEW_MS), leaseRenewalStopped.promise]);
				if (stopLeaseRenewal) return;
				if (!renewCredentialRefreshLease(credentialId, owner, Date.now() + this.#oauthRefreshLeaseTtlMs)) {
					throw new AIError.ConfigurationError("OAuth refresh ownership was lost before persistence");
				}
			}
		})().catch(error => {
		...
		try {
			...
			const expectedRow = this.#store.listAuthCredentials(provider).find(row => row.id === credentialId);
			const expected =
				expectedRow?.credential.type === "oauth" ? serializeCredential(provider, expectedRow.credential) : null;
			const updateIfMatches = this.#store.tryUpdateAuthCredentialIfMatches?.bind(this.#store);
			if (!expected || !updateIfMatches) {
				if (this.#replaceCredentialById(provider, credentialId, updated) === -1) return credential;
				return refreshed;
			}
			if (
				!updateIfMatches(credentialId, expected.data, updated, {
					owner,
					nowMs: Date.now(),
				})
			) {
				return this.#readChangedOAuthCredential(provider, credentialId, credential) ?? credential;
			}
			const entries = this.#getStoredCredentials(provider);
			const index = entries.findIndex(entry => entry.id === credentialId);
			if (index !== -1) {
				const nextEntries = [...entries];
				nextEntries[index] = { id: credentialId, credential: updated };
				this.#setStoredCredentials(provider, nextEntries);
			}
			return refreshed;
		} catch (error) {
			releaseLease =
				!(error instanceof AIError.AbortError) &&
				!(error instanceof AIError.OAuthError && error.kind === "timeout");
			throw error;
		} finally {
			stopLeaseRenewal = true;
			leaseRenewalStopped.resolve();
			await leaseRenewal;
			if (releaseLease) releaseCredentialRefreshLease(credentialId, owner);
		}
	}
	...
	async getApiKeyWithOrigin(
		...
		const configKey = this.#configOverrides.get(provider);
		if (configKey !== undefined && !options?.preferOAuth) {
			const apiKey = await this.#configValueResolver(configKey);
			this.#clearSessionCredential(provider, sessionId);
			return apiKey === undefined ? undefined : { apiKey, origin: { kind: "config" } };
		}

		const oauthOptions =
			configKey !== undefined && options?.preferOAuth ? { ...options, allowBlockedOAuthFallback: false } : options;
	...
	#clearHealedBlockScope(
		...
		// Probe the scope being healed plus every scope name this provider has ever
		// persisted under ("main" is the pre-rename chat scope, "shared" the
		// pre-split catch-all); missing one lets a fresh block heal immediately.
		const probedScopes = new Set(["", "chat", "main", "spark", "shared", blockScope ?? ""]);
		for (const probedScope of probedScopes) {
			reconcileAfterMs = Math.max(
				reconcileAfterMs,
				this.#readPersistedCredentialBlockReconcileAfter(credentialId, providerKey, probedScope),
			);
		}
