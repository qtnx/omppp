...
export interface UsageLedgerStore {
	...
	/**
	 * Append observed per-request costs for local cost aggregation. Stores
	 * without durable cost history (e.g. the broker remote store) omit it and
	 * the recording caller reports `false` instead of guessing an attribution.
	 */
	recordUsageCosts(entries: UsageCostHistoryEntry[]): void;
	/** Read recorded cost history, filtered by the query when supported. */
