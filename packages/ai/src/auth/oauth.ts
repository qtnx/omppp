...
import type { OAuthProvider, OAuthProviderId } from "../registry/oauth/types";
...
export class OAuthAccounts implements OAuthApi {
	...
	accounts(provider: string, sessionId?: string): OAuthAccountSummary[] {
		// Only a runtime `--api-key` hides the connected accounts: a config-sourced
		// key replaces the bytes a request authenticates with, but the user's
		// connected accounts are still theirs to see and manage.
	...
	/**
	 * Get the provider's OAuth `accountId`, preferring the credential that is
	 * session-sticky for `sessionId` and falling back to the first OAuth
	 * credential. Returns `undefined` when no OAuth credential carries an
	 * `accountId`. Read-only: never refreshes or advances selection.
	 */
	accountId(provider: string, sessionId?: string): string | undefined {
		// An explicit runtime/config API key authenticates the request, so OAuth
		// account attribution must not be injected alongside it.
		if (this.#deps.overrides.has(provider)) return undefined;
		const accountId = this.#deps.affinity.activeOAuth(provider, sessionId)?.accountId;
	...
	/**
	 * Force-refresh the stored credential whose secret matches `apiKey` and
	 * return its fresh API key. Reloads first so a peer's rotation is honoured,
	 * and refreshes through the durable lease path so the rotated token is
	 * persisted once.
	 */
	async refreshMatching(
		...
		// Reload first so a peer's token rotation is reflected: the force-refresh
		// must start from the persisted refresh token, not a superseded in-memory one.
		await this.#deps.pool.reload();
		const matched = this.#deps.pool
			.entries(provider)
			.find(entry => entry.credential.type === "oauth" && entry.credential.access === apiKey);
		if (!matched || matched.credential.type !== "oauth") return undefined;
		// An expired copy forces the refresh path even when the stored lease row is fresh.
		const stale: OAuthCredential = { ...matched.credential, expires: 0 };
		try {
			...
			// The row came back with the rejected token still in place: the refresh did
			// not produce new bytes, so this is a failure for the caller to rotate on.
			if (refreshed.access === matched.credential.access) return undefined;
			const resolved = await getOAuthApiKey(provider as OAuthProvider, { [provider]: refreshed });
			return resolved?.apiKey;
		} catch (error) {
			// The matched row is deliberately left enabled: the caller retries through
			// rotation instead, and a transport blip must not disable a credential.
			// Log the failure class only — the raw provider body can carry token material.
			const message = String(error);
			logger.debug("Force-refresh of the matched credential failed; falling back to rotation", {
				provider,
				cause: message.includes("invalid_grant") ? "invalid_grant" : "transient_or_network",
			});
			return undefined;
		}
	}
