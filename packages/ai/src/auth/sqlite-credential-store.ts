...
function hasSqliteErrorCode(err: unknown): err is { code: string } {
...
export function isSqliteBusyError(err: unknown): boolean {
	if (!hasSqliteErrorCode(err)) return false;
	return err.code.startsWith("SQLITE_BUSY");
}
...
const TRANSIENT_SQLITE_IOERR_CODES: Record<string, true> = {
	SQLITE_IOERR_FSTAT: true,
	SQLITE_IOERR_READ: true,
	SQLITE_IOERR_WRITE: true,
	SQLITE_IOERR_SHORT_READ: true,
	SQLITE_IOERR_LOCK: true,
};

/** Whether a store-open failure may clear with a bounded retry. */
export function isSqliteTransientError(err: unknown): boolean {
	if (isSqliteBusyError(err)) return true;
	if (!hasSqliteErrorCode(err)) return false;
	return TRANSIENT_SQLITE_IOERR_CODES[err.code] === true;
}
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// so retry a narrow set of transient SQLite failures with bounded
	...
	#initializeSchema(): void {
		...
		// Keep the WAL transition separate from schema DDL so a transient
		// WAL filesystem error retries the whole open without partially
		// interpreting the following schema statements.
