...
export type AuthStorageOptions = {
	...
	/** Durable refresh-lease TTL (ms). Defaults to 15s. */
	oauthRefreshLeaseTtlMs?: number;
	/** Poll interval (ms) while another process holds a refresh lease. Defaults to 50ms. */
	oauthRefreshLeasePollMs?: number;
	/**
	 * Give up waiting for a peer-held refresh lease after this long (ms) and
	 * resolve from the credential already on disk, posting nothing. Defaults to
	 * the lease TTL plus 15s.
	 */
...
export type AuthApiKeyOptions = {
	...
	/**
	 * Cap on the wait for a peer-held OAuth refresh lease to release (and on the
	 * broker-bound refresh round trip) before this caller gives up and resolves
	 * from the credential it already holds.
	 */
	timeoutMs?: number;
...
/**
 * A resolved API key plus the credential stratum it came from — the
 * origin-bearing counterpart of {@link AuthStorage.keys.get}, used by callers
 * that must report or audit which credential authenticated a request.
 */
...
export interface StoredOAuthRefreshResult<T extends OAuthCredential = OAuthCredential> {
	...
	removed: boolean;
	/**
	 * The acquisition wait for a peer-held refresh lease expired, so nothing was
	 * posted and `credential` is exactly what is on disk. Callers resolve with it
	 * (the peer owns the refresh) instead of treating a stale token as a failure.
	 */
	leaseWaitTimedOut?: boolean;
}
...
export interface KeysApi {
	 ...
	 * Auth-gateway auditing uses it to report `credentialOrigin` /
	 * `credentialAuthType` for a request it is proxying.
	 */
	withOrigin(
		provider: string,
		sessionId?: string,
...
export interface OAuthApi {
	...
	/**
	 * The provider's OAuth `accountId`, preferring the session-sticky credential.
	 * Read-only; returns `undefined` when no OAuth credential carries one.
	 */
	 ...
	 * return its fresh API key. The auth-retry path uses it when a request was
	 * rejected with a token the storage has already replaced, so the retry does
	 * not replay the rejected bytes.
	 */
	refreshMatching(provider: string, apiKey: string, options?: { signal?: AbortSignal }): Promise<string | undefined>;
}
...
export interface UsageApi {
	...
	/**
	 * Synchronous, advisory headroom probe for model routing: it reads cached
	 * usage and credential blocks for the model's ranking scope and never
	 * resolves tokens or waits on a usage fetch. Missing reports, missing scoped
	 * limits, providers without a ranking strategy, and broker-backed stores are
	 * optimistic — only a cached exhausted/over-threshold window or a credential
	 * block reports no room.
	 */
	headroom(model: Model<Api>, opts?: { utilizationMax?: number; windowMode?: "all" | "any" }): UsageHeadroom;
	/**
	 * Record one observed provider request cost for later local usage
	 * aggregation. Returns `false` when the store keeps no cost history or the
	 * request's credential cannot be attributed unambiguously.
	 */
	recordCost(
		provider: Provider,
		costUsd: number,
