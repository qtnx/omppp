...
/** Default cap on how long a caller waits for a peer-held refresh lease. */
const OAUTH_REFRESH_LEASE_WAIT_TIMEOUT_MS = OAUTH_REFRESH_LEASE_TTL_MS + 15_000;
...
/** Per-call behaviour for {@link OAuthRefresher.refresh}. */
export interface RefreshOptions {
	/** Keep the stored row when the refresh fails; the caller owns the fallback rotation. */
...
export interface OAuthRefresherDeps {
	...
	/** Durable lease TTL; tests shorten it to observe handoff. Defaults to {@link OAUTH_REFRESH_LEASE_TTL_MS}. */
	leaseTtlMs?: number;
	/** Poll interval while waiting for a peer-held lease. Defaults to {@link OAUTH_REFRESH_LEASE_POLL_MS}. */
	leasePollMs?: number;
	/** Acquisition wait cap before resolving from disk without posting. Defaults to {@link OAUTH_REFRESH_LEASE_WAIT_TIMEOUT_MS}. */
...
export class OAuthRefresher {
	...
	async refreshStored<T extends OAuthCredential = OAuthCredential>(
		...
		const leaseTtlMs = this.#deps.leaseTtlMs ?? OAUTH_REFRESH_LEASE_TTL_MS;
		const leasePollMs = this.#deps.leasePollMs ?? OAUTH_REFRESH_LEASE_POLL_MS;
		const leaseWaitTimeoutMs = this.#deps.leaseWaitTimeoutMs ?? OAUTH_REFRESH_LEASE_WAIT_TIMEOUT_MS;
		// Accumulated wait rather than wall-clock: the wait is timer-driven, so it must
		// hold under virtual timers too (tests advance timers, not the clock).
		let waitedMs = 0;
		...
		while (leaseStore) {
			...
			if (leaseStore.tryAcquireCredentialRefreshLease(row.id, owner, Date.now() + leaseTtlMs)) {
				leasedCredentialId = row.id;
				break;
			}
			// A peer's lease outlived our patience: hand back the credential that is
			// already on disk and post nothing, rather than blocking the caller.
			const leaseExpiresAt = leaseStore.getCredentialRefreshLeaseExpiresAt(row.id);
			const waitMs =
				leaseExpiresAt === undefined
					? leasePollMs
					: Math.min(Math.max(leaseExpiresAt - Date.now(), leasePollMs), 250);
			if (waitedMs + waitMs > leaseWaitTimeoutMs) {
				return { credential: current, refreshed: false, removed: false, leaseWaitTimedOut: true };
			}
			waitedMs += waitMs;
	...
	async #refreshOAuthCredentialUnshared(
		...
		if (credentialId !== undefined && hasRefreshLeases(this.#deps.store)) {
			...
			const result = await this.refreshStored(provider, {
				...
				// Callers that own the fallback (bearer-matched refresh) keep the row so
				// they can rotate instead of losing the credential to one failure.
				...(options?.keepCredentialOnRefreshFailure
			...
			if (result.credential) {
				...
				} else if (result.leaseWaitTimedOut) {
					// A peer held the lease past our patience: resolve with what is on
					// disk instead of failing the request or re-posting the refresh.
