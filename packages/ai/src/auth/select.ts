...
export class CredentialSelector {
	...
	async #rankApiKeySelections(args: {
		...
		const usagePromise: Promise<Array<UsageRankingResult<ApiKeyCredential> | null>> = Promise.all(
			args.order.map(async idx => {
				...
				const usage = await this.#deps.usage.reportForRanking(args.provider, selection.credential, {
					...args.options,
		...
		const usagePromise = Promise.all(
			args.order.map(async idx => {
				...
				if (
					...
					usage = await this.#deps.usage.reportForRanking(args.provider, selection.credential, {
						...args.options,
						timeoutMs: this.#deps.usage.requestTimeoutMs,
					});
					usageChecked = true;
	...
	/**
	 * Cache-first usage probe for the sticky gate. Falls back to an awaited
	 * Anthropic fetch only when the request carries a model id, so the gate can
	 * still see model-scoped parity data when the cache holds none.
	 */
	async #stickyProbeReport(
		provider: string,
		credential: OAuthCredential,
		options?: AuthApiKeyOptions,
	): Promise<UsageReport | null> {
		const cached = this.#deps.usage.cachedReport(provider, credential, options?.baseUrl);
		if (cached) return cached;
		if (provider === "anthropic" && options?.modelId) return this.#deps.usage.report(provider, credential, options);
		return null;
	}

	/**
	 * Keep sticky probes in acquisition parity with {@link CredentialSelector.#rankOAuthSelections}.
	 * If the gate yields on usage data the ranker will not also see and hard-block, the re-pin
	 * step restores the sticky credential and the session ranks every request without switching.
	 * Read-only: it never blocks, mutates, or refreshes a credential.
	 */
	async #stickyShouldYield(
		provider: string,
		providerKey: string,
		stickyIndex: number,
		credentials: readonly { credential: OAuthCredential; index: number }[],
		strategy: CredentialRankingStrategy,
		rankingContext: CredentialRankingContext,
		...
		const stickyReport = await this.#stickyProbeReport(provider, sticky.credential, options);
		if (!stickyReport) return false;
		if (!isUsageLimitReached(scopedUsageLimits(strategy, stickyReport, rankingContext))) return false;

		for (const selection of credentials) {
			if (selection.index === stickyIndex) continue;
			if (this.#deps.blocks.isBlocked(provider, providerKey, selection.index, blockScope)) continue;
			const siblingReport = await this.#stickyProbeReport(provider, selection.credential, options);
			// A sibling we cannot measure counts as room: the ranker will fetch it.
			if (!siblingReport) return true;
			if (!isUsageLimitReached(scopedUsageLimits(strategy, siblingReport, rankingContext))) return true;
		}
		return false;
	}
	...
	async resolveOAuth(
		...
		await Promise.all(
			candidates.map(async candidate => {
				...
				// blocked-fallback pass refreshes it itself. Refreshing it here only
				// repeats a round trip that already failed — a transient refresh failure
				// blocks the row for OAUTH_REFRESH_FAILURE_BACKOFF_MS.
				if (!force && this.#deps.blocks.isBlocked(provider, providerKey, candidate.selection.index, blockScopes)) {
					return;
				}
		...
		// Gateway callers that prefer the OAuth pool (`preferOAuth`) treat an
		// all-blocked pool as unavailable rather than re-probing a credential the
		// pool itself parked.
	...
	async tryOAuth(
		...
		if ((checkUsage && !allowBlocked) || hasPlanRequirement) {
			if (usagePrechecked) {
				...
				// Cache-first, matching the fork's preflight: a broker-backed store
				// hands the snapshot to its own hook and the preflight sees "unknown"
				// rather than awaiting an authoritative read that would reject a
				// pinned credential on a report the ranker never consulted.
				usage = this.#deps.usage.cachedReport(provider, selection.credential, options?.baseUrl);
				usageChecked = true;
			}
		...
		try {
			...
			if ((checkUsage && !allowBlocked) || hasPlanRequirement) {
				...
				if (!usageChecked || !sameAccount) {
					usage = this.#deps.usage.cachedReport(provider, updated, options?.baseUrl);
					usageChecked = true;
				}
				if (applyPlanFilter && planGate?.(usage) !== true) {
					return undefined;
