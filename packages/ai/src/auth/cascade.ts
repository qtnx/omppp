...
export class KeyCascade implements KeysApi {
	...
	/**
	 * Origin-bearing form of {@link KeyCascade.get}: same precedence, one
	 * decision, plus the credential stratum the winning key came from. Callers
	 * that must report or audit which credential authenticated a request (the
	 * auth-gateway) use this instead of calling `get` and `source` separately,
	 * which could disagree when a refresh rotates the pool in between.
	 */
	async withOrigin(
		provider: string,
		sessionId?: string,
		...
		// won't accept. Gateway callers can opt into OAuth-pool-first resolution
		// (`preferOAuth`) while retaining this key as the fallback.
		const configKey = this.#deps.overrides.configKey(provider);
		if (configKey !== undefined && !options?.preferOAuth) {
			const apiKey = await this.#deps.overrides.resolve(configKey);
			if (sessionId) this.#deps.affinity.forget(provider, sessionId);
			return apiKey === undefined ? undefined : { apiKey, origin: { kind: "config" } };
		}

		// A gateway that prefers OAuth must not fall back to an already-blocked
		// credential: the pool is the point, and its blocks are its own signal.
		const oauthOptions =
		...
		const loginApiKeySelection = await this.#deps.selector.selectApiKey(
			...
			credential =>
				credential.source === "login" &&
				!this.isKeylessFallback(provider, credential) &&
				!this.#unresolvedApiKeyCredentials.has(`${provider}:${credential.key}`),
		);
		if (loginApiKeySelection) {
			...
			// missing env var, a failing `!command`). Remember that so later
			// requests stop paying the resolver, and keep descending the cascade.
			this.#unresolvedApiKeyCredentials.add(`${provider}:${loginApiKeySelection.credential.key}`);
		}
	...
	/**
	 * Credentials whose config reference last failed to resolve, keyed by
	 * `provider:reference`. Skipped by the cascade so a broken `!command` or a
	 * missing env var is not re-resolved on every request.
	 */
	#unresolvedApiKeyCredentials: Set<string> = new Set();

	/** Drop memoized resolution failures for a provider: its credential list changed. */
	forgetUnresolvedApiKeys(provider: string): void {
		const prefix = `${provider}:`;
		for (const key of [...this.#unresolvedApiKeyCredentials]) {
			if (key.startsWith(prefix)) this.#unresolvedApiKeyCredentials.delete(key);
		}
	}
