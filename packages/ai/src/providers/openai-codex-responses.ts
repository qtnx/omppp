...
export interface OpenAICodexWebSocketDebugStats {
	...
	websocketHandshakeSuccesses: number;
	websocketHandshakeFailures: number;
	websocketReconnects: number;
	websocketDisconnects: number;
	websocketFallbacks: number;
	lastWebSocketHandshakeMs?: number;
	totalWebSocketHandshakeMs: number;
	lastWebSocketFirstEventMs?: number;
	totalWebSocketFirstEventMs: number;
	...
	sseSuccesses: number;
	sseFailures: number;
	lastSseConnectMs?: number;
	totalSseConnectMs: number;
	lastSseStatus?: number;
	lastSseError?: string;
...
type CodexWebSocketSessionState = {
	...
	prewarmed: boolean;
	/** Current websocket turn-state token for chained metadata replay. */
	turnState?: string;
	stats: OpenAICodexWebSocketDebugStats;
};

interface CodexTransportTelemetryContext {
	provider: string;
	model: string;
	sessionId?: string;
	url?: string;
...
class CodexStreamRuntime {
	...
	constructor(initial: {
		...
		this.recordWebSocketAppendState = initial.recordWebSocketAppendState ?? initial.transport === "websocket";
	}
...
function getCodexWebSocketRetryDelayMs(retry: number): number {
	const baseDelay = parseCodexPositiveInteger($env.PI_CODEX_WEBSOCKET_RETRY_DELAY_MS, CODEX_WEBSOCKET_RETRY_DELAY_MS);
	return baseDelay * Math.max(1, retry);
}

function getCodexWebSocketConnectTimeoutMs(): number {
	return parseCodexPositiveInteger($env.PI_CODEX_WEBSOCKET_CONNECT_TIMEOUT_MS, CODEX_WEBSOCKET_CONNECT_TIMEOUT_MS);
}

function getCodexWebSocketPrewarmConnectTimeoutMs(): number {
...
function createCodexWebSocketTransportError(message: string): CodexWebSocketTransportError {
	return new CodexWebSocketTransportError(message);
}
...
function updateCodexSessionMetadataFromHeaders(
	...
	if (state && state.turnState === undefined && responseTurnState && responseTurnState.length > 0) {
		state.turnState = responseTurnState;
	}
...
function updateCodexSessionMetadataFromEvent(
	...
	if (!headers) return;
	updateCodexSessionMetadataFromHeaders(turnState, state, headers);
}
	...
	if (websocketState && shouldUseCodexWebSocket(model, websocketState, options?.preferWebsockets)) {
		...
		while (true) {
			try {
				...
				const isFatal = isCodexWebSocketFatalError(error);
				const isOversized = isCodexWebSocketOversizedError(error, websocketState);
...
async function openCodexWebSocketTransport(
	...
	if (websocketRequest.input && Array.isArray(websocketRequest.input)) {
		const hasUserInput = (websocketRequest.input as InputItem[]).some(isCodexUserInputItem);
		if (hasUserInput && options?.codexCompaction?.phase !== "mid_turn") {
			websocketState.turnState = undefined;
		} else {
			attachCodexWebSocketClientMetadataTurnState(websocketRequest, websocketState);
		}
	}
	...
	const eventStream = useDedicatedConnection
		? closeCodexWebSocketWhenDone(requestEventStream, websocketConnection)
		: requestEventStream;
	return {
		eventStream,
		requestBodyForState,
...
async function openCodexSseTransport(
	...
	if (
		...
		!requestContext.isolatedTransportState &&
		isCodexWebSocketPreferred(model, options?.preferWebsockets)
	) {
...
function scheduleCodexWebSocketBackgroundReconnect(
	...
	state.backgroundReconnect = (async () => {
		try {
			...
			await getOrCreateCodexWebSocketConnection(state, requestContext.turnState, url, websocketHeaders, {
				provider: model.provider,
			...
			recordCodexWebSocketFailure(state, false, {
				oversized: error instanceof Error && isCodexWebSocketOversizedError(error, state),
			});
...
class CodexStreamProcessor {
	...
	async #tryReconnectWebSocketOnConnectionLimit(error: unknown): Promise<boolean> {
		...
		if (this.output.content.length > 0) {
			// Content already reached the consumer. Replaying the full request would
			// emit those deltas again, so surface the connection-limit error instead.
			return false;
		}
		...
		this.runtime.resetAccumulators();
		this.firstTokenTime = undefined;
		if (this.runtime.websocketStreamRetries >= getCodexWebSocketRetryBudget()) {
			recordCodexWebSocketFailure(websocketState, true);
			await this.#reopenSseStream(websocketState);
			return true;
		}
		this.runtime.websocketStreamRetries += 1;
		await scheduler.wait(getCodexWebSocketRetryDelayMs(this.runtime.websocketStreamRetries), {
			signal: this.requestSetup.requestSignal,
		});
		await this.#reopenWebSocketStream(websocketState);
		return true;
	...
	async #reopenWebSocketStream(state: CodexWebSocketSessionState): Promise<void> {
		try {
			...
			this.runtime.recordWebSocketAppendState = next.recordWebSocketAppendState;
			...
			recordCodexWebSocketFailure(state, true, { oversized: isCodexWebSocketOversizedError(error, state) });
	...
	(async () => {
		...
		try {
			...
			logCodexTransport({
				...
				model: model.id,
				sessionId: requestContext.transportSessionId,
				websocketDisabled: requestContext.websocketState?.disableWebsocket ?? false,
				fallbackCount: requestContext.websocketState?.fallbackCount ?? 0,
			});
...
function buildCodexChainedRequestBody(
	...
	if (appendInput && appendInput.length > 0 && state?.lastResponseId) {
		...
		if (appendInput.some(isCodexUserInputItem)) {
			...
			attachCodexWebSocketClientMetadataTurnState(body, state);
		}
		return body;
	}
	if (chainable && state) {
...
function getWebSocketCloseDetails(event: Event): { code?: number; reason?: string } {
...
class CodexWebSocketConnection {
	...
	constructor(url: string, headers: Record<string, string>, options: CodexWebSocketConnectionOptions) {
		...
		this.#connectTimeoutMs = options.connectTimeoutMs ?? getCodexWebSocketConnectTimeoutMs();
	}
	...
	async connect(signal?: AbortSignal): Promise<void> {
		...
		try {
			...
			throw createCodexWebSocketTransportError(
				`websocket constructor failed: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	...
	async *streamRequest(
		...
		try {
			...
			while (true) {
				...
				if (!sawFirstEvent) {
					this.#onFirstEvent?.({
						durationMs: Date.now() - requestSentAt,
	...
	#push(item: Record<string, unknown> | Error | null): void {
		...
		const queueCapacity = getCodexWebSocketMessageQueueCapacity();
		if (item !== null && this.#queue.length >= queueCapacity) {
			this.#failQueue(
				new CodexWebSocketTransportError(`websocket message queue exceeded ${queueCapacity} items`),
				"queue-overflow",
			);
			return;
		}
...
function createCodexWebSocketConnectionForState(
	...
	return new CodexWebSocketConnection(url, headerRecord, {
		onHandshakeHeaders: handshakeHeaders => {
			updateCodexSessionMetadataFromHeaders(options?.turnState, state, handshakeHeaders);
		},
...
async function openCodexSseEventStream(
	...
	// A non-2xx response keeps the guard armed past this block: its error body is
	// read below and a stalled body must still hit the deadline (issue #12664).
	let keepGuardArmed = false;
	try {
		...
		keepGuardArmed = !response.ok;
	} catch (error) {
		const durationMs = Date.now() - startedAt;
		const message = error instanceof Error ? error.message : String(error);
		if (state) {
			state.stats.sseFailures += 1;
			state.stats.lastSseConnectMs = durationMs;
			state.stats.totalSseConnectMs += durationMs;
			state.stats.lastSseError = message;
		}
		logCodexTransport({
			phase: "sse_error",
			transport: "sse",
		...
		if (!keepGuardArmed) clearPreResponseTimeout?.();
	}
	const durationMs = Date.now() - startedAt;
	if (state) {
		state.stats.lastSseConnectMs = durationMs;
		state.stats.totalSseConnectMs += durationMs;
		state.stats.lastSseStatus = response.status;
	}
	...
	if (!response.ok) {
		...
		try {
			const parsedError = await CodexApiError.fromResponse(response);
			const error =
				parsedError.status !== 401 && CODEX_AUTH_ERROR_MESSAGE.test(parsedError.info.raw || parsedError.message)
					? new CodexApiError({ ...parsedError.info, status: 401 }, response.headers)
					: parsedError;
			if (state) {
				state.stats.sseFailures += 1;
				state.stats.lastSseError = error.message;
			}
			logCodexTransport({
				phase: "sse_error",
				transport: "sse",
	...
	if (state) {
		state.stats.sseSuccesses += 1;
		state.stats.lastSseError = undefined;
	}
	logCodexTransport({
		phase: "sse_response",
		transport: "sse",
...
function logCodexTransport(details: Record<string, unknown>): void {
	logger.debug("OpenAI Codex transport", details);
}
