...
function buildClaudeCodeBetas({
	...
	if (!agentRequest && !redactThinking && !disableStrictTools && supportsContextManagement)
...
function getCacheControl(
	...
	// OAuth on the official Anthropic API matches Claude Code's native 1h policy where the model
	// line supports it, while API-key, gateway, Foundry, and non-Anthropic routes stay short.
	// An explicit cacheRetention or PI_CACHE_RETENTION always takes precedence.
	const fallback =
		isOAuth &&
...
function convertContentBlocks(
	...
	for (const block of content) {
		...
		if (block.providerFile?.provider !== "anthropic" && !block.url && !isImageContentAvailable(block)) {
...
/** Skip OAuth cloak blocks that cannot be cached across requests. */
		...
		if (options?.thinkingEnabled || model.compat.requiresThinkingEnabled) {
			...
			if (mode === "anthropic-adaptive" && !compat.disableAdaptiveThinking) {
				...
				if (model.thinking?.supportsDisplay) {
					adaptive.display = effectiveThinkingDisplay(model, true, thinkingOptions.thinkingDisplay);
				}
				thinking = adaptive;
