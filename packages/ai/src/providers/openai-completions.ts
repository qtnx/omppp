 ...
 * assistant output commits the attempt. OpenRouter 402s that reserve more
 * output than remaining credit can cover retry once with the advertised cap
 * inside the replay-safe attempt.
 */
	...
	for (let i = 0; i < transformedMessages.length; i++) {
		...
		if (msg.role === "user" || msg.role === "developer") {
			...
			for (; j < transformedMessages.length && transformedMessages[j].role === "toolResult"; j++) {
				...
				const availableImageContent = imageContent.filter(
					image => isImageContentAvailable(image) || image.url !== undefined,
				);
