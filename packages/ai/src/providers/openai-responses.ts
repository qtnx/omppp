...
function isXaiOAuthGrok46Model(model: Model<"openai-responses">): boolean {
	return bareModelId(model.requestModelId ?? model.id)
		.toLowerCase()
		.startsWith("grok-4.6");
}
...
export function buildParams(
	...
	const outputToken = resolveOpenAIOutputTokenParam({
		...
		providerOutputClamp: model.compat.providerOutputClamp ?? resolveOpenAIResponsesOutputClamp(model),
	});
		...
		model.provider === "xai-oauth"
			? options?.reasoning === undefined
				...
				: isXaiOAuthGrok46Model(model)
					...
					: null
			: resolveReasoningSummaryOption(model, options);
