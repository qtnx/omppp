...
// --target=bun-linux-arm64, embeds the matching native, outputs dist/ompx-<target>.
...
const outName = crossTarget ? `ompx-${crossTarget}` : "ompx";
		...
		try {
			...
			await runCommand(
				[
					...
					// Worker re-entry dispatch is handled by `cli.ts` declaring the
					// compiled binary as the worker host and routing hidden argv
					// selectors before command registration. Legacy pi-* extension
					// compat surfaces are served through the virtual registry backed by
					// `legacy-pi-bundled-registry.ts`, which static-imports each
					// surface so the bundler includes them via the main module graph.
					"--outfile",
