 ...
 */
function extractApprovalPaths(args: unknown, mode: EditMode, cwd?: string): string[] {
	...
	if (input && mode === "hashline") {
		...
		if (hashlinePaths && hashlinePaths.length > 0) return hashlinePaths;
		const fallbackPaths = [...input.matchAll(/^\[([^#\r\n]+)(?:#[0-9a-fA-F]{4})?\]/gm)]
			.map(match => match[1])
	...
	// ACP may invoke the edit tool with a configured mode that differs from the
	// payload syntax. Still classify hashline headers (and move destinations)
	// before falling back to the record path; otherwise an internal URI payload
	// is treated as an unknown filesystem write.
	if (input) {
		const hashlinePaths = extractHashlineApprovalPaths(input, cwd);
		if (hashlinePaths && hashlinePaths.length > 0) return hashlinePaths;
		const fallbackPaths = [...input.matchAll(/^\[([^#\r\n]+)(?:#[0-9a-fA-F]{4})?\]/gm)]
...
export class EditTool implements AgentTool<TInput> {
	readonly approval = (args: unknown) => {
		...
		const targets = extractApprovalPaths(args, this.mode, this.session.cwd);
