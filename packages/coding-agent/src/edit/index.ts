...
/** Scheme-prefixed edit target owned by an internal-URL handler, not the filesystem. */
const INTERNAL_URL_TARGET_RE = /^[a-z][a-z0-9+.-]*:\/\//i;
...
export class EditTool implements AgentTool<TInput> {
	...
	async execute(
		...
		let outcome;
		try {
			// The native engine reads, patches, and writes every target inside
			// `apply`, so the per-file write lock has to span the whole call:
			// locking only the write callback would let a concurrent same-file edit
			// stage against pre-write bytes and clobber the first writer.
			outcome = await withFileWriteLocks(this.#mutationLockPaths(params), signal, () =>
				editSession.apply({ lspBatchId: batch?.id, lspFlush: batch?.flush ?? false }, (_error, request) =>
					this.#write(request, signal),
				),
			);
	...
	#inspect(args: unknown): EditInspection {
		...
		// Non-enumerable: the cache is internal bookkeeping on an object the
		// caller owns and may serialize or compare structurally.
		if (tagged)
			Object.defineProperty(tagged, kInspection, {
				value: { mode: this.mode, inspection },
				configurable: true,
				writable: true,
			});
		return inspection;
	...
	/**
	 * Filesystem paths this call may mutate, for the per-file write lock. Move
	 * intents contribute both source and destination so a rename serializes
	 * against an edit of either side. Internal-resource targets (`memory://`,
	 * `skill://`, …) are owned by their protocol handler and are left unlocked,
	 * exactly as `write` leaves router-handled writes. A payload the inspector
	 * cannot parse yields no paths — `apply` then fails on the same parse error
	 * without writing anything.
	 */
	#mutationLockPaths(params: EditParams): string[] {
		const inspection = this.#inspect(params);
		const targets = [...inspection.paths];
		for (const fileOp of inspection.fileOps) {
			targets.push(fileOp.path);
			if (fileOp.to) targets.push(fileOp.to);
		...
		return targets
			.filter(target => target.length > 0 && !INTERNAL_URL_TARGET_RE.test(target))
			.map(target => path.resolve(this.session.cwd, target));
	}
