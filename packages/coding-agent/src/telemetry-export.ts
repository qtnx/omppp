 ...
 * no-op tracer and the spans are silently dropped. This module registers OTLP
 * trace, log, and metric providers plus run-level callbacks when standard
 * `OTEL_*` endpoints are configured, allowing headless and embedded OMPx hosts
 * to export internal telemetry without vendor coupling.
 *
...
/**
 * Periodic flush interval. A long-lived `ompx` process (the ACP server is
