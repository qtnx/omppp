...
export async function loadProjectContextFiles(
	...
	if (!options.workspaceRoots || options.workspaceRoots.length === 0) {
		return dedupeExactContextFiles(await loadProjectContextFilesForCwd(resolvedCwd, options.disabledExtensions));
	}
...
export async function buildSystemPrompt(options: BuildSystemPromptOptions = {}): Promise<BuildSystemPromptResult> {
	...
	const contextFilesPromise = (async () => {
		...
		// Additional roots are context, but the active/tagged roots stay closest and win exact-content dedupe.
		return dedupeExactContextFiles([...extra.flat(), ...primary]);
	})();
