...
/** Keep the last occurrence of an exact absolute context-file path. */
export function dedupeExactContextFiles(
...
export async function loadProjectContextFiles(
	...
	if (!options.workspaceRoots || options.workspaceRoots.length === 0) {
		return dedupeContainedContextFiles(
			dedupeExactContextFiles(await loadProjectContextFilesForCwd(resolvedCwd, options.disabledExtensions)),
		);
	}
	...
	const contextFileGroups = await Promise.all(
		contextCwds.map(contextCwd => loadProjectContextFilesForCwd(contextCwd, options.disabledExtensions)),
	);
	...
	return dedupeContainedContextFiles(
		dedupeExactContextFiles(merged.filter((file, index) => lastIndexByPath.get(file.path) === index)),
	);
}
...
export interface BuildSystemPromptOptions {
	...
	/** Compact discoverable built-in tool summaries advertised when native discovery hides them. */
...
export async function buildSystemPrompt(options: BuildSystemPromptOptions = {}): Promise<BuildSystemPromptResult> {
	...
	const taggedWorkspaceRootPaths = new Set((workspaceRoots ?? []).map(root => path.resolve(root.path)));
	const additionalRoots = [
		...new Set(
			additionalWorkspaceRoots
				.map(root => path.resolve(root))
				.filter(root => root !== resolvedCwd && !taggedWorkspaceRootPaths.has(root)),
		),
	];
	const contextFilesPromise = (async () => {
		const primary = providedContextFiles
			? providedContextFiles
			: await logger.time("loadProjectContextFiles", loadProjectContextFiles, {
					cwd: resolvedCwd,
					workspaceRoots,
				});
		...
		// Additional roots are context, but the active/tagged roots stay closest and
		// win exact-path dedupe before contained-block dedupe removes redundant content.
		return dedupeContainedContextFiles(dedupeExactContextFiles([...extra.flat(), ...primary]));
	})();
	const workspaceTreePromise = (async () => {
		...
		if (additionalRoots.length === 0 || !includeWorkspaceTree) return primary;
		...
		return {
			...primary,
			agentsMdFiles: [...primary.agentsMdFiles, ...extraTrees.flatMap(tree => tree.agentsMdFiles)],
		};
	})();
	...
	const filteredSkills = hasSkillReader
		? skills
