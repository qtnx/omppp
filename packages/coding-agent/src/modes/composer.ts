	...
	#offerHistory(
		...
		if (!this.#headerRetired) {
			...
			if (renderedHeader.length > 0) {
				...
				const overflowing = renderedHeader.length + chromeRows + liveRows > rows;
				if (!this.#historyFlush && !overflowing) return undefined;
				// Overflow or flush: settle the intro so retirement commits the
				// resting frame instead of clipping the live header off-screen.
				if (welcome !== undefined && !welcome.isTranscriptBlockFinalized()) {
					welcome.stopIntro();
				}
				const settledHeader = this.#header.render(width);
				const headerRows = settledHeader.length > 0 ? settledHeader : renderedHeader;
				this.#offeredHistory = {
					id: this.#nextHistoryId++,
					rows: [...headerRows, ""],
					kind: "append",
