...
import { buildSkillPromptMessage, loadSkills, parseSkillInvocation, setActiveSkills } from "../../extensibility/skills";
...
export class AcpAgent implements Agent {
	...
	async #handleLifetimeEvent(record: ManagedSessionRecord, event: AgentSessionEvent): Promise<void> {
		if (event.type !== "thinking_level_changed" && event.type !== "model_changed" && event.type !== "mode_changed") {
			return;
		}
		try {
	 ...
	 * refreshes task agents, resets the capability cache, refreshes skills, slash
	 * commands, SSH, MCP runtime tools, and the base system prompt, then
	 * re-advertises commands so the client sees newly installed/disabled plugins.
	 */
