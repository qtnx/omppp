...
import { interruptHint } from "@oh-my-pi/pi-tui/chrome/shared";
import {
...
export class EventController {
		...
		if (this.ctx.streamingComponent && event.message.role === "assistant") {
			...
			for (let contentIndex = 0; contentIndex < this.ctx.streamingMessage.content.length; contentIndex++) {
				...
				if (partialJson) {
					...
					// `setTarget` carries `__partialJson` through every live update for preview renderers.
				} else {
	...
	#loaderUi(): TUI {
		const ui = this.ctx.ui as TUI & { requestComponentRender?: TUI["requestComponentRender"] };
		if (typeof ui.requestComponentRender === "function") {
			return ui;
		}
		// Minimal shim for controller harnesses whose mock UI only exposes the
		// legacy `requestRender()`; production TUI provides `requestComponentRender`.
		return {
			requestComponentRender: () => this.ctx.ui.requestRender(),
		} as unknown as TUI;
	}
	...
	#scheduleIdleCompaction(): void {
		...
		// maintenance flow may reset the session before this timer fires. Some focused
		// controller tests provide only the historical `session` handle.
		const viewSession = this.ctx.viewSession ?? this.ctx.session;
		...
		this.#idleCompactionTimer = setTimeout(() => {
			...
			if (this.ctx.editor.getText().trim()) return;
			if (this.#currentContextTokens() < threshold) return;
			void viewSession.runIdleCompaction();
		}, timeoutMs);
	...
	/**
	 * Append the turn recap as a short wrapped block in the transcript so a user
	 * returning to an open session can see what the agent just did.
	 */
	async #emitTurnSummaryLine(): Promise<void> {
		// `agent_end` can fire between steered/queued turns; only summarize once the
		// agent has truly gone idle awaiting the user.
