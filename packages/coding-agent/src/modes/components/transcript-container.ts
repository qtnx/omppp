...
export class TranscriptContainer extends Container {
	...
	/** Whether none of this block's rows have been offered to native history. */
	...
	/** Whether this block is still in the mutable live region (not yet committed). */
	isBlockInLiveRegion(component: Component): boolean {
		this.#syncEntries();
		const index = this.#entries.findIndex(entry => entry.component === component);
		if (index < 0) return false;
		return this.#entries[index]!.state !== "committed";
	}
	...
	acknowledgeFinalizedBatch(id: number): void {
		...
		if (offered.kind === "append") {
			...
			for (let index = this.#frontier; index < offered.end; index++) {
				...
				entry.emitted = 0;
				// The frame provider accepted this whole block as immutable history.
				// Legacy retractable blocks use the callback to freeze their complete
				// rendered prefix before later UI dismissal.
				(
