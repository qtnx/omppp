...
function releaseCommittedPayloads(child: Component): boolean {
	const release = (child as Component & FinalizableBlock).releaseCommittedPayloads;
		...
		ViewportTailProvider
{
	...
	#releasedPayloadVersions = new WeakMap<Component, number | null>();
	#renderWidth = -1;
	...
	override render(width: number): readonly string[] {
		...
		for (const segment of segments) {
			...
			if (segment.startRow + segment.rowCount > this.#committedRows) break;
			if (segment.rowCount > 0 && segment.finalized) {
				const releasedVersion = segment.version ?? null;
				if (this.#releasedPayloadVersions.get(segment.component) !== releasedVersion) {
					if (releaseCommittedPayloads(segment.component)) {
						this.#releasedPayloadVersions.set(segment.component, releasedVersion);
					}
				}
			}
		}
	...
	#compactCommittedPrefix(): void {
		...
		// Keep the current frame when the whole transcript has moved to native
		// history; payload release above already sheds heavyweight backing data.
		if (dropUntil >= this.children.length) return;
