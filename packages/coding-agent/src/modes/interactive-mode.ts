...
import { loadSkills, type Skill, setActiveSkills } from "../extensibility/skills";
...
export class InteractiveMode implements InteractiveModeContext {
	...
	set loopLimit(value: LoopRuntime | undefined) {
		this.loopRuntime = value;
	}
	...
	#deferLoopAutoSubmit(
		callback: () => void,
		delayMs = this.loopRuntime?.intervalMs ?? DEFAULT_LOOP_INTERVAL_MS,
	): void {
		// Delay before every repeated prompt so loop mode does not immediately
		// send the next command after the prior turn yields.
	...
	#deferLoopReadinessCheck(callback: () => void): void {
		this.#deferLoopAutoSubmit(callback, LOOP_READY_RETRY_MS);
	}
	...
	async #runLoopIteration(action: LoopContextMode, source: LoopPromptSource): Promise<void> {
		...
		if (!hasLoopIterationRemaining(this.loopRuntime)) {
			this.disableLoopMode("Loop iteration limit reached. Loop mode disabled.");
			return;
		}
	...
	async handleLoopCommand(args = ""): Promise<string | undefined> {
		...
		this.showStatus(
			`Loop mode enabled. Repeating ${describeLoopConfig(loopConfig)}.${remainingSuffix}${conditionSuffix}${promptFileSuffix} Esc cancels the current iteration; /loop again to disable.`,
		);
	...
	async #clearTransientModeState(options?: {
		...
		if (this.planModeEnabled || this.planModePaused) {
			...
			try {
				...
				} else if (this.#planModePreviousTools !== undefined) {
					await this.session.setActiveToolsByNamePreservingMCPSelection(this.#planModePreviousTools);
				}
			} finally {
		...
		if (this.goalModeEnabled || this.goalModePaused) {
			...
			await this.session.setActiveToolsByNamePreservingMCPSelection(nextTools);
		...
		if (this.vibeModeEnabled && !options?.preserveVibe) {
			const ownerScope = this.#vibeModeOwnerScope;
			// The previous-tools snapshot belongs to the source session. On a session
			// switch, remove only transient vibe tools so the target tool set survives.
	...
	async #reconcileModeFromSession(options?: { preserveActiveGoal?: boolean }): Promise<void> {
		...
		if (sessionContext.mode === "goal" || sessionContext.mode === "goal_paused") {
			...
			// Active goal mode exposes the goal tool. A paused goal remains resumable
			// through the /goal menu but must not retain goal-mode tools.
			if (restored?.enabled === true) {
				const previousTools = this.session.getEnabledToolNames().filter(name => name !== "goal");
				await this.session.setActiveToolsByNamePreservingMCPSelection([...new Set([...previousTools, "goal"])]);
			}
	...
	async #enterPlanMode(options?: {
		...
		if (this.session.getAllToolNames().includes("resolve")) {
			planAugmentations.push("resolve");
		}
		...
		try {
			await this.session.setActiveToolsByNamePreservingMCPSelection(uniquePlanTools);
		} catch (error) {
	...
	async #enterGoalMode(options: { objective?: string; resume?: boolean; silent?: boolean }): Promise<void> {
		...
		await this.session.setActiveToolsByNamePreservingMCPSelection(goalTools);
		this.goalModePaused = false;
	...
	#isKeepContextDisabled(contextUsage: ContextUsage | undefined): boolean {
		return contextUsage !== undefined && (contextUsage.percent ?? 0) > PLAN_KEEP_CONTEXT_DISABLE_THRESHOLD_PERCENT;
	}
	...
	async #finishProcessExit(_options: { printResumeHint: boolean; beforeQuit?: () => void }): Promise<void> {
	...
	async restart(): Promise<void> {
		...
		const [cmd, ...baseArgs] = resolveCliEntryCmd();
		const command = sandboxOmpxCommand(
			{
				cmd,
				args: [...baseArgs, ...restartArgv(process.argv.slice(2), this.#resumableSessionId())],
				shell: false,
			},
			{ cwd, env: Bun.env },
		);
		...
		if (process.platform !== "win32") {
			try {
				execReplace([command.cmd, ...command.args]); // never returns on success
			} catch (err) {
	...
	handleLearningCommand(text: string): Promise<void> {
		return this.#commandController.handleLearningCommand(text);
	}
