...
export function computeNonMessageTokens(session: NonMessageTokenSource, tokenizer?: Tokenizer): number {
	const resolvedTokenizer =
		tokenizer ?? new Tokenizer((session as NonMessageTokenSource & { model?: Model }).model ?? undefined);
	...
	const tokens =
		resolvedTokenizer.countTokens(Array.from(systemPromptParts, part => part ?? "")) +
		estimateToolSchemaTokens(tools, resolvedTokenizer, session.settings?.revision);
	entry.tokens = tokens;
	return tokens;
}
...
export function computeNonMessageBreakdown(
	...
	const toolsTokens = estimateToolSchemaTokens(tools, resolvedTokenizer, session.settings?.revision);
	const systemPromptParts = session.systemPrompt ?? EMPTY_STRING_PARTS;
	const systemContextTokens = resolvedTokenizer.countTokens(
		Array.from(systemPromptParts.slice(1), part => part ?? ""),
	);
	const systemPromptTokens = Math.max(0, resolvedTokenizer.countTokens(systemPromptParts[0] ?? "") - skillsTokens);
...
export function computeContextBreakdown(
	...
	const tokenizer = session.agent?.tokenizer ?? new Tokenizer(model);
