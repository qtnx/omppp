...
import {
	getMarkdownTheme as getBaseMarkdownTheme,
	getEditorTheme,
	getSelectListTheme,
	getSettingsListTheme,
	getSymbolTheme,
	highlightCode as highlightBaseCode,
...
export { getEditorTheme, getSelectListTheme, getSettingsListTheme, getSymbolTheme, setMarkdownMermaidRendering };
...
const BASIC_HIGHLIGHT_KEYWORDS: Record<string, true> = {
	as: true,
	async: true,
	await: true,
	break: true,
...
let configuredMarkdownTheme: MarkdownTheme | undefined;
let configuredMarkdownThemeBase: MarkdownTheme | undefined;
...
export function getSyntaxHighlightingMode(): SyntaxHighlightingMode {
	return syntaxHighlightingDisabledByEnv() ? "off" : syntaxHighlightingMode;
}
...
function highlightBasicLine(line: string, currentTheme: Theme): string {
	...
	while (index < line.length) {
		...
		if (isIdentifierStart(char)) {
			...
			output += Object.hasOwn(BASIC_HIGHLIGHT_KEYWORDS, word) ? currentTheme.fg("syntaxKeyword", word) : word;
			index = end;
			continue;
		}
		output += char;
...
export function highlightCode(code: string, lang?: string, highlightTheme: Theme = theme): string[] {
	switch (getSyntaxHighlightingMode()) {
		case "off":
			return code.split("\n");
		case "basic":
			return basicHighlightCode(code, highlightTheme);
		default:
			return highlightBaseCode(code, lang, highlightTheme);
	}
}

export function getMarkdownTheme(): MarkdownTheme {
	const base = getBaseMarkdownTheme();
	if (configuredMarkdownThemeBase === base && configuredMarkdownTheme) return configuredMarkdownTheme;
	configuredMarkdownThemeBase = base;
	configuredMarkdownTheme = {
		...base,
		highlightCode: (code: string, lang?: string): string[] => {
			switch (getSyntaxHighlightingMode()) {
				...
				default:
					return base.highlightCode?.(code, lang) ?? code.split("\n");
			}
		},
	};
	return configuredMarkdownTheme;
}
