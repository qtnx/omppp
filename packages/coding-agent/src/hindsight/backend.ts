...
import { type BankScope, resolveBankScope } from "./bank";
...
export const hindsightBackend: MemoryBackend = {
	...
	async beforeAgentStartPrompt(
		...
		const context =
			[primary.mentalModelsSnippet, preparation.context].filter((part): part is string => !!part).join("\n\n") ||
			undefined;
		return {
			context,
...
async function rebuildPrimaryStateOnScopeChange(session: AgentSession): Promise<boolean> {
	...
	const next = await resolveBankScope(config, session.sessionManager.getCwd());
