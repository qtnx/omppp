...
export class ResolveTool implements AgentTool<typeof resolveSchema, ResolveToolDetails> {
	...
	readonly description = prompt.render(resolveReminderPrompt);
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			if (!params.extra || result.details == null) return result;
			return { ...result, details: { ...result.details, extra: params.extra } };
		});
	}
}
