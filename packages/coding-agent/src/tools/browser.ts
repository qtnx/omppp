...
async function invokeBrowser(
	...
	try {
		...
		switch (parsed.action) {
			...
			case "annotate":
				return await annotateBrowser(session, name, parsed, details, timeoutMs, context.signal);
		}
	} catch (error) {
...
async function annotateBrowser(
	session: ToolSession,
	name: string,
	params: BrowserParams,
	details: BrowserPreludeDetails,
	...
	if (!enabled) {
		...
		const text = `Annotation overlay disabled on tab ${JSON.stringify(name)}`;
		return toolResult(details).text(text).done();
	}
	...
	const queueAnnotation = createBrowserAnnotationListener(session, name);
	...
	if (!waitForSubmission) {
		enableBackgroundDelivery();
		const text = `Annotation overlay active on tab ${JSON.stringify(name)}${launchNote}. ${deliveryNote}`;
		return toolResult(details).text(text).done();
	}
