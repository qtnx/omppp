...
function isWorkerTabSession(tab: TabSession): tab is WorkerTabSession {
	return tab.backend === "worker";
}

export function registerTabForTest(tab: TabSession): () => void {
	const previous = tabs.get(tab.name);
	const unsubscribe = isWorkerTabSession(tab)
		? tab.worker.onMessage((msg: WorkerOutbound) => handleTabMessage(tab, msg))
		: () => undefined;
...
export async function setAnnotateMode(name: string, enabled: boolean, timeoutMs: number): Promise<void> {
	...
	if (tab?.state !== "alive") throw new ToolError(`No live tab named ${JSON.stringify(name)}. Open it first.`);
	if (!isWorkerTabSession(tab)) throw new ToolError(`Annotation overlay is only supported for worker-backed tabs.`);
