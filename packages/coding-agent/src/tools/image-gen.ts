...
export function setPreferredImageProvider(provider: ImageProviderPreference): void {
	setImageProviderOrder(provider === "auto" ? [] : [provider]);
}
function assertImageAspectRatioSupported(provider: ImageProvider, aspectRatio: ImageGenParams["aspect_ratio"]): void {
	...
	throw new Error(
		`Aspect ratio ${aspectRatio} is only supported by xAI image generation. Set providers.imageOrder to start with xai or use one of ${COMMON_IMAGE_ASPECT_RATIOS.join(", ")}.`,
	);
}
