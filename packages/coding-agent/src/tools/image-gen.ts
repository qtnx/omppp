...
function resolveHostedImageCarrier(
	...
	const active =
		activeModel?.provider === selectedModel.provider && isOpenAIHostedImageModel(activeModel)
			? activeModel
			: undefined;
	const prioritized = (): Model | undefined => {
	...
	// A Codex subscription generates through the prioritized hosted model even when the session chats
	// an older GPT model; OpenAI API-key requests keep the session's own hosted model first.
	if (selectedModel.provider === "openai-codex") return prioritized() ?? active;
