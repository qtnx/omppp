...
/** Validate a mounted device payload exactly as dispatch does. */
export function validateXdevDeviceArgs(device: Tool, content: string, toolCallId: string): Record<string, unknown> {
	return parseDeviceArgs(device as AiTool, content, toolCallId, () => renderDocs(device));
}
...
export class XdevRegistry {
	...
	/** Build an isolated registry view for prompt rendering before live mounts are published. */
	forkWithDynamic(tools: Iterable<Tool>): XdevRegistry {
		const fork = new XdevRegistry(this.#builtins.values());
		fork.reconcile(tools);
		return fork;
	}
	...
	async dispatch(
		...
		const validated = validateXdevDeviceArgs(inst, content, toolCallId);
