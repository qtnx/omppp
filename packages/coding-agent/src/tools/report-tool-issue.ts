 ...
 * `~/.omp/autoqa.db` (XDG: `$XDG_DATA_HOME/omp/autoqa.db`). Idempotently
 * runs schema creation, legacy-column migrations, and index setup so every
 * consumer — tool execution, manual `ompx grievances push`, and future debug
 * scripts — sees the same prepared schema. Returns `null` only on a hard open
 ...
 * handle too — having a second `openDb` in the CLI led to migrations not
 * running on the manual-push path.
 */
...
/** Legacy hidden tool retained for sessions and SDK callers that do not expose xd:// devices. */
		...
		async execute(_toolCallId, rawParams) {
			try {
				...
				if (allowedToolNames.size === 0 || allowedToolNames.has(canonicalTool)) {
					if (isAutoQaEnabled(session.settings)) {
						await recordToolIssue(session, canonicalTool, params.report);
					}
				}
			} catch (error) {
				logger.error("Failed to record tool issue", { error });
