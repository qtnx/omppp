...
export class IrcTool implements AgentTool<typeof ircSchema, IrcDetails> {
	...
	#executeList(registry: AgentRegistry, senderId: string): AgentToolResult<IrcDetails> {
		...
		return {
			...
			details: { op: "list", from: senderId, peers, channels: peers.map(peer => peer.id) },
		};
	}
	...
	async #executeSend(
		...
		const shouldAwaitReply = params.await === true || params.awaitReply === true;
		if (isBroadcast && shouldAwaitReply) {
		...
		const awaitAbort = shouldAwaitReply ? new AbortController() : undefined;
		const awaitCancelled = new Error("IRC await cancelled");
		let removeAwaitAbortListener: (() => void) | undefined;
		const waiting = shouldAwaitReply
			? bus
			...
			: undefined;
		if (shouldAwaitReply && signal && awaitAbort) {
			if (signal.aborted) {
		...
		try {
			...
			const notFound = receipts.filter(receipt => receipt.outcome === "failed").map(receipt => receipt.to);
			if (targets.length === 0) {
			...
			if (shouldAwaitReply && waiting && timeoutMs !== undefined) {
				lines.push("");
			...
			return {
				...
				details: {
					...
					to,
					receipts,
					delivered: delivered.map(receipt => receipt.to),
					notFound,
