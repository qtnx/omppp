...
export class ComputerWorkerCore {
	...
	async #ensureSession(snapshot: ComputerSessionSnapshot): Promise<NativeDesktopSession> {
		...
		try {
			// Keep readiness and ping paths addon-free; load native desktop only when
			// first run actually needs a desktop session.
			const createSession =
		...
		return {
			...
			clipboard: {
				read: async (): Promise<string> => {
					const { signal } = getContext();
					throwIfAborted(signal);
					// Clipboard access stays outside readiness-only worker imports.
