...
export interface SlashCommandRuntime {
	...
	/**
	 * Reload plugin state (caches, skill snapshot, system prompt, slash command
	 * registry, project registries) and re-emit available commands. Used by
	 * `/reload-plugins`, `/move`, and `/marketplace`/`/plugins` mutations so the
	 * session sees a consistent view after plugin or project-scope changes.
	 */
