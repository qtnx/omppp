...
import { sanitizeText, setProjectDir } from "@oh-my-pi/pi-utils";
...
function sanitizeInlineText(value: string, maxWidth: number = TRUNCATE_LENGTHS.LINE): string {
	const singleLine = replaceTabs(sanitizeText(value)).replaceAll(/\r?\n/g, " ");
...
export const BUILTIN_LIFECYCLE_SLASH_COMMANDS: ReadonlyArray<SlashCommandSpec> = [
	...
	},
	{
		...
		getTuiAutocompleteDescription: runtime => {
			...
			if (!usage || usage.percent === null) return "Compact: context unavailable";
