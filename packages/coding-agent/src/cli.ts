...
const UPSTREAM_STATS_SYNC_WORKER_ARG = "__omp_worker_stats_sync";
const UPSTREAM_TAB_WORKER_ARG = "__omp_worker_tab";
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG || arg === UPSTREAM_STATS_SYNC_WORKER_ARG) {
	...
	if (arg === TAB_WORKER_ARG || arg === UPSTREAM_TAB_WORKER_ARG) {
		if (parentPort) installWorkerInbox(parentPort);
		await import("./tools/browser/tab-worker-entry");
		return true;
	}
	if (arg === JS_EVAL_WORKER_ARG || arg === UPSTREAM_JS_EVAL_WORKER_ARG) {
		if (parentPort) installWorkerInbox(parentPort);
...
export async function runCli(argv: string[]): Promise<void> {
	...
	// `__omp_worker_`/legacy dispatch, and importers (`runCli` in profile-CLI tests,
