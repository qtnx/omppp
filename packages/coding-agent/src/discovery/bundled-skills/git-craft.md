...
- `publish` for a frontend/UI repo (web app, game client, anything a browser renders — detect via the app manifest: `next`/`vite`/`react`/`vue`/`svelte`/`angular`/`phaser`/`three`/`pixi` deps, `public/`+`index.html`, or a dev-server script) additionally REQUIRES, before the PR/MR is opened: (1) one `browser_qa` subagent run against the running app driving the changed flow (`browser_jev` first for DOM flows — one goal per call; `browser_use` for games/canvas/3D/visual state; the `browser` prelude for selectors/console/network), telling it the screenshot directory (`.omp/qa/<branch>/`) and which states matter; the subagent decides which captures to `save:` (2–5 decisive before/after images, never every step — screenshots are inline-only unless saved) and returns the paths in each case's `evidence`, which you `read` yourself when you need to verify; (2) those screenshots attached to the PR/MR body under `SCREENSHOTS` so a human can review without checking out — `gh pr create` accepts images pasted into the body via a pushed `.omp/qa/**` path or `gh api` upload; `glab mr create` via `glab api projects/:id/uploads -F file=@<png>` then the returned markdown. A red/`blocked` QA verdict blocks publishing: fix-forward, rerun only the failed cases, then publish. Skip only when the change cannot render (pure types/config/docs) and say so in VERIFIED.
...
QA: <frontend/UI only — browser_qa verdict per case (pass/fail/blocked) + the model/tools it used>
SCREENSHOTS: <frontend/UI only — 2–5 decisive before/after images, each captioned with the flow + state it proves>
...
- ALWAYS assume other agents are editing the same working tree. `reset` (any mode), `checkout -- .`, `restore .`, bare `stash`, `clean`, branch deletion, history rewrites = irreversible tier: run `git status --porcelain` FIRST (a sanctioned use of status); any entry you did not write is a peer's uncommitted work and the command is forbidden. Per-path `checkout -- <f>` / `restore <f>` / `stash push -m <why> -- <f>` is allowed only when the whole uncommitted diff of `<f>` is yours: read `git diff -- <f>` first, and a stash is popped in the same task and never dropped. A merge, rebase, or cherry-pick that needs a clean tree runs in its own `git worktree add ../wt-<name> <base>`, never by cleaning the shared tree.
