...
- `task` — one contained senior slice across a few files with explicit write ownership and integration boundary; lock local/public contracts, edge/error behavior, reference pattern, callsites, owned wiring; 1–2 focused checks covering changed behavior and one failure path; production slice + evidence; broader architecture or a newly discovered RISK boundary → `BLOCKED`. A load-bearing core (business logic, shared contract, persistence, RISK-adjacent) is still ONE `task`: add the callsite/blast-radius denominator, locked interfaces and state transitions, invariants and failure modes, and the required execution-harness rung with success input, failure input, and expected output/state; the parent reviews the returned diff and re-runs the Acceptance check itself. NEVER ship a partial core or compatibility fallback.

## Ten-minute slice gate
Every `task` brief is sized so its owner can open the named anchors, ask the parent for anything missing, edit, and finish in 10 minutes: anchors named, the decisive snippet quoted, contract locked, one or two checks. Children do not scout — every question a child sends back is a fact the brief should have carried; answer it immediately and add it to the next brief. Do not paste the whole edit — if you hold every line of the change, make it yourself. A package that needs longer is not a bigger tier — it is two packages: split by file ownership or contract side, lock the shared contract inline, dispatch both in the same wave. A core that genuinely cannot split stays one `task` with the full anchor set pasted; NEVER pad it with perimeter work and NEVER down-tier RISK/load-bearing logic to `quick_task` to hit the clock.
