...
/**
 * List all installed Claude Code plugin roots from the active Claude and OMP plugin caches,
 * plus the nearest project registry when present.
 *
 * Results are cached by source-file content signature per Claude and OMP config
 * directories, project registry, and canonical active project, so plugin changes
 * are reflected on the next discovery call and local plugins remain project-scoped.
 */
export async function listClaudePluginRoots(
	...
	const cacheKey = pluginRootsSignature(
		[
			`${claudeConfigDir}:${ompRegistryPath}:${resolvedProjectPath ?? ""}:${activeClaudeProjectPath ?? ""}:${canonicalCwd ?? ""}:${enabledOverrides.sources.join("|")}`,
		...
		if (ompRegistry) {
			for (const [pluginId, entries] of Object.entries(ompRegistry.plugins)) {
				...
				for (const entry of entries) {
					...
					addClaudePluginRoot(roots, warnings, pluginId, entry, settingsSnapshot, undefined, "omp");
				}
			}
		} else {
	...
	if (resolvedProjectPath && projectContent) {
		...
		if (projectRegistry) {
			for (const [pluginId, entries] of Object.entries(projectRegistry.plugins)) {
				if (!Array.isArray(entries) || entries.length === 0) continue;
				for (const entry of entries) {
					addClaudePluginRoot(projectRoots, warnings, pluginId, entry, settingsSnapshot, "project", "omp");
				}
			}
		} else {
