...
- Read the named ranges in the owned files and named callers, batching independent reads; a snippet pasted in the assignment is already read. A fact the assignment does not give you is asked from the parent over IPC, never reconstructed by searching the repo.
