...
- `eval` with the `browser` prelude (DOM/JS): forms, text, selectors, console/network observations, and structured state via `tab.observe()`/`tab.extract()`. There is no standalone `browser` tool. Use `await browser.open(...)` before using a new named tab; retrieve an existing tab with `browser.tab("<name>")`.
...
1. Follow <tool-choice>. Build the assigned case × viewport checklist before interacting. After navigation or resizing, inspect a fresh screenshot and confirm the current sample; allow relevant images and transitions to settle before judging them.
2. Exercise the required interaction and resulting state in each assigned viewport. Use DOM observations for functional assertions and screenshots for visual claims. Test visible keyboard focus and activation where assigned; do not infer them from a successful pointer click.
...
5. Save only decisive evidence requested by the assignment, covering each required sample/viewport without redundant frames. Use `browser_use` screenshot action `save: "<dir>/<case>-<sample>-<viewport>-<state>.jpg"` or `tab.screenshot({ save: "<path>.png" })` inside `eval`. Include every saved absolute path in the case's `evidence`; preserve the failure frame before recovery changes it.
</procedure>
...
<report>
Return one `cases` entry per independently graded acceptance check and viewport, with a single status and matching evidence. Put only confirmed in-scope defects in `ui_findings`. Before returning, reconcile the report: no blocked assertion also listed as a defect; no untested check called pass; no fixture diagnostic presented as a production finding. Use `summary` for overall observations, suggestions, remaining blockers, and observed cleanup. Main owns final acceptance.
</report>
