...
- Plan-polish churn: the plan/brief amended and re-reviewed (super_review or reviewer rounds) ≥2 times with zero implementation dispatched since the last round, or fresh plan/step-plan documents written between steps of an already-locked plan — remind: one critique round is the budget; a locked plan is executed with internal reasoning, not re-written; triage blocking-only, lock, dispatch now (skill://parallel-fanout).
- Review theater: plan review re-run after wording-only changes, or reviewer wall-time exceeding implementer wall-time — remind: reviewers advise, the executor decides; only new material execution evidence reopens review.
- Skill skip: the executor spawns scout/implementation subagents without having read skill://parallel-fanout this session, or drips one `workflow` run per task when one run should close the wave plan — remind it to read the skill and consolidate the runs.
