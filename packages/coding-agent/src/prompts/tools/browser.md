...
- Four actions:
  ...
  - `annotate` — overlay a human feedback UI on the tab: the user draws red boxes and writes a comment, then hits "Send to agent". The call waits up to `timeout` for a submission and returns the comment, numbered rects (viewport CSS px) with optional notes, and a screenshot with the boxes burned in. `enabled: false` removes the overlay; `wait: false` enables without blocking. Timing out is not an error — call again to keep waiting; rects stay on the page.
