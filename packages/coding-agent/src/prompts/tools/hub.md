...
- **`start`** launches `application` + `args` directly. `cwd` defaults to the session directory; `pty` defaults true.
  - Every regex field (`ready.log`, `logs.grep`, `wait.pattern`) is a JavaScript `RegExp` compiled with the `u` flag; PCRE inline modifiers such as `(?i)` are REJECTED — use `[Rr]eady`. `ready.port` is a TCP port. Both supplied? BOTH MUST pass. `ready.timeout` is seconds. Readiness MUST be observed; process creation alone is not readiness.
...
- **`logs`** defaults to the last 100 lines; `head: true` reads the beginning; `follow: true` waits for output after `cursor` — reuse the returned cursor next call.
