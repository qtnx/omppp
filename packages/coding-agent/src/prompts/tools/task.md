...
- Brief so the owner never re-discovers what YOU already know: the exact `file:line`/`file:symbol` anchors, the contract, the pattern to mirror, and the one decisive snippet where the change hinges. Do NOT paste the whole edit — if you already hold every line of the change, make the edit yourself; delegation buys concurrency, not typing. The owner surveys its own slice FAST from those anchors (targeted reads inside the owned files), never the repo. First action is a named-anchor read or the edit, never a search. Cross-task contracts go in {{#if batchEnabled}}batch `context`{{else}}the task{{/if}} before dispatch. Every task skips formatters, linters, and project-wide suites — you run those once at the end.
- Ten-minute slices: size every `task` so its owner finishes inside 10 min — a couple of minutes of targeted reads inside its owned files, then edit, then one Acceptance run. Too big → two briefs (split by file ownership or contract side; lock the shared contract inline; same wave), never a longer runtime. A brief whose anchors you cannot name is not ready to dispatch — do that one lookup yourself and name them.
