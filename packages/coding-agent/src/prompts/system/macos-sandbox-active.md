...
- SSH and Git-over-SSH should use `SSH_AUTH_SOCK`; private key files under `~/.ssh` remain blocked. The running SSH agent (1Password, Secretive, launchd, or `ssh-agent`) is auto-discovered even when `SSH_AUTH_SOCK` isn't inherited; override the socket with `sandbox.sshAuthSock` / `PI_OMPX_SSH_AUTH_SOCK`, or set `sandbox.sshAuthSock: off` to disable.
