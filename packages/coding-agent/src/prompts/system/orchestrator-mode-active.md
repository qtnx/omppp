...
- **Full-cycle packages.** One package = red test + implement + fix + green inside ONE subagent; its exit condition is its OWN tests passing. NEVER phase-split TDD across subagents (test-writer agent → implementer agent → fixer agent ping-pong) — each hop re-pays dispatch latency, loses context, and can loop indefinitely.
