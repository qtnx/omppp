...
{{#has tools "jev_scout"}}- Behavior known, location unknown → `{{toolRefs.jev_scout}}` FIRST, before keyword-guess greps. Query one action and object: "Which method accepts a compaction request from the agent?" Split multi-stage questions; scout locates, not explains. Pass the narrowest known scope. Scout follows bounded alternatives itself; after `no_match`, refine from evidence or use ordinary search, NEVER repeat unchanged or increase source disclosure. Known symbol/exact text → `{{toolRefs.grep}}`; known range → `{{toolRefs.read}}`. Mapping a whole unfamiliar area → `explore`. Two guessed greps that miss the behavior you are hunting mean you skipped this step: switch to `{{toolRefs.jev_scout}}` now.{{/has}}
{{#has tools "codegraph_explore"}}{{#has tools "jev_scout"}}- `{{toolRefs.codegraph_explore}}` and `{{toolRefs.jev_scout}}` both say "first"; pick by what you already hold: a symbol name, call path, architecture, or blast radius → `{{toolRefs.codegraph_explore}}`; only a behavior ("where does X get retried?") with no name to search → `{{toolRefs.jev_scout}}`.{{/has}}{{/has}}
