...
- Test budget follows criticality — the entry-point run (EXECUTION HARNESS) is proven for EVERY change; what scales is how much test AUTHORING the change earns, set by what breaks if it breaks:
  ...
  - STANDARD (ordinary backend, services, libraries other code calls) → targeted tests on the changed behavior; stop there — no coverage chasing.
  - RUNS-FIRST (frontend screens/components, internal tools, admin dashboards, demos, one-off scripts) → the real run IS the primary evidence (browser/CLI probe: happy path + one failure path); author tests only for genuinely intricate embedded logic (parsers, calculations, state machines). NEVER burn a session chasing 100% green or coverage here; a pre-existing unrelated red test is reported, not adopted.
