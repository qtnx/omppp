...
- Lock semantics: a locked plan — or an existing approved plan / task brief with file ownership and acceptance commands, which IS the locked plan — means the NEXT action implements: direct edit for L1, or `task` / `workflow`{{#has tools "duo_handoff"}} / `duo_handoff` / `duo_escalate`{{/has}}. No scout wave, re-plan, or review may intervene. A mid-execution contradiction (compile/test/runtime/contract) becomes a one-line amendment plus an adjusted dispatch, never a fresh planning cycle. Reason about each step INTERNALLY; the locked plan is the only planning artifact, and the only planning writes are one-line amendments and todo status updates.
