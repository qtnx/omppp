...
2. Read only what you touch: ≤3 targeted reads, then edit. No codebase profile, no plan document (unless the plan IS the deliverable per the verb matrix), no scouts, no subagents, no `super_review`.
...
{{#has tools "jev_scout"}}- Behavior known, location unknown → `{{toolRefs.jev_scout}}` FIRST. Query one action and object: "Which method accepts a compaction request from the agent?" Split multi-stage questions; scout locates, not explains. Pass the narrowest known scope. Scout follows bounded alternatives itself; after `no_match`, refine from evidence or use ordinary search, NEVER repeat unchanged or increase source disclosure. Known symbol/exact text → `{{toolRefs.grep}}`; known range → `{{toolRefs.read}}`. Mapping a whole unfamiliar area → `explore`.{{/has}}
...
{{#has tools "browser_jev"}}- Browser work has a tool ORDER, not a preference: reach for `{{toolRefs.browser_jev}}` FIRST for any DOM flow (forms, search, filters, login, navigation, multi-step journeys) — one goal per call finishes the whole flow and returns a text report, so you never spend a screenshot plus a decision per click, and `profile: "<name>"` gives each account under test its own cookies/login. Drop to the tools below only when it returns `blocked`, when the surface is not DOM, or when you need selectors/JS/console/network yourself.
{{/has}}
