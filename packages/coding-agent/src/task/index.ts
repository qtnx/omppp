...
const PLAN_MODE_AGENT_TOOL_ALLOWLIST: ReadonlySet<string> = new Set(["ast_grep", "irc", "report_finding"]);
...
function validateSpawnParams(params: TaskParams, batchEnabled: boolean): string | undefined {
	...
	if (batchEnabled && tasks !== undefined) {
		...
		if (hasTask || hasLegacyAssignment) {
			return "Top-level `task` is not part of the batch shape. Put the work in `tasks[]` items.";
		}
		for (let i = 0; i < tasks.length; i++) {
			const item = tasks[i];
			const itemTask = item?.task ?? item?.assignment;
			if (!item || typeof itemTask !== "string" || itemTask.trim() === "") {
				const label = item?.name ?? item?.id;
				return `Task ${i + 1}${label ? ` (\`${label}\`)` : ""} is missing \`task\`. Every task needs complete, self-contained instructions.`;
			}
		}
		...
		// New agent-centric calls require shared context. Legacy fork callers
		// retain their historically optional context while they migrate.
		const usesAgentCentricItems = tasks.some(item => typeof item.task === "string");
		if (usesAgentCentricItems && (typeof params.context !== "string" || params.context.trim() === "")) {
...
function resolveSpawnItems(params: TaskParams): TaskItem[] {
	...
	const item: TaskItem = {
		...
		agent: params.agent,
		task: params.task ?? params.assignment,
 ...
 * item's identity substituted in. Each spawn's `agent` resolves from the item,
 * then the call-level default, then the session spawn policy. `tasks` never
 * leaks into a spawn; the shared `context` rides along unchanged. Keys are only
 * materialized when present — `#runSpawn`
...
function mergeSyncPayloads(
	...
	for (let position = 0; position < spawns.length; position++) {
		...
		if (!payload) {
			contentParts.push(`Task ${item.name?.trim() || item.id?.trim() || `#${index + 1}`}: cancelled before start.`);
			continue;
		}
...
/** Generic worker tiers where multiple spawns should prompt a specialist check. */
...
export function buildSpecializationAdvisory(
	agentNamesOrAgent: string[] | string | undefined,
	itemsOrDepth: TaskItem[] | boolean,
	maybeDepth?: boolean,
): string | undefined {
	const legacy = !Array.isArray(agentNamesOrAgent);
	const depthCapacity = legacy ? maybeDepth === true : itemsOrDepth === true;
	if (!depthCapacity) return undefined;

	if (legacy) {
		const agentName = agentNamesOrAgent;
		const items = itemsOrDepth as TaskItem[];
		...
		if (rolelessCount === 0 || (!generic && !(items.length >= 2 && rolelessCount === items.length))) return undefined;
		return `Tip: spawned ${rolelessCount} \`${agentName ?? "task"}\` worker${rolelessCount === 1 ? "" : "s"} without a \`role\`. Tailored specialists outperform generic workers.`;
	}
	...
	return (
		...
		`Check the agent list for a closer specialist type — e.g. read-only research belongs on \`agent: "explore"\`.`
	);
}
...
function discoverAgentsForCreate(cwd: string): Promise<DiscoveryResult> {
	const fn = taskDiscovery.discoverAgents;
...
function buildReviewGateConfigError(
	...
	const assignment = (params.task ?? params.assignment ?? "").trim();
	const result: SingleResult = {
		index: 0,
		id: params.name?.trim() || params.id?.trim() || generateTaskName(),
		agent: agentName,
		agentSource,
	...
	return {
		content: [{ type: "text", text }],
		details: { projectAgentsDir, results: [result], totalDurationMs: 0 },
	};
}
...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
	...
	async execute(
		...
		const resolvedAgents = spawnItems.map(item => item.agent?.trim() || params.agent?.trim() || defaultAgent);
		...
		const withAdvisory = (result: AgentToolResult<TaskToolDetails>): AgentToolResult<TaskToolDetails> => {
			if (!advisory || result.details?.results?.length === 0) return result;
			let appended = false;
	...
	async #runSpawn(
		...
		const startTime = Date.now();
		const { agents, projectAgentsDir } = await taskDiscovery.discoverAgents(this.session.cwd);
		...
		if (reviewGateEnabled) {
			...
			if (!reviewerAgent || !fixerAgent) {
				...
				return buildReviewGateConfigError(params, projectAgentsDir, agentName, agent.source, text);
			}
			const failOnRaw =
		...
		try {
			...
			const sessionWithCompactContext = this.session as ToolSession & {
				getCompactContext?: () => string | undefined;
			};
			const compactContext = shouldWriteConversationContext
				? sessionWithCompactContext.getCompactContext?.()
				: undefined;
			const writeContextSnapshot = async (): Promise<ContextFileEntry | undefined> => {
			...
			const baseContextFiles = this.session.contextFiles?.filter(
				file => path.basename(file.path).toLowerCase() !== "agents.md",
			);
			const withContextSnapshot = (snapshot: ContextFileEntry | undefined): ContextFileEntry[] | undefined =>
				snapshot ? [snapshot, ...(baseContextFiles ?? [])] : baseContextFiles;
			...
			const runReviewGateForTask = async (
				...
				const buildParentProgress = (): AgentProgress => ({
					...
					recentTools: latestProgress.recentTools,
					recentOutput: latestProgress.recentOutput,
				...
				const runGateSubagent = async (
					...
					const gateId = `${agentId}-${role}-${request.iteration}`;
					...
					return runSubprocess({
						...
						contextFiles: withContextSnapshot(await writeContextSnapshot()),
						...
						preloadedExtensionPaths: worktree ? undefined : this.session.extensionPaths,
						preloadedCustomToolPaths: worktree ? undefined : this.session.customToolPaths,
						localProtocolOptions,
			...
			const runTask = async (): Promise<SingleResult> => {
				if (!isIsolated) {
					...
					try {
						...
						const result = await runSubprocess(sharedRunOptions);
						if (reviewGateConfig && taskBaseline) {
							...
							const gated = await runReviewGateForTask(result, () =>
				...
				try {
					if (!repoRoot || !isolationContext) {
						throw new Error("Isolated task execution not initialized.");
					}
					const taskBaseline = structuredClone(isolationContext.baseline);
			...
			if (isReviewGateBlockedResult(result)) {
				const locationNote = isIsolated
					? "Its patch was not applied."
					: "Its changes remain in the working tree for follow-up fixes.";
				mergeSummary += `\n\n<system-notification>Review-blocked task was withheld by the review gate. ${locationNote}</system-notification>`;
			}
	...
	#buildResultPayload(
		...
		const baseOutput = formatResultOutputFallback(result);
		const output = reviewBlocked
			? result.reviewGate?.failureReason?.trim()
				? `${baseOutput}\n\nReview gate blocked: ${result.reviewGate.failureReason.trim()}`
				: `${baseOutput}\n\nReview gate blocked.`
			: baseOutput;
		const outputCharCount = reviewBlocked ? output.length : (result.outputMeta?.charCount ?? output.length);
		...
		const summary = prompt.render(taskSummaryTemplate, {
			...
			truncated,
			meta:
				!reviewBlocked && result.outputMeta
					? {
