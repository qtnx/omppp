...
export class SecretObfuscator {
	...
	#collectRegexSecretValues(text: string, skipGeneratedPlaceholders = false): Set<string> {
		const values = new Set<string>();
		const knownPlaceholderRanges = skipGeneratedPlaceholders ? this.#knownPlaceholderRanges(text) : [];
		for (const entry of this.#regexEntries) {
			entry.regex.lastIndex = 0;
			for (;;) {
				...
				if (knownPlaceholderRanges.some(range => match.index >= range.start && matchEnd <= range.end)) {
					continue;
				}
				values.add(match[0]);
			}
	...
	collectRegexSecretValuesForObfuscation(text: string): Set<string> {
		const values = this.#collectRegexSecretValues(text, true);
		let result = text;
		let origin = "I".repeat(text.length);
		for (const [secret, replacement] of [...this.#replaceMappings].sort((a, b) => b[0].length - a[0].length)) {
			({ text: result, origin } = this.#replaceOutsidePlaceholdersTracked(result, origin, secret, replacement, "I"));
		}
		for (const secretValue of this.#collectRegexSecretValues(result, true)) {
			values.add(secretValue);
		}
		for (const secretValue of this.#collectRegexSecretValuesAfterRegexReplacements(result, origin, true)) {
			values.add(secretValue);
		}
		return values;
	}

	#collectRegexSecretValuesAfterRegexReplacements(
		text: string,
		origin: string,
		skipGeneratedPlaceholders = false,
	): Set<string> {
		const values = new Set<string>();
		let simulated = text;
		...
		for (const entry of this.#regexEntries) {
			...
			for (const match of [...matches].sort((a, b) => b.start - a.start)) {
				...
				for (const secretValue of this.#collectRegexSecretValues(replacement, skipGeneratedPlaceholders)) {
					values.add(secretValue);
				}
			...
			for (const secretValue of this.#collectRegexSecretValues(simulated, skipGeneratedPlaceholders)) {
				values.add(secretValue);
			}
		}
		return values;
