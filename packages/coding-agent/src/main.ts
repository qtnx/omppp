...
export async function buildSessionOptions(
	...
	if (prewalkEnabled) {
		...
		try {
			...
			if (!resolved.model) throw new Error(`Model "${target}" not found`);
			options.prewalk = { target: resolved.model, thinkingLevel: resolved.thinkingLevel };
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			if (!message.startsWith("No API key for ") && !message.startsWith('Model "')) throw error;
			process.stderr.write(`${chalk.yellow(`Warning: prewalk disabled — ${message}`)}\n`);
		}
	}
...
export async function runRootCommand(
	...
	if (mode === "acp") {
		...
		try {
			extensionsResult = parsedArgs.trustedExtensions?.length
