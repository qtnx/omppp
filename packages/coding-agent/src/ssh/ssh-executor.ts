...
export async function executeSSH(
	...
	if (options?.compatEnabled) {
		...
		if (info.compatShell) resolvedCommand = wrapInPosixShell(info.compatShell, command);
	...
	try {
		...
		if (err instanceof ptree.Exception) {
			if (err instanceof ptree.TimeoutError) {
				return { exitCode: undefined, cancelled: true, ...(await sink.dump(`SSH: ${err.message}`)) };
			}
			if (err.aborted) {
				return { exitCode: undefined, cancelled: true, ...(await sink.dump(`Command aborted: ${err.message}`)) };
			}
			return { exitCode: err.exitCode, cancelled: false, ...(await sink.dump(`Unexpected error: ${err.message}`)) };
		}
		throw err;
	} finally {
		abortWaiter.cleanup();
