...
describe("AgentSession duo wiring helpers", () => {
	...
	test("orchestrator overlay retains the complete canonical base before its mode delta", () => {
		...
		expect(orchestratorBlock.startsWith(basePrompt[0] ?? "")).toBe(true);
		expect(orchestratorBlock).toContain("Safe Orchestrator Mode");
		expect(orchestratorBlock.indexOf("Safe Orchestrator Mode")).toBeGreaterThan((basePrompt[0] ?? "").length);
		expect(orchestratorBlock.split(baseSkillsSection)).toHaveLength(2);
		expect(orchestratorBlock).toContain(baseInternalUrlsSection);
	});

	test("orchestrator overlay retains bases without Skills and renders an empty base once", () => {
		const baseWithoutSkills = [
			"Base identity block.\n\n# Internal URLs\n- `rule://<name>`: rule details",
			"Tool block",
		];
		const [normalOverlay, ...normalSecondaryBlocks] = buildSystemPromptWithOrchestratorOverlay(baseWithoutSkills);
		const [emptyOverlay, ...emptySecondaryBlocks] = buildSystemPromptWithOrchestratorOverlay(["", "Tool block"]);

		expect(normalSecondaryBlocks).toEqual(["Tool block"]);
		expect(normalOverlay.startsWith(baseWithoutSkills[0] ?? "")).toBe(true);
		expect(normalOverlay).toContain("# Internal URLs");
		expect(normalOverlay).toContain("Safe Orchestrator Mode");
		expect(emptySecondaryBlocks).toEqual(["Tool block"]);
		expect(emptyOverlay).not.toContain("undefined");
		expect(emptyOverlay.split("Safe Orchestrator Mode")).toHaveLength(2);
	});
