...
import { reportSoftCrash } from "@oh-my-pi/pi-utils/crash-report";
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	const customCommandsPromise =
		options.disableExtensionDiscovery || minimalExtensionRuntime || options.restrictToolNames === true
	...
	try {
		...
		//      (cwd, eventBus, runtime). Compatibility fallback for callers that
		...
		try {
			if (minimalExtensionRuntime) {
				...
				extensionsResult = await withContextGcDbPath(contextGcDbPath, async () =>
					logger.time("bindPreparedExtensions", bindPreparedExtensions, preparedExtensions, cwd, eventBus),
				);
		...
		// Discovery began with the other cwd/agentDir-only scans, but restricted and
		// minimal sessions receive an already-empty result and never evaluate modules.
		...
		const rebuildSystemPromptUnscoped = async (
			...
			let promptContextFilesPromise = usingInitialCwd
			...
			if (hasSession && options.contextFiles === undefined) {
				...
				promptContextFilesPromise = Promise.resolve(contextFiles);
			}
			...
			const promptTools = projectSystemPromptToolMetadata(tools, {
				...(nativeTools && !inlineToolDescriptors
					? { mode: "compact" as const, toolNames: [...toolNames, ...mountedPromptToolNames] }
					: { mode: "full" as const }),
				overrides: {
			...
			if (typeof options.systemPrompt !== "function") {
				return { ...defaultPrompt, systemPrompt: defaultSystemPrompt };
			}
			const customPrompt = await options.systemPrompt(defaultSystemPrompt);
			return {
		...
		if (explicitlyRequestedToolNames) {
			const checkpointRequested = explicitlyRequestedToolNames.includes("checkpoint");
			const rewindRequested = explicitlyRequestedToolNames.includes("rewind");
			if (checkpointRequested && !rewindRequested) {
				explicitlyRequestedToolNames.push("rewind");
			} else if (rewindRequested && !checkpointRequested) {
		...
		const xdevWriteAvailable =
			...
			(explicitlyRequestedToolNameSet === undefined ||
				...
				toolSession.deviceOnlyWrite === true);
		const xdevTransportAvailable = xdevReadAvailable && xdevWriteAvailable;
		...
		// Use the effective registry winner's inactive state, while retaining the fork's
		...
		const alwaysInclude: string[] = restrictToolNames
			...
			: [...sdkCustomTools.map(tool => tool.name), ...registeredTools.map(tool => tool.definition.name)].filter(
					name =>
						...
						(!mcpDiscoveryEnabled ||
							!isMCPToolName(name) ||
							retainAmbientMcpToolsTopLevel ||
							toolSession.deviceOnlyWrite === true) &&
						shouldAutoIncludeCustomTool(name.toLowerCase()),
				);
		...
		if (toolSession.xdev) {
			...
			// `createTools` has already mounted the default discoverable built-ins.
			// Discovery-all can remove those names from `initialToolNames`, but that
			// only changes their top-level exposure; it must not erase their device
			// catalog before the first provider request.
			const mountedNames = new Set(
				xdevReadAvailable && xdevWriteAvailable
					? [...toolSession.xdev.mountedNames].filter(name => {
							const tool = toolRegistry.get(name);
							return (
								tool !== undefined &&
								explicitlyRequestedToolNameSet?.has(name) !== true &&
		...
		const initialPromptToolRegistry = new Map(
			Array.from(toolRegistry).filter(
				([name, tool]) => tool.defaultInactive !== true || initialToolNames.includes(name),
			),
		);
