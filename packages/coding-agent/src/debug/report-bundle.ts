...
import { APP_STORAGE_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@oh-my-pi/pi-utils";
...
async function collectSameDayLogs(linesPerFile: number): Promise<string> {
	...
	const today = LOG_FILE_PATTERN.exec(path.basename(getLogPath()))?.[1];
	if (!today) return "";
...
const LOG_FILE_PATTERN = new RegExp(`^${APP_STORAGE_NAME}\\.(\\d{4}-\\d{2}-\\d{2})\\.\\d+\\.log(?:\\.\\d+)?$`);
