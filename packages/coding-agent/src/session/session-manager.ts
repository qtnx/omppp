...
import { type BlobPutOptions, type BlobPutResult, BlobStore, lazyImageDataSync, parseBlobRef } from "./blob-store";
...
export class SessionManager {
	...
	/**
	 * Replace older plain compaction summaries on the branch that just received
	 * a newer summary. Compactions carrying preserved metadata remain complete:
	 * rewinding to one of those entries still needs both its summary and archive.
	 */
		...
		for (const entry of this.#index.pathTo(leafId)) {
			...
			if (entry.preserveData !== undefined) continue;
			if (
		...
		try {
			...
			if (writer.appendSync && !this.#storage.defersSyncPublish) {
				...
				void confirmed
					...
					.catch(err => {
						if (!this.#recoverHotAppendFailure(err)) {
							this.#fileIsCurrent = false;
							this.#rewriteRequired = true;
							this.#noteDiskFailure(err);
						}
	...
	snapshotForReplication(copy: <T>(value: T) => T = structuredClone): {
		...
		return { header: copy(this.#header), entries };
	}
	...
	setWorkspaceRoots(roots: readonly WorkspaceRoot[]): void {
		...
		const unchanged =
			...
			previous.every((root, index) => {
				const nextRoot = serialized.at(index);
				return (
					nextRoot !== undefined &&
		...
		if (this.#persist && this.#sessionFile && this.#storage.existsSync(this.#sessionFile)) {
			void this.#rewriteAtomically().catch(() => {});
		}
	}
