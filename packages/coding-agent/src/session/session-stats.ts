...
interface PendingContextSnapshot {
	...
	cutoffCount: number;
	/** Prompt messages already in flight when this snapshot was captured. */
...
function messageSnapshotKey(message: AgentMessage): string {
	const rawContent = "content" in message ? message.content : message;
	const content =
		typeof rawContent === "string"
			? rawContent
			: Array.isArray(rawContent)
				? rawContent.map(block => (block.type === "text" ? block.text : JSON.stringify(block))).join("\u0000")
				: JSON.stringify(rawContent);
	const customType = "customType" in message ? message.customType : "";
	return `${message.role}\u0000${customType}\u0000${content}`;
}
...
export class SessionStatsTracker {
	...
	setPendingSnapshot(snapshot: Omit<PendingContextSnapshot, "epoch" | "pendingMessageKeys"> | undefined): void {
		this.#pendingContextSnapshot = snapshot
			? {
					...snapshot,
					pendingMessageKeys: snapshot.pendingMessages.map(messageSnapshotKey),
	...
	rebaseAfterCompaction(): void {
		...
		const pendingMessageKeys = new Set(pending.pendingMessageKeys);
		const messages = this.#host.agent.state.messages.filter(
			message => !pending.pendingMessages.includes(message) && !pendingMessageKeys.has(messageSnapshotKey(message)),
		);
