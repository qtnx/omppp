...
/**
 * Resolve a blob ref against the agent-wide blob store, for render paths that
 * materialize attachment links without a session handle. Returns the input
 * unchanged when it is not a ref, and the raw ref when the blob is gone, so
 * callers can skip the unavailable image.
 */
export function resolveAgentImageDataSync(data: string): string {
	return resolveImageDataSync(new BlobStore(getBlobsDir()), data);
}
