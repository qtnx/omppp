...
function getEditDestructiveIntent(args: unknown): { kind: "delete" | "move"; paths: string[] } | undefined {
	...
	if (input) {
		try {
			...
			const deleteSection = patch.sections.find(section => section.fileOp?.kind === "rem");
			if (deleteSection) return { kind: "delete", paths: [deleteSection.path] };
			const moveSection = patch.sections.find(section => section.fileOp?.kind === "move");
			if (moveSection?.fileOp?.kind === "move") {
				return { kind: "move", paths: [moveSection.path, moveSection.fileOp.dest] };
			}
		} catch {
