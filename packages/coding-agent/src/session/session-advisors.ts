 ...
 */
const ADVISOR_OFF_BY_DEFAULT_MODEL_IDS = ["claude-opus-5", "claude-fable-5", "gpt-5.6", "gpt-6-astra"] as const;
...
export class SessionAdvisors {
	...
	/** Reset per-user-prompt completion review state. */
	onUserPrompt(): void {
		this.#advisorDoneGateRejections = 0;
	}

	/**
	 * Review a mutating completion claim before the primary turn is allowed to
	 * settle. Rejects append a persisted reminder and schedule one continuation.
	 */
	async checkDoneGate(finalMessage: AssistantMessage): Promise<boolean> {
		...
		if (passiveCanRun) {
			...
			const postPromptSignal = this.#host.postPromptSignal();
			...
			const delayedNull = (delayMs: number): Promise<null> => {
				const { promise, resolve } = Promise.withResolvers<null>();
				timers.push(setTimeout(() => resolve(null), delayMs));
				return promise;
			};
			const cleanup = () => {
			...
			try {
				...
				const consultRace = consultPromise.then(() => delayedNull(5_000));
				const { promise: abortRace, resolve: resolveAbort } = Promise.withResolvers<null>();
				...
				verdict = await Promise.race([verdictPromise, consultRace, abortRace, delayedNull(90_000)]);
			} finally {
				cleanup();
			}
	...
	/** Pin the live advisor to Duo's configured advisor/planner model. */
	...
	/** Release Duo's advisor pin without changing a user-owned advisor setting. */
	stopDuoAdvisor(): void {
	...
	/** Resume all live runtimes after Duo returns the main stream to its executor. */
	resumeAll(): void {
	...
	#availableModelsForAdvisorRuntime(): Model[] {
		if (typeof this.#host.modelRegistry.getAvailable !== "function") return [];
		return this.#host.modelRegistry.getAvailable();
	}
	...
	#restoreDuoAdvisorPin(snapshot: DuoStateSnapshot | undefined): void {
		...
		if (this.#duoOwnsAdvisor) this.#advisorEnabled = true;
		const pinned = resolveModelFromString(
			snapshot.advisorModelId,
			this.#availableModelsForAdvisorRuntime(),
		...
		if (!this.#host.modelRegistry.hasConfiguredAuth(pinned)) {
			this.#host.emitNotice(
				"warning",
	...
	#resolveDuoAdvisorPin(planner: Model): { model: Model; thinkingLevel?: ThinkingLevel } {
		...
		if (resolved.model && this.#host.modelRegistry.hasConfiguredAuth(resolved.model)) {
			return { model: resolved.model, thinkingLevel: concreteThinkingLevel(resolved.thinkingLevel) };
		}
		this.#host.emitNotice(
			"warning",
			`Duo advisor model "${configured}" unavailable; using planner model.`,
			"advisor",
		);
	...
	#resolveAdvisorRuntimeDescriptors(emitWarnings: boolean): AdvisorRuntimeDescriptor[] {
		...
		for (const config of roster) {
			...
			if (this.#duoAdvisorPinnedModel) {
				...
				const sel = resolveAdvisorRoleSelection(this.#host.settings, this.#availableModelsForAdvisorRuntime());
				if (!sel) {
					this.#advisorStatuses.set(slug, { name: config.name, status: "no_model" });
					if (emitWarnings) {
	...
	#buildAdvisorRuntime(seedToCurrent = false, emitWarnings = true): boolean {
		...
		// Leave stale entries. Effective descriptor resolution also applies any
		// active Duo pin before the runtime is built.
		this.#advisorStatuses.clear();
		const descriptors = this.#resolveEffectiveAdvisorRuntimeDescriptors(emitWarnings);
		...
		for (const descriptor of descriptors) {
			...
			const duoStatus = this.#host.duoStatus();
			const duoPhase = duoStatus?.phase ?? this.#initialDuoPhase;
			const duoTakeoverTools =
				duoPhase !== undefined
					? [
							new RequestTakeoverTool(this.#host.requestDuoTakeover, reason =>
								this.#host.requestDuoPlanTakeover(reason),
							),
						]
					: [];
			...
			const duoAdvisorTools =
				isDuoOwnedAdvisor && duoPhase !== undefined && this.#toolSession
					? [
							new ReadAdvisorStateTool(this.#toolSession),
							new UpdateAdvisorStateTool(this.#toolSession),
							new UpdateBriefTool(this.#toolSession),
							new SetTodosTool(this.#toolSession),
							new SetExecutorEffortTool((level, reason) => this.#host.setDuoExecutorEffort(level, reason)),
						]
					: [];
			...
			if (duoPhase !== undefined && duoPhase !== "inactive" && duoPhase !== "suspended") {
				systemPrompt.push(
					renderDuoAdvisorInstructions({
						cooldownRemaining: 0,
			...
			const advisorAgentFacade: AdvisorAgent = {
				prompt: async (input, images) => {
					...
					try {
						...
						else await advisorAgent.prompt(input, images);
			...
			const runtime = new AdvisorRuntime(
				advisorAgentFacade,
				{
					...
					onTurnError: (error, _model, failedMessages, signal) =>
		...
		if (this.#advisors.length > 0) this.#host.advisorRuntimeStarted();
	...
	#routeAdvice(advisor: ActiveAdvisor, note: string, severity?: AdvisorSeverity): void {
		...
		void this.#host
			.sendCustomMessage(
				...
				{ deliverAs: "steer", triggerTurn: true, interruptToolExecution: channel !== "boundary" },
			)
	...
	/** Enqueue a non-blocking consult for the primary advisor. */
	consultAsync(question: string): boolean {
		const advisor = this.#advisors[0];
