...
function externalizeSnapcompactFrameForPersistenceSync(
	...
	if (
		...
		frame.data.includes("[Session persistence truncated") ||
...
function truncateForPersistence(obj: unknown, blobStore: BlobStore, key?: string): unknown {
	...
	if (key === "frames" && isPersistedSnapcompactFrame(obj)) {
		return externalizeSnapcompactFrameForPersistenceSync(obj as { data: string; mimeType?: string }, blobStore);
	}
