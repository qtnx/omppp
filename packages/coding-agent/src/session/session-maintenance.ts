...
export class SessionMaintenance {
	...
	async checkCompaction(
		...
		if (sameModel && !errorIsFromBeforeCompaction && AIError.isContextOverflow(assistantMessage, contextWindow)) {
			...
			// Compaction was disabled or skipped without rewriting history. Retry
			// on a larger context only after the current-model recovery path has
			// had a chance to preserve the session's compaction contract.
		...
		// deliverable. Compaction gets first chance to recover the current model;
		// promotion is the fallback only when no history rewrite happened.
		...
		try {
			...
			if (!this.#model) {
				if (reason === "requested") {
					this.#host.emitNotice(
						"warning",
						"remote compaction is unavailable for this session (no active model) — using a local summary instead",
						"compaction",
					);
				}
				await this.#host.emitSessionEvent({
			...
			if (compactionPrep.kind === "fromHook") {
				...
				let candidates = this.#getCompactionModelCandidates(
					availableModels,
					reason === "requested" ? shouldUseOpenAiRemoteCompaction : undefined,
				);
				if (reason === "requested" && candidates.length === 0) {
					this.#host.emitNotice(
						"warning",
						`remote compaction is unavailable for ${this.#model.id} (no provider-native remote-capable model in the fallback chain) — using a local summary instead`,
						"compaction",
					);
			...
			const baseResult = continuationScheduled
				? COMPACTION_CHECK_CONTINUATION
				: noProgressDeadEnd
					? COMPACTION_CHECK_BLOCK_AUTOMATIC_CONTINUATION
					: COMPACTION_CHECK_NONE;
			return { ...baseResult, historyRewritten: true };
		} catch (error) {
			if (autoCompactionSignal.aborted) {
				await this.#host.emitSessionEvent({
					type: "auto_compaction_end",
