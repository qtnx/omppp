...
import { convertToLlm, stripImagesFromMessage, stripOversizedCompactionSummaryImagesForCodex } from "./messages";
...
export class SessionMaintenance {
	...
	async checkCompaction(
		...
		if (overflowEvidence || (payloadRejection && !trustedPayloadRejection)) {
			...
			// Compaction did not rewrite or defer the turn, and no promotion target exists.
			if (payloadRejection && compactionOutcome === undefined) {
	...
	async runAutoCompaction(
		...
		const compactionSettings = this.#host.settings.getGroup("compaction");
		if (reason !== "idle" && reason !== "requested" && !compactionSettings.enabled) return COMPACTION_CHECK_NONE;
