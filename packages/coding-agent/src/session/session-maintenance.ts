...
export interface PendingAgentCompactionRequest {
	reason: string;
...
export interface SessionMaintenanceHost {
	...
	pendingAgentCompactionRequest(): PendingAgentCompactionRequest | undefined;
	consumePendingAgentCompactionRequest(): void;
...
export class SessionMaintenance {
	...
	/**
	 * Adjusts raw context usage after persisted Context-GC unload deltas.
	 * A failed or abandoned cleanup writes no unload delta and retains its base usage.
	 */
	estimateEffectiveContextTokens(baseTokens: number): number {
		try {
	...
	canCompact(): boolean {
		...
		return (
			branch.at(-1)?.id === staleEntry?.id &&
			(snapcompact.getPreservedArchive(staleEntry?.preserveData)?.frames.length ?? 0) > 1
		);
	}
	...
	#estimatePendingPromptProviderBytes(messages: AgentMessage[], maxPayloadBytes: number): number | undefined {
		try {
			const providerMessages = this.#host.convertToLlmForSideRequest([...this.#host.messages(), ...messages]);
			const systemPrompt = this.#host.agent.state.systemPrompt;
			const providerSystemPrompt = Array.isArray(systemPrompt)
				? systemPrompt.map(prompt => this.#host.obfuscateTextForProvider(prompt) ?? prompt)
				: [this.#host.obfuscateTextForProvider(systemPrompt) ?? systemPrompt];
	...
	async runPrePromptCompactionIfNeeded(messages: AgentMessage[]): Promise<void> {
		...
		if (!shouldCompactForTokens && !shouldCompactForProviderBytes) {
		...
		// Speculation is keyed to token pressure. A provider payload cap can be
		// exceeded while token use is low, and an armed summary does not shrink the
		// request being sent now, so that path must compact synchronously.
		if (shouldCompactForTokens && this.deferThresholdCompactionToSpeculation(contextTokens, contextWindow)) {
			logger.debug("Pre-prompt token threshold deferred to speculative compaction", {
				contextTokens,
				contextWindow,
			});
			return;
		...
		// A payload-bound prompt must compact before considering promotion: a larger
		// model window does not necessarily raise the provider's request-size limit.
		const outcome = await this.runAutoCompaction("threshold", false, false, false, {
			autoContinue: false,
			suppressContinuation: true,
		...
		if (outcome.historyRewritten || outcome.deferredHandoff || outcome.continuationScheduled) return;

		if (await this.#promoteContextModel()) {
			logger.debug("Pre-prompt context promotion took over after compaction could not rewrite history", {
				contextTokens,
				contextWindow,
	...
	async maintainContextMidRun(
		...
		const agentRequested = this.#host.pendingAgentCompactionRequest();
		...
		if (!shouldCompact(contextTokens, contextWindow, compactionSettings) && !agentRequested) {
			this.maybeStartSpeculativeCompaction(contextTokens, contextWindow);
			return;
		}
		...
		if (!agentRequested && this.deferThresholdCompactionToSpeculation(contextTokens, contextWindow)) {
		...
		if (agentRequested) this.#host.consumePendingAgentCompactionRequest();
		...
		if (!agentRequested && (await this.#promoteContextModel())) {
		...
		const result = await this.runAutoCompaction(agentRequested ? "requested" : "threshold", false, false, false, {
			autoContinue: false,
			suppressContinuation: true,
			triggerContextTokens: contextTokens,
			phase: "mid_turn",
			customInstructions: agentRequested?.focus,
			detachPostCommit: true,
		});
		if (result.automaticContinuationBlocked) {
			this.#midTurnCompactionDeadEnds.add(activeMessages);
	 ...
	 *
	 * Four cases (in order):
	 * 1. Input overflow: run context maintenance; if it cannot rewrite history,
	 *    promote to a larger model and retry.
	 * 2. Output incomplete (stopReason === "length", e.g. `response.incomplete`):
	 *    run compaction/handoff; if it cannot rewrite history, try promotion.
	...
	async checkCompaction(
		...
		if (overflowEvidence || (payloadRejection && !trustedPayloadRejection)) {
			...
			const compactionOutcome =
				...
				hasUsableCompactionMethod("overflow", this.#model, compactionSettings, excludeMediaForPayloadRejection)
					...
					: undefined;
			if (
				compactionOutcome?.historyRewritten ||
				compactionOutcome?.deferredHandoff ||
				compactionOutcome?.continuationScheduled
			) {
			...
			// No promotion target available; fall through to compaction. The
			// pre-promotion attempt above ran only when a method was already
			// configured, so retry here through the overflow-usable method
			// selection, which also carries the media-exclusion decision taken up
			// front for a payload rejection (#11482).
			...
			if (compactionResult !== undefined) return compactionResult;
			if (payloadRejection) {
				this.#host.emitNotice(
					"warning",
					usageBackedOverflow
			...
			return compactionOutcome ?? COMPACTION_CHECK_NONE;
		}
		...
		if (sameModel && !errorIsFromBeforeCompaction && assistantMessage.stopReason === "length") {
			...
			logger.warn("response.incomplete with no recovery path (compaction + promotion both unavailable)", {
				model: `${assistantMessage.provider}/${assistantMessage.model}`,
			});
			return COMPACTION_CHECK_NONE;
		}
		...
		if (shouldThresholdCompact) {
			...
			const outcome = await this.runAutoCompaction("threshold", false, false, allowDefer, {
				autoContinue,
				triggerContextTokens: postMaintenanceContextTokens,
				phase: "pre_turn",
				terminalTextAnswer: isTerminalTextAssistantAnswer(assistantMessage),
			});
			if (outcome.historyRewritten || outcome.deferredHandoff || outcome.continuationScheduled) return outcome;

			if (await this.#tryContextPromotion(assistantMessage)) {
				logger.debug("Auto-compaction threshold could not rewrite history; context promotion took over", {
					contextTokens,
					contextWindow,
					model: `${assistantMessage.provider}/${assistantMessage.model}`,
				});
	...
	async runAutoCompaction(
		...
		if (reason !== "idle" && reason !== "requested" && !compactionSettings.enabled && !explicitNewContextRequest) {
			return COMPACTION_CHECK_NONE;
		}
		if (this.#usesExperimentalContextManagement()) {
		...
		try {
			...
			if (this.#autoCompactionAbortController === autoCompactionAbortController) {
				this.#autoCompactionAbortController = undefined;
				// Auto-compaction can schedule a queued continuation before its controller
				// is cleared. That task correctly refuses to run while maintenance is active,
				// so re-drain after the outer controller is gone rather than stranding the queue.
				this.#host.drainStrandedQueuedMessages();
			}
		}
		return COMPACTION_CHECK_NONE;
