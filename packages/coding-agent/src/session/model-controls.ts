import { type Agent, type AgentMessage, type AgentTurnEndContext, ThinkingLevel } from "@oh-my-pi/pi-agent-core";
...
export class ModelControls {
	...
	constructor(
		...
		if (options.reasoningSlide) {
			const { afterTurns, onFirstAction } = options.reasoningSlide;
			...
			this.#reasoningSlide = options.reasoningSlide;
		}
	}
	...
	/** Advances the configured one-way model slide at a completed assistant-turn boundary. */
	...
	#noteReasoningSlidePlanDelivery(reasoningSlide: ReasoningSlide, message: AgentTurnEndContext["message"]): void {
		...
		if (textChars >= REASONING_SLIDE_PLAN_MIN_CHARS) {
			...
			this.#host.emitNotice("info", "Reasoning slide: plan delivered.", "reasoning-slide");
		}
	}
	...
	#maybeInjectReasoningSlidePlanNudge(reasoningSlide: ReasoningSlide): void {
		...
		this.#host.emitNotice("info", "Reasoning slide: injected deep-plan nudge.", "reasoning-slide");
	}
