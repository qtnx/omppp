...
import { type CustomMessage, MEMORY_CONTEXT_MESSAGE_TYPE } from "./messages";
...
interface SessionToolsOptions {
	...
	rebuildSystemPrompt?: (
		...
		options?: SystemPromptRebuildContext & { directToolNames?: readonly string[] },
...
/**
 * Prompt plus staged volatile context for one agent run. The recall rides a
 * hidden user-attributed message instead of the system prompt so provider
 * prefix caches survive it; the caller publishes `memoryContextMessage` only
 * after delivery validation, gated on {@link AgentStartContext.commitMemory}.
 */
...
export class SessionTools {
	...
	async #applyActiveToolsByName(toolNames: string[], forcePromptRefresh = false, signal?: AbortSignal): Promise<void> {
		...
		const transportNeeded =
			(writeSelected || this.#deviceOnlyWriteTransportAvailable) &&
		...
		const nextEnabledToolNames = new Set([...validToolNames, ...mountNames]);
		const nextToolPredicateNames = codeMode.active ? [...nextEnabledToolNames] : appliedNames;
		const nextCodeModeDirectToolNames = codeMode.active ? appliedNames : undefined;
		...
		// Revoke removed write immediately while rebuilding its prompt. Other
		// removals stay visible until commit so tool changes remain atomic.
		if (previousActiveToolNames.includes("write") && !nextToolPredicateNames.includes("write")) {
			this.#toolPredicateNames = nextToolPredicateNames;
		...
		try {
			...
			if (this.#rebuildSystemPrompt) {
				...
				const promptToolNames = codeMode.active ? [...nextEnabledToolNames] : appliedNames;
		...
		// Publish the new runtime/tool-predicate surface only after the matching
		// prompt rebuild succeeds, so readers never observe a split generation.
		this.#enabledToolNames = nextEnabledToolNames;
		this.#setMountedNames(mountNames);
		this.#toolPredicateNames = nextToolPredicateNames;
		this.#setActiveToolNames?.(nextToolPredicateNames);
		this.#codeModeDirectToolNames = nextCodeModeDirectToolNames;
		try {
	...
	/**
	 * Builds the stable system prompt and the staged volatile memory context for
	 * one agent run. Volatile recall is returned as a hidden user-attributed
	 * message rather than appended to the system prompt: keeping it out of the
	 * prompt preserves provider prefix caches, while the recall still reaches the
	 * model and the persisted transcript.
	 *
	 * `stageMemory: false` skips the backend hook entirely — a queued batch
	...
	async #applyMCPToolRefresh(mcpTools: CustomTool[], options?: { activateAll?: boolean }): Promise<void> {
		await this.#runMCPToolMutation(async () => {
			...
			// The settled mutation promise may retain this async frame; drop rollback
			// references as soon as the new generation commits.
