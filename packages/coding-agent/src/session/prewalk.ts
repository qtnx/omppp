...
function isAwaitingUserAnswer(message: AssistantMessage): boolean {
	...
	const candidate = promptLine(lastLine);
	const question =
	...
	const responseCue = USER_RESPONSE_CUE_RE.test(candidate.text.replace(/[.!?。！？]+$/, "").trim());
	return question || responseCue;
}
...
export class PrewalkCoordinator {
	...
	async advanceAtTurnEnd(liveMessages: AgentMessage[], context: AgentTurnEndContext | undefined): Promise<void> {
		...
		} else if (this.#continuePending && !isAwaitingUserAnswer(context.message)) {
			this.#continuePending = false;
