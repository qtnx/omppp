...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		const advisorSkillsAndRulesPrompt = buildAdvisorSkillsAndRulesPrompt(this.#tools.baseSystemPrompt);
		const advisorContextPrompt =
	...
	#persistMessageEnd(message: AgentMessage, promptGeneration: number = this.#promptGeneration): void {
	...
	async #probeUsageHealthBeforeEnqueue(signal?: AbortSignal): Promise<boolean> {
		...
		try {
			try {
				await this.#modelRegistry.authStorage.getModelUsageHealth(model.provider, {
					modelId: model.id,
					sessionId: this.sessionId,
	...
	#createMagicKeywordNotices(text: string): CustomMessage[] {
		...
		if (this.#magicKeywordEnabled("workflow") && requestsWorkflow(text)) {
			const enabledToolNames = this.getEnabledToolNames();
			if (
				enabledToolNames.includes("workflow") ||
				(enabledToolNames.includes("task") && enabledToolNames.includes("eval"))
			) {
				keywordNotices.push({
					role: "custom",
