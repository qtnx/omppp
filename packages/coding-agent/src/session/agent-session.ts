...
export class AgentSession {
	...
	async #queueUserMessage(
		...
		const queuedMessages = dollarMentionMessages.length > 0 ? [...dollarMentionMessages, message] : message;
		if (mode === "followUp") {
	...
	async #runAutoCompaction(
		...
		try {
			if (compactionSettings.strategy === "handoff" && reason !== "overflow" && reason !== "topic-switch") {
