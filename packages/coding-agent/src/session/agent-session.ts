...
export class AgentSession {
	...
	#wakeForIrc(records: CustomMessage[], onWakeFailure?: () => void | Promise<void>): void {
		...
		void this.agent
			...
			.then(async () => {
				...
				if (failedResponse) await handleWakeFailure(failedResponse.errorMessage ?? "provider error");
			})
			.catch(handleWakeFailure)
			.finally(() => {
				if (parkedFollowUps.length > 0) {
	...
	#createCommandContext(): ExtensionCommandContext {
		...
		return {
			...
			getAsyncJobSnapshot: options => this.getAsyncJobSnapshot({ recentLimit: options?.recentLimit }),
	...
	async followUp(text: string, images?: ImageContent[], options?: FollowUpOptions): Promise<void> {
		...
		// The synthetic branch below is agent-authored (plan directives, reminders);
		// only the user-attributed follow-up carries text a user could have typed.
		const expandedText = await this.#applyPromptSecretPolicy(templated, options?.synthetic);
		if (!(await this.#runUsageAwarePreflight())) return;
