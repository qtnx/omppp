...
import {
	parseDuoStateSnapshot,
	SessionDuoOrchestrator,
...
export {
	...
	resolveDuoPlanTakeoverDecision,
...
export class AgentSession {
	...
	#subagentWaitDepth = 0;
	#heldSteering: Array<{ text: string; images: ImageContent[] | undefined }> = [];
	#pendingAgentCompactionRequest: { reason: string; focus: string | undefined } | undefined;
	#pendingAgentShakeRequest: { mode: ShakeMode } | undefined;
	...
	#wakeForIrc(records: AgentMessage[], onWakeFailure?: () => void | Promise<void>): void {
		...
		const handleWakeFailure = async (error: unknown): Promise<void> => {
			logger.error("IRC wake turn produced no completed response", { error: String(error) });
			try {
				await onWakeFailure?.();
			} catch (callbackError) {
				logger.warn("IRC wake-failure callback failed", { error: String(callbackError) });
			}
		};
		...
		void this.whenWorkPoolYieldSettled()
			.then(() => {
				...
				// agent.prompt() claims streaming with no await in between, and
				// the yield contract above mutates synchronously, so no install can
				// interleave here.
				if (this.#workPoolYieldItems.length > 0) {
			...
			.then(async () => {
				const failedResponse = this.agent.state.messages
					.slice(messageCount)
					.find(
				...
				if (failedResponse) {
					turnError = failedResponse.errorMessage ?? "provider error";
					await handleWakeFailure(turnError);
				}
			})
			.catch(async error => {
	...
	constructor(config: AgentSessionConfig) {
		...
		const bashHost: BashRunnerHost = {
			...
			appendSessionMessage: message => {
				...
				this.#stampLiveMessageEntryId(message, entryId);
		...
		const ircHost: IrcBridgeHost = {
			...
			wakeForIrc: (records, onWakeFailure) => this.#wakeForIrc(records, onWakeFailure),
		...
		const prewalkHost: PrewalkCoordinatorHost = {
			...
			setActiveToolsByName: names => this.setActiveToolsByNamePreservingMCPSelection(names),
		...
		const modelControlsHost: ModelControlsHost = {
			...
			onModelChanged: () => {
				if (!this.#duoSwitchInProgress) this.#duoOrchestrator?.notifyManualModelChange();
			},
		...
		this.#models = new ModelControls(modelControlsHost, {
			...
			reasoningSlide: config.reasoningSlide,
		...
		const sessionToolsHost: SessionToolsHost = {
			...
			planModeEnabled: () => this.#planModeState?.enabled === true,
			orchestratorModeEnabled: () => this.#duoOrchestrator?.orchestratorModeState?.enabled === true,
			model: () => this.model,
		...
		this.#tools = new SessionTools(sessionToolsHost, {
			...
			explicitlyRequestedToolNames: config.explicitlyRequestedToolNames,
			effectiveDiscoveryMode: config.effectiveDiscoveryMode,
		...
		if (config.persistInitialMCPToolSelection && config.mcpDiscoveryEnabled) {
			this.sessionManager.appendMCPToolSelection(this.#tools.getSelectedMCPToolNames());
		}
		...
		let restoredDuoSnapshot: DuoStateSnapshot | undefined;
		const branch = this.sessionManager.getBranch();
		for (let index = branch.length - 1; index >= 0; index--) {
			const entry = branch[index];
			if (entry.type !== "custom_message" || entry.customType !== "duo_state") continue;
			const parsed = parseDuoStateSnapshot(entry.details);
			if (!parsed) continue;
			restoredDuoSnapshot = parsed;
			break;
		}
		...
		const advisorsHost: SessionAdvisorsHost = {
			...
			agentKind: () => this.#agentKind,
			emitSessionEvent: event => this.#emitSessionEvent(event),
			advisorRuntimeStarted: () => {
				void this.#tools.ensureConsultToolAvailable().catch(error => {
					logger.warn("Failed to activate live advisor tool", {
			...
			setDuoExecutorEffort: (level, reason) =>
		...
		const duoHost: SessionDuoOrchestratorHost = {
			...
			setThinkingLevel: level => this.setThinkingLevel(level),
			configuredThinkingLevel: () => this.configuredThinkingLevel(),
			ensureDuoAdvisorStarted: planner => this.#advisors.ensureDuoAdvisorStarted(planner),
			stopDuoAdvisor: () => this.#advisors.stopDuoAdvisor(),
			scheduleDuoAdvisorRevive: retryAfterMs => this.#duoOrchestrator.scheduleAdvisorRevive(retryAfterMs),
			pauseAdvisors: () => this.#advisors.pauseAll(),
			resumeAdvisors: () => this.#advisors.resumeAll(),
			injectDuoBrief: (content, deliverAs) => {
				void this.sendCustomMessage(
					{ customType: "duo", content, display: true, attribution: "agent" },
					{ deliverAs },
				);
			},
			emitNotice: (level, message, source) => this.emitNotice(level, message, source),
			persistDuoSnapshot: snapshot => {
				this.sessionManager.appendCustomMessageEntry(
					"duo_state",
					"",
					false,
					this.#advisors.snapshotWithDuoAdvisorPin(snapshot),
					"agent",
				);
			},
			planArtifactReady: () => {
			...
			persistModeChange: enabled => {
				if (this.#goalModeState?.enabled === true) {
					this.sessionManager.appendModeChange("goal", {
						goal: this.#goalModeState.goal,
						orchestrator: enabled,
					});
				} else {
					this.sessionManager.appendModeChange(enabled ? "orchestrator" : "none");
				}
			},
		...
		const maintenanceHost: SessionMaintenanceHost = {
			...
			pendingAgentCompactionRequest: () => this.#pendingAgentCompactionRequest,
			consumePendingAgentCompactionRequest: () => {
	...
	/**
	 * True when a waiting tool should return so queued IRC, yield-queue, or
	 * interrupting steering context can be injected at the next boundary.
	 */
	...
	/** True while this session has queued work that can resume the current agent flow or owner-scoped async work. */
	hasPendingAgentWork(): boolean {
		return (
			this.agent.peekSteeringQueue().filter(isDisplayableQueuedMessage).length > 0 ||
			this.agent.peekFollowUpQueue().filter(isDisplayableQueuedMessage).length > 0 ||
	...
	/**
	 * Subscriber fan-out happens synchronously below, before any extension
	 * await, so consecutive events reach subscribers in emission order without
	 * a FIFO ticket. The earlier gate ordered fan-out that ran *after* the
	 * extension emit — an instant refusal delivered its assistant `message_end`
	 * to the TUI before its own `message_start` — and would now only let a
	 * stalled extension listener hold every later event's emit promise (and the
	 * prompt awaiting it) hostage.
	 */
	...
	#stampLiveMessageEntryId(message: AgentMessage, entryId: string): void {
		(message as AgentMessage & { entryId?: string }).entryId = entryId;
	}

	#appendCustomMessageToAgent(message: CustomMessage): void {
	...
	#prePersistPromptMessages(messages: readonly AgentMessage[]): void {
		for (const message of messages) {
			if (message.role === "hookMessage" || message.role === "custom") {
				...
				if (message.role === "custom") (message as CustomMessage).entryId = entryId;
				continue;
			}
			if (
				message.role !== "user" &&
			...
			const entryId = this.#appendSessionMessage({ ...message });
			this.#prePersistedPromptMessages.add(message);
			if (message.role === "fileMention") (message as FileMentionMessage).entryId = entryId;
		}
	}
	...
	#persistMessageEnd(message: AgentMessage, promptGeneration: number): void {
		...
		if (!alreadyPersisted) this.#persistSessionMessageIfMissing(message);
	}
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "assistant") {
				...
				if (assistantMsg.provider === "opencode-go") {
					this.#modelRegistry.authStorage.usage.recordCost(assistantMsg.provider, assistantMsg.usage.cost.total, {
		...
		if (event.type === "agent_end") {
			...
			if (msg.stopReason === "aborted") {
				...
				if (this.#pendingAgentShakeRequest) {
					const { mode } = this.#pendingAgentShakeRequest;
					this.#pendingAgentShakeRequest = undefined;
					await this.shake(mode);
				}
				if (this.#pendingAgentCompactionRequest) {
					const request = this.#pendingAgentCompactionRequest;
					this.#pendingAgentCompactionRequest = undefined;
					await this.#maintenance.runAutoCompaction("requested", false, false, false, {
			...
			if (activeGoal) {
				...
				if (compactionContinues || compactionResult.automaticContinuationBlocked) {
					const queuedMessageDrainScheduled = !compactionContinues && this.agent.hasQueuedMessages();
					if (queuedMessageDrainScheduled) this.#scheduleQueuedMessageDrain();
					...
					await emitAgentEndNotification(
						compactionResult.continuationScheduled || queuedMessageDrainScheduled
							? { willContinue: true }
							: undefined,
					);
					return;
			...
			const queuedMessageDrainScheduled =
			...
			if (msg.stopReason !== "error" && emptyOutputRecovery !== "terminal") {
				...
				const doneGateContinuationScheduled = await this.#advisors.checkDoneGate(msg);
	...
	async #doDispose(options: AgentSessionDisposeOptions = {}): Promise<void> {
		...
		try {
			await disableAnnotateHttp(this);
		} catch (error) {
	...
	/**
	 * Preserve enqueue cancellation semantics without selecting a fallback before
	 * a queued turn is ready to run. The dequeue hook performs the authoritative
	...
	async #probeUsageHealthBeforeEnqueue(signal?: AbortSignal): Promise<boolean> {
		...
		try {
			try {
				await this.#modelRegistry.authStorage.health.model(model.provider, {
					modelId: model.id,
					sessionId: this.sessionId,
				...
				// Match the full preflight's fail-open behavior for unavailable health data.
			}
	...
	isMCPDiscoveryEnabled(): boolean {
		return this.#tools.isMCPDiscoveryEnabled();
	}

	enableMCPDiscoveryWithSearchTool(mode?: EffectiveToolDiscoveryMode): Promise<void> {
		return this.#tools.enableMCPDiscoveryWithSearchTool(mode);
	}

	activateDiscoveredMCPTools(toolNames: string[]): Promise<string[]> {
		return this.#tools.activateDiscoveredMCPTools(toolNames);
	}

	isToolDiscoveryEnabled(): boolean {
		return this.#tools.isToolDiscoveryEnabled();
	}

	getDiscoverableTools(filter?: Parameters<SessionTools["getDiscoverableTools"]>[0]) {
	...
	/** Replace the session skill snapshot after plugin or skill rediscovery. */
	setSkills(skills: readonly Skill[], warnings: readonly SkillWarning[], settings?: SkillsSettings): void {
		this.#tools.setSkills(skills, warnings, settings);
	}
	...
	setActiveToolsByNamePreservingMCPSelection(toolNames: string[]): Promise<void> {
		return this.#tools.setActiveToolsByNamePreservingMCPSelection(toolNames);
	}
	...
	/** Resumes an interrupted assistant turn after the sandbox supervisor relaunches this process. */
	...
	requestCompactionFromAgent(reason: string, options?: { focus?: string }): ToolCompactionRequest {
		...
		if (this.#pendingAgentCompactionRequest) {
			if (options?.focus !== undefined) this.#pendingAgentCompactionRequest.focus = options.focus;
			return { status: "already-scheduled" };
		}
		if (!this.#maintenance.canCompact()) {
			return { status: "unavailable", detail: "nothing to compact yet (session too small or already compacted)" };
		}
		this.#pendingAgentCompactionRequest = { reason, focus: options?.focus };
	...
	considerCompactionWhileWaiting(reason: string, options?: { focus?: string }): ToolWaitingCompactionCheck {
		...
		if (this.#pendingAgentCompactionRequest) {
			...
			return this.requestCompactionFromAgent(reason, focus ? { focus } : undefined);
		}
		const contextWindow = this.model?.contextWindow ?? 0;
		if (contextWindow === 0) return { status: "unavailable", detail: "current model has no context window" };
		const usage = this.getContextUsage({ contextWindow });
		if (!usage || !shouldCompact(usage.tokens, contextWindow, compactionSettings)) return { status: "not-needed" };
		const focus = options?.focus ? `${subagentWaitFocusPrompt}\n${options.focus}` : subagentWaitFocusPrompt;
		return this.requestCompactionFromAgent(reason, { focus });
	}

	/** Schedules agent-requested shake for the next settled turn. */
	requestShakeFromAgent(mode: ShakeMode): ToolShakeRequest {
		if (this.#pendingAgentShakeRequest) return { status: "already-scheduled" };
	...
	setPlanModeState(state: PlanModeState | undefined): void {
		...
		if (previous?.enabled && this.#planReferencePath.trim()) {
			this.#duoOrchestrator.notifyPlanApproved();
		}
	}
	getOrchestratorModeState(): OrchestratorModeState | undefined {
		return this.#duoOrchestrator.orchestratorModeState;
	}
	...
	getDuoStatus(): DuoStatus | undefined {
		return this.#duoOrchestrator.status;
	}

	setDuoEnabled(enabled: boolean): Promise<void> {
		return this.#duoOrchestrator.setDuoEnabled(enabled);
	}
	...
	duoHandoffToExecutor(resolution: string, scope?: DuoExecutionScope): Promise<DuoHandoffResult> {
		return this.#duoOrchestrator.handoffToExecutor(resolution, scope);
	}
	...
	duoReplan(): Promise<boolean> {
		return this.#duoOrchestrator.replan();
	}

	duoSummon(): Promise<boolean> {
		return this.#duoOrchestrator.summonPlanner();
	}
	...
	async #restoreMCPToolSelectionFromSessionContext(useDefaultSelection = true): Promise<void> {
		const context = this.sessionManager.buildSessionContext();
		await this.#tools.restoreMCPToolSelection(
			context.selectedMCPToolNames,
			context.hasPersistedMCPToolSelection,
			useDefaultSelection,
		);
	}
	...
	#createMagicKeywordNotices(text: string): CustomMessage[] {
		...
		for (const keyword of MAGIC_KEYWORDS) {
			...
			// OMPx fork: orchestrate/workflow gate on explicit directive intent, the
			// rest on a standalone prose occurrence.
			...
			if (!keyword.applies(context)) continue;
			keywordNotices.push({
				role: "custom",
	...
	async #dispatchPrompt(
		...
		const mentionsExpanded = options?.synthetic ? templated : this.#modelMentions.expandMentions(templated);
		const expandedText = await this.#applyPromptSecretPolicy(mentionsExpanded, options?.synthetic);
	...
	> {
		...
		for (let attempt = 0; attempt < AGENT_START_POLICY_MAX_ATTEMPTS; attempt++) {
			...
			const agentStartContext = await this.#tools.buildAgentStartContext(prompt, { stageMemory }, isCurrent, signal);
			if (!isCurrent()) return cancelled;
			...
			return {
				...
				commit: () => {
					...
					if (result?.systemPrompt !== undefined) {
						...
						// The agent prompt carries the runtime overlay, which a base rebuild during
						// the awaits above may have changed.
	...
	async #promptWithMessage(
		...
		try {
			...
			const userCompactIntent =
				message.role === "user" && !options?.skipCompactionCheck && detectUserCompactIntent(expandedText);
			let compactionPerformedForIntent = false;
			if (userCompactIntent) {
				const result = await this.#maintenance.runAutoCompaction("requested", false, false, false);
				compactionPerformedForIntent = result.historyRewritten === true;
				if (compactionPerformedForIntent) {
					...
					if (Array.isArray(message.content)) {
						...
						if (textBlock) textBlock.text = stripped;
					}
				}
			} else {
	...
	#createCommandContext(): ExtensionCommandContext {
		...
		return {
			...
			getAsyncJobSnapshot: (options?: { recentLimit?: number }) =>
				this.getAsyncJobSnapshot({ recentLimit: options?.recentLimit }),
	...
	exitSubagentWait(): void {
		if (this.#subagentWaitDepth === 0) return;
		this.#subagentWaitDepth--;
		if (this.#subagentWaitDepth !== 0 || this.#heldSteering.length === 0) return;
		const held = this.#heldSteering;
		this.#heldSteering = [];
		const text = [
			"User steering received while you were blocked waiting on subagents:",
			...held.map(message => message.text),
			"Subagent results in this wait batch are authoritative; integrate them before applying this steering.",
		].join("\n\n");
		void this.#queueUserMessage(
			text,
			held.flatMap(message => message.images ?? []),
			"steer",
		);
	}
	...
	#canAutoContinueForFollowUp(): boolean {
		...
		// Follow-up-only resume may also start from an empty reconstructed context:
		// Agent.continue() dequeues the follow-up as the opening turn in that case.
		const last = this.agent.state.messages.at(-1);
		return last === undefined || last.role === "assistant" || last.role === "toolResult";
	}
	...
	async #dispatchCustomMessage<T = unknown>(
		...
		if (this.isStreaming) {
			...
			if (options?.deliverAs === "followUp") {
				...
				this.agent.steer(normalizedAppMessage, {
					interruptToolExecution: options?.interruptToolExecution,
				});
			}
			this.#scheduleIdleQueueDrain();
	 ...
	 *
	 * Resolves after queue acceptance, not model completion.
	 */
	...
	async newSession(options?: NewSessionOptions): Promise<boolean> {
		...
		try {
			...
			this.#clearSessionScopedToolState();
			await this.#restoreMCPToolSelectionFromSessionContext();
			if (this.#tools.isMCPDiscoveryEnabled()) {
				this.sessionManager.appendMCPToolSelection(this.#tools.getSelectedMCPToolNames());
			}
			this.#clearCheckpointRuntimeState();
			this.setTodoPhases([]);
	...
	async #checkQaCompletion(): Promise<boolean> {
		if (this.#duoOrchestrator.orchestratorModeState?.enabled !== true) return false;
		const runningQaJobs =
		...
		if (this.#qaGateReminderSent) return false;
	...
	async switchSession(
		...
		try {
			...
			await this.#restoreMCPToolSelectionFromSessionContext(!switchingToDifferentSession);
			this.#reconnectToAgent();
			try {
				await this.#sessionSwitchReconciler?.();
			} catch (error) {
	...
	getContextBreakdown(options?: {
		...
		const breakdown = this.#stats.getContextBreakdown(options);
		if (!breakdown) return undefined;
		return { ...breakdown, usedTokens: this.#maintenance.estimateEffectiveContextTokens(breakdown.usedTokens) };
	}
	...
	async #retryInactiveAdvisorAfterModelDiscovery(): Promise<void> {
		...
		// Legacy SDK/test registries may not own online discovery. No refresh
		// capability means there is no deferred model population to retry.
		if (typeof this.#modelRegistry.awaitBackgroundRefresh !== "function") return;
	...
	/** Test-only access to the primary advisor runtime. */
	...
	/** Queue a non-blocking consult for the primary advisor. */
	consultAdvisorAsync(question: string): boolean {
		return this.#advisors.consultAsync(question);
	}
