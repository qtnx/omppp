...
import type { AutoCompactionReason, RecoveredRetryError } from "../extensibility/shared-events";
...
/** Hidden verification checklist steered into the run at the switch. */
const REASONING_SLIDE_CHECKLIST_MESSAGE_TYPE = "reasoning-slide-checklist";
/** Minimum visible text length for a post-nudge assistant turn to count as the delivered plan. */
...
/** Tools whose first successful call marks the execution phase. Bash is
 * excluded because it is also commonly used for exploration. */
 ...
 * assistant-turn boundary. Exactly one trigger must be configured.
 */
...
export interface AgentSessionConfig {
	...
	thinkingLevel?: ConfiguredThinkingLevel;
	/** Legacy one-way model downshift retained for existing SDK callers. */
...
function buildSessionMetadata(
	...
	if (provider === "anthropic") {
		...
		if (typeof accountUuid === "string" && accountUuid.length > 0) {
			...
			// identifier. Include both OMPx's persistent install id and the Claude
...
function getEditDestructiveIntent(args: unknown): { kind: "delete" | "move"; paths: string[] } | undefined {
	...
	if (input) {
		try {
			...
			let movePaths: string[] | undefined;
			for (const section of patch.sections) {
				const fileOp = section.fileOp;
				if (fileOp?.kind === "rem") return { kind: "delete", paths: [section.path] };
				if (fileOp?.kind === "move" && movePaths === undefined) movePaths = [section.path, fileOp.dest];
			}
			if (movePaths) return { kind: "move", paths: movePaths };
		} catch {
...
/** R7: pure guard composition for message-receipt planning takeover. */
export function resolveDuoPlanTakeoverDecision(
	...
	if (agentKind !== "main") return { request: false };
	if (duoStatus?.phase !== "executing") return { request: false };
	if (isStreaming) return { request: false };
	if (!signalsEnabled || !planningNeededEnabled) return { request: false };
	const detection = detectPlanningNeeded(expandedText);
	if (!detection.needed) return { request: false };
	return { request: true, reason: detection.evidence.join("; ") };
}
...
/** Result of the duo orchestrator ownership decision (mirror of the plan-mode guard). */
export interface DuoOrchestratorOwnershipDecision {
	/** Which orchestrator state transition to apply, if any. */
	apply: "enable" | "disable" | undefined;
	/** The new `#duoOwnsOrchestrator` flag value. */
	owns: boolean;
}

/**
 * Decide how the duo host adapter should react to a `setOrchestratorEnabled`
 * request without ever tearing down a user-owned orchestrator session.
 * Ownership is granted ONLY on a duo-driven off->on flip; a no-op (the user
 * already had orchestrator on) never adopts, so a later single-scope disable is
 * refused and live mode stays orchestrator (per the live-status amendment).
 */
export function resolveDuoOrchestratorOwnership(
	...
	// requested === false, current === true: only release what duo owns.
...
export class AgentSession {
	...
	// Task5: mirror #duoOwnsPlanMode so duo never tears down a user-owned orchestrator session.
	...
	// Keep the original bus failure hook with each queued record; a stranded-aside wake can fail
	// after the initial send has already reported delivery, so it still needs redelivery.
	#pendingIrcInterrupts: { record: CustomMessage; onWakeFailure?: (message: IrcMessage) => void }[] = [];
	#pendingIrcAsides: { record: CustomMessage; onWakeFailure?: (message: IrcMessage) => void }[] = [];
	...
	#mcpDiscoveryEnableInFlight: Promise<void> | undefined;
	...
	#resumeStrandedIrcAsides(): void {
		...
		const pending = [...this.#pendingIrcInterrupts, ...this.#pendingIrcAsides];
		...
		// Preserve each record's bus callback across the batched stranded-aside wake
		// so a failed wake requeues only the exact IRC messages it did not answer.
		this.#wakeForIrc(records, {
			onWakeFailure: message => {
				for (const entry of pending) {
					const details = entry.record.details as { id?: string } | undefined;
					if (details?.id === message.id) entry.onWakeFailure?.(message);
				}
			},
		});
	}
	 ...
	 *  re-drains anything that stranded during it. Queue work is parked across the wake so success
	 *  and failure are decided from the IRC wake response only; otherwise a queued follow-up in the
	 *  same loop can mask an empty wake response or make a completed wake look failed. */
	#wakeForIrc(records: CustomMessage[], opts?: { onWakeFailure?: (message: IrcMessage) => void }): void {
		// Park queued work so the wake outcome is classified only from the IRC wake turn.
		// Follow-ups/steers can otherwise ride the same agent loop and make an empty IRC
		// response look successful (or make a successful IRC response look failed).
		const parkedSteering = [...this.agent.peekSteeringQueue()];
		const parkedFollowUps = [...this.agent.peekFollowUpQueue()];
		if (parkedSteering.length > 0 || parkedFollowUps.length > 0) {
			this.agent.replaceQueues([], []);
		}
		this.#resetPromptMaintenanceState();
		// Provider/request failures may be swallowed by Agent.#runLoop into
		// agent.state.error while the prompt promise still resolves, so success
		// requires observing a completed non-error assistant response, not just a
		// fulfilled promise.
		let completedWakeResponse = false;
		const unsubscribe = this.agent.subscribe(event => {
			if (event.type !== "agent_end") return;
			completedWakeResponse ||= event.messages.some(message => {
		...
		// Reconstruct from irc:incoming metadata rather than rendered prompt text;
		// this preserves the exact recipient/body/reply target for bus redelivery.
		const redeliver = (): void => {
			if (!opts?.onWakeFailure) return;
			for (const record of records) {
				const details = record.details as
					| { id?: string; from?: string; to?: string; message?: string; replyTo?: string }
					| undefined;
				if (!details?.id || !details.from || !details.to || details.message === undefined) continue;
				opts.onWakeFailure({
					id: details.id,
					from: details.from,
					to: details.to,
		...
		void this.agent
			...
			.catch(error => {
				wakeFailed = true;
				logger.error("IRC wake turn failed", { error: String(error) });
				redeliver();
			})
			.then(() => {
				if (wakeFailed) return;
				// A fulfilled prompt with state.error (or no completed assistant output)
				// is still an undelivered wake: log it and redeliver so the IRC message
				// is never lost behind the earlier "woken" receipt.
				if (!completedWakeResponse || this.agent.state.error) {
					logger.error("IRC wake turn produced no completed response", {
						error: this.agent.state.error,
						messageIds: records
							.map(record => (record.details as { id?: string } | undefined)?.id)
			...
			.finally(() => {
				...
				if (parkedSteering.length > 0 || parkedFollowUps.length > 0) {
					this.agent.replaceQueues(
						[...parkedSteering, ...this.agent.peekSteeringQueue()],
	...
	#scrubReasoningSlidePlanNudge(liveMessages: AgentMessage[]): void {
		...
		const isPlanNudge = (message: AgentMessage): boolean =>
			message.role === "custom" && message.customType === REASONING_SLIDE_PLAN_MESSAGE_TYPE;
	...
	async #advancePrewalk(liveMessages: AgentMessage[], context: AgentTurnEndContext | undefined): Promise<void> {
		...
		} else if (this.#prewalkContinuePending && !isAwaitingUserAnswer(context.message)) {
	...
	constructor(config: AgentSessionConfig) {
		...
		this.agent.setOnTurnEnd(async (messages, signal, context) => {
			...
			try {
				...
				logger.error("duo takeover-signals evaluation failed", { err });
			}
			await this.#duoController?.notifyTurnEnd();
		...
		this.agent.setAsideMessageProvider(() => {
			...
			this.#pendingIrcInterrupts = [];
			this.#pendingIrcAsides = [];
			const thunks: AsideMessage[] = pendingIrc.map(entry => () => entry.record);
	...
	#buildDuoHost(): DuoControllerHost {
		return {
			...
			resumeAdvisor: () => {
				...
				// delta feed.
				for (const advisor of this.#advisors) advisor.runtime.resume();
			},
			...
			setOrchestratorEnabled: async enabled => {
				...
				// session-wiring seam) so duo never tears down a user-owned orchestrator.
				const decision = resolveDuoOrchestratorOwnership(
					enabled,
					this.getOrchestratorModeState()?.enabled === true,
	 ...
	 *  Done claims are handled by the advisor done gate and QA requirement, not
	 *  by takeover signals. */
	#evaluateDuoTakeoverSignals(): void {
		const controller = this.#duoController;
		if (controller?.status.phase !== "executing") return;
	...
	#parseDuoSnapshot(value: unknown): DuoStateSnapshot | undefined {
		...
		return {
			...
			duoOwnsAdvisor,
			takeoverPurpose:
				record.takeoverPurpose === "recover" || record.takeoverPurpose === "plan"
	...
	#resolveAdvisorRuntimeDescriptors(emitWarnings: boolean): AdvisorRuntimeDescriptor[] {
		...
		for (const config of roster) {
			...
			// Resolve the advisor's model. Duo advisor pinning deliberately overrides
			// normal advisor selection while Fable/Opus handoff mode owns the advisor;
			// otherwise an explicit `model` override wins, then the `advisor` role chain.
	...
	#resolveEffectiveAdvisorRuntimeDescriptors(emitWarnings: boolean): AdvisorRuntimeDescriptor[] {
		...
		for (const descriptor of descriptors) {
			...
			descriptor.thinkingLevel = this.#duoPinnedAdvisorThinkingLevel(
				descriptor.thinkingLevel ?? ThinkingLevel.Medium,
			);
	...
	#buildAdvisorRuntime(seedToCurrent = false): boolean {
		...
		const advisorFallbackModel = fallbackSelection.model;
		if (!advisorFallbackModel) {
			logger.debug("advisor fallback model unavailable; refusal fallback disabled", {
				value: fallbackSetting,
				warning: fallbackSelection.warning,
			});
		}
		...
		for (const descriptor of descriptors) {
			...
			const duoTakeoverTools = this.#duoController
				? [
			...
			const runtime = new AdvisorRuntime(
				advisorAgentFacade,
				{
					...
					onTurnError: async (error, failedModel, failedMessages) => {
						if (await this.#recoverAdvisorTurn(advisorRef, error, failedMessages)) return true;
						// The in-stream retry rotates sibling credentials within a request
						// but does not block the final exhausted credential. Mark only
						// usage-limit outcomes; transient advisor failures must retain the
						// normal retry path.
						const message = error instanceof Error ? error.message : String(error);
						if (!isUsageLimitOutcome(extractHttpStatusFromError(error), message)) return false;
		...
		if (
			this.#toolSession?.restrictToolNames !== true &&
			this.#advisors.length > 0 &&
			this.settings.get("advisor.consult") !== false &&
			!this.#toolRegistry.has("consult")
		) {
			const toolSession = this.#toolSession;
			if (toolSession) {
				// The advisor can attach after createTools' one-time registry build, so add consult live.
				const consultTool = wrapToolWithMetaNotice(new ConsultTool(toolSession));
				this.#toolRegistry.set(consultTool.name, consultTool);
				this.#builtInToolNames.add(consultTool.name);
				const activeToolNames = [...new Set([...this.getEnabledToolNames(), ...this.#liveDuoToolNames()])];
				void this.#applyActiveToolsByName(activeToolNames, {
					respectRequestedToolNames: false,
					preserveMCPSelection: true,
				}).catch(error => {
					logger.warn("Failed to activate live advisor tools", {
	...
	async #checkAdvisorDoneGate(finalMessage: AssistantMessage): Promise<boolean> {
		...
		const advisorAvailable = advisor !== undefined && !advisor.runtime.disposed;
		const duoStatus = this.#duoController?.status;
		...
		const passiveCanRun = advisorAvailable && this.#advisorDoneGateRejections < 2 && doneGateEnabled;
		...
		// Capture the generation NOW: a late reject or QA requirement must schedule
		// the continuation against this prompt turn, never leak into a subsequent user prompt.
		const generation = this.#promptGeneration;

		if (passiveCanRun) {
			if (!advisor || advisor.runtime.disposed) return false;
			...
			try {
				const consultPromise = advisor.runtime.consult(doneReviewMd, {
					timeoutMs: 90_000,
	...
	async #maybeRestoreAdvisorRetryFallbackPrimary(advisor: ActiveAdvisor): Promise<void> {
		...
		const thinkingToApply =
			advisor.thinkingLevel === fallback.lastAppliedThinkingLevel
				...
				: (advisor.thinkingLevel ?? ThinkingLevel.Inherit);
	...
	async #recoverAdvisorTurn(
		...
		for (const selector of this.#findRetryFallbackCandidates(role, currentSelector, currentModel)) {
			...
			const originalThinkingLevel = advisor.thinkingLevel ?? ThinkingLevel.Inherit;
	...
	async #promoteAdvisorContextModel(advisor: ActiveAdvisor, currentModel: Model): Promise<boolean> {
		...
		const advisorThinkingLevel = advisor.thinkingLevel ?? ThinkingLevel.Inherit;
		try {
	...
	#processAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (event.type === "agent_end") {
			...
			if (msg.stopReason === "aborted") {
				...
				// A deliberate abort voids any agent-requested compaction scheduled mid-turn;
	 ...
	 * `device_id` is derived from both the persistent OMPx install id and that
	...
	#isRequestedToolAllowed(name: string): boolean {
		...
		if (normalized === TOOL_DISCOVERY_SEARCH_TOOL_NAME) {
			return this.#mcpDiscoveryEnabling || this.isToolDiscoveryEnabled();
		}
		...
		if (
			...
			(normalized === "write" &&
				this.#builtInToolNames.has("write") &&
				(this.#mountedXdevToolNames.size > 0 || this.#planModeState?.enabled === true)) ||
	...
	#collectDiscoverableMCPToolsFromRegistry(): Map<string, DiscoverableTool> {
		// The explicit toolNames allowlist pins MCP selection only when it names MCP
		// tools directly. An allowlist with no MCP entries (e.g. ["read","edit","bash"])
		// does not restrict what discovery can surface, so deferred MCP discovery still
		// exposes the full toolset; an allowlist that names MCP tools keeps unrequested
		// MCP tools out (OMPx explicit-allowlist contract).
		const allowlistPinsMCPTools =
			this.#requestedToolNames !== undefined && [...this.#requestedToolNames].some(isMCPToolName);
		const mcpTools = filterBySource(collectDiscoverableTools(this.#toolRegistry.values()), "mcp").filter(
			tool => !allowlistPinsMCPTools || this.#isRequestedToolAllowed(tool.name),
		);
	...
	/**
	 * Reconcile discovery-mode MCP selections with the branch being restored.
	 * Historical branches without selection metadata use their captured session
	 * default instead of inheriting selections from the branch left behind.
	 */
	async #restoreMCPSelectionsForSessionContext(
		...
		await this.#applyActiveToolsByName([...this.#getActiveNonMCPToolNames(), ...restoredMCPToolNames], {
	...
	/** Live duo/advisor tools must survive allowlists and mode transitions once their runtimes exist. */
	#liveDuoToolNames(): string[] {
		const names: string[] = [];
		const phase = this.#duoController?.status.phase;
		if (phase !== undefined && phase !== "inactive" && phase !== "suspended") {
			for (const name of ["duo_handoff", "duo_escalate"]) {
				if (this.#toolRegistry.has(name)) names.push(name);
			}
		}
		if (
			(this.isAdvisorActive() || this.#advisorEnabled) &&
			this.settings.get("advisor.consult") !== false &&
			this.#toolRegistry.has("consult")
		) {
			names.push("consult");
	...
	async deactivateVibeTools(nextToolNames: string[]): Promise<void> {
		...
		await this.#applyActiveToolsByName(nextToolNames, { preserveMCPSelection: true });
	}
	...
	async #reconcileActiveToolsForDiscoveryMode(
		...
		const activeToolNames = this.getEnabledToolNames();
		...
		if (currentMode === "all" && this.#toolRegistry.has("search_tool_bm25")) {
			...
			nextActiveToolNames = filterInitialToolsForDiscoveryAll(activeNamesWithDiscovery, {
				loadModeOf: name => (this.#builtInToolNames.has(name) ? this.#toolRegistry.get(name)?.loadMode : undefined),
	...
	async enableMCPDiscoveryWithSearchTool(): Promise<void> {
		if (this.#mcpDiscoveryEnableInFlight) {
			await this.#mcpDiscoveryEnableInFlight;
			return;
		}
		const inFlight = this.#runMCPToolMutation(() => this.#enableMCPDiscoveryWithSearchTool());
		this.#mcpDiscoveryEnableInFlight = inFlight;
		try {
			await inFlight;
		} finally {
			if (this.#mcpDiscoveryEnableInFlight === inFlight) {
				this.#mcpDiscoveryEnableInFlight = undefined;
			}
		}
	}

	async #enableMCPDiscoveryWithSearchTool(): Promise<void> {
		if (!this.#toolRegistry.has(TOOL_DISCOVERY_SEARCH_TOOL_NAME)) {
			this.#mcpDiscoveryEnabled = false;
			return;
		}
		const activeToolNames = [...new Set([...this.getActiveToolNames(), ...this.#mountedXdevToolNames])];
		if (activeToolNames.includes(TOOL_DISCOVERY_SEARCH_TOOL_NAME)) {
	...
	isToolDiscoveryEnabled(): boolean {
		return (
			this.#mcpDiscoveryEnabled || (!this.#mcpDiscoveryEnabling && this.#resolveEffectiveDiscoveryMode() !== "off")
		);
	}
	...
	async #applyActiveToolsByName(
		...
		// Auto-QA survives runtime mutations except modes with an exact tool contract.
		if (
		...
		const nextSelectedMCPToolNames = options?.preserveMCPSelection
			? new Set(previousSelectedMCPToolNames)
			: this.#mcpDiscoveryEnabled || this.#mcpDiscoveryEnabling
				? new Set(
						selectedTools
							.map(({ name }) => name)
							.filter(
		...
		const nextSelectedDiscoveredToolNames = new Set(this.#selectedDiscoveredToolNames);
		for (const name of nextSelectedDiscoveredToolNames) {
			if (!activeNameSet.has(name) || isMCPToolName(name) || !this.#toolRegistry.has(name)) {
				nextSelectedDiscoveredToolNames.delete(name);
			}
		}
		...
		if (nextSelectedMCPToolNames) this.#selectedMCPToolNames = nextSelectedMCPToolNames;
	...
	/** Standing handler the resolve tool uses when no queued invocation owns the action. */
	...
	/**
	 * Rediscover the SSH capability and reconcile its active registration without
	...
	async refreshSshTool(options?: { activateIfAvailable?: boolean }): Promise<void> {
		...
		const previousEnabledToolNames = this.getEnabledToolNames();
		const wasActive = previousEnabledToolNames.includes("ssh");
		...
		const nextActive = previousEnabledToolNames.filter(name => name !== "ssh" && this.#toolRegistry.has(name));
		if (refreshedTool && sshAllowed && (wasActive || (options?.activateIfAvailable && !previousSshTool))) {
			nextActive.push(refreshedTool.name);
		}
		await this.#applyActiveToolsByName(nextActive, { preserveMCPSelection: true });
	}
	...
	#baseSystemPromptWithModeOverlay(): string[] {
		...
		const duoOverlayVars = {
			...
			// LIVE orchestrator state (actual mode wins over declared scope): the executor
			// overlay renders direct-mode text when duo turned orchestrator off for a single-phase task.
			orchestrator: this.#orchestratorModeState?.enabled === true,
		};
	...
	/**
	 * Replace MCP tools in the registry and refresh the active tool set. Outside
	 * discovery mode, a full prior enabled set admits newly connected tools; a
	 * narrowed set stays narrowed. Discovery selections stay explicit unless
	 * `activateAll` is requested. Changes take effect without restarting.
	 */
	async refreshMCPTools(mcpTools: CustomTool[], options?: { activateAll?: boolean }): Promise<void> {
		await this.#runMCPToolMutation(() => this.#refreshMCPTools(mcpTools, options));
	}
	...
	async #refreshMCPTools(mcpTools: CustomTool[], options?: { activateAll?: boolean }): Promise<void> {
		...
		if (!this.buildDisplaySessionContext().hasPersistedMCPToolSelection) {
			const sessionDefaults = this.#getSessionDefaultSelectedMCPToolNames(this.sessionFile);
			this.#selectedMCPToolNames = new Set([
				...this.#selectedMCPToolNames,
				...(sessionDefaults.length > 0 ? sessionDefaults : this.#getConfiguredDefaultSelectedMCPToolNames()),
			]);
		}

		const previousEnabledMCPToolNames = new Set(this.getEnabledToolNames().filter(isMCPToolName));
		const previouslyAllEnabled = [...previousMCPTools.keys()].every(name => previousEnabledMCPToolNames.has(name));
		...
		try {
			...
			await this.#applyActiveToolsByName([...this.#getActiveNonMCPToolNames(), ...this.getSelectedMCPToolNames()], {
				previousSelectedMCPToolNames,
			});
			this.#rememberSessionDefaultSelectedMCPToolNames(
				this.sessionFile,
	...
	async refreshRpcHostTools(rpcTools: AgentTool[]): Promise<void> {
		...
		try {
			await this.#applyActiveToolsByName(
				...
				{
					respectRequestedToolNames: this.#toolSession?.restrictToolNames === true,
					preserveMCPSelection: true,
				},
			);
		} catch (error) {
	...
	async setOrchestratorModeState(
		...
		if (state?.enabled) {
			...
			if (!reuseRestoreSnapshot) {
				...
				this.#orchestratorModePreviousToolNames = this.getEnabledToolNames();
			}
	...
	/** R7: automatic duo planning takeover when an incoming user message is plan-shaped. */
	async #maybeDuoPlanTakeover(expandedText: string): Promise<void> {
		const decision = resolveDuoPlanTakeoverDecision(
			...
			this.isStreaming,
			this.settings.get("duo.takeover.signals.enabled"),
	...
	#queueHiddenNextTurnMessage(message: CustomMessage, triggerTurn: boolean): void {
		...
		if (triggerTurn) this.#triggerHiddenNextTurnDelivery();
	}

	#triggerHiddenNextTurnDelivery(): void {
		const generation = this.#promptGeneration;
	...
	async #promptAgentInitiatedMessage(
		...
		this.#beginInFlight();
		try {
			this.#prePersistPromptMessages([message]);
	...
	async sendCustomMessage<T = unknown>(
		...
		if (options?.deliverAs === "nextTurn") {
			...
			normalizedAppMessage.entryId = this.sessionManager.appendCustomMessageEntry(
	...
	async newSession(options?: NewSessionOptions): Promise<boolean> {
		...
		if (this.#mcpDiscoveryEnabled) {
			await this.#applyActiveToolsByName(
				[...this.#getActiveNonMCPToolNames(), ...this.#getConfiguredDefaultSelectedMCPToolNames()],
				{ persistMCPSelection: false },
			);
	...
	considerCompactionWhileWaiting(reason: string, options?: { focus?: string }): ToolWaitingCompactionCheck {
		...
		if (contextWindow <= 0) {
			return { status: "unavailable", detail: "current model has no context window" };
		}
	...
	#estimatePendingPromptProviderBytes(messages: AgentMessage[], maxPayloadBytes = 0): number | undefined {
		try {
			...
			const providerSystemPrompt = Array.isArray(systemPrompt)
				? systemPrompt.map(prompt => this.#obfuscateTextForProvider(prompt) ?? prompt)
				: [this.#obfuscateTextForProvider(systemPrompt) ?? systemPrompt];
	...
	async #maintainContextMidRun(
		...
		// Waiting job polls schedule this request so the safe mid-run boundary
		// compacts before large subagent results are injected into the next prompt.
		...
		const wantsCompaction = shouldCompact(contextTokens, contextWindow, compactionSettings);
		if (!wantsCompaction && !agentRequested) return;

		if (agentRequested) {
			// Consume only after every guard above passed; #checkCompaction then sees
			// no duplicate request while guarded/skipped runs retain the pending one.
			this.#pendingAgentCompactionRequest = undefined;
		}
		...
		await this.#runAutoCompaction(agentRequested ? "requested" : "threshold", false, false, false, {
			autoContinue: false,
	...
	async #checkCompaction(
		...
		if (sameModel && !errorIsFromBeforeCompaction && AIError.isContextOverflow(assistantMessage, contextWindow)) {
			...
			if (compactionEnabled) {
				const outcome = await this.#runRecoveryCompactionWithRollback("overflow", assistantMessage, allowDefer, {
					autoContinue,
				});
			...
			// Compaction is disabled or was skipped/cancelled without rewriting history;
			// fall back to promotion so an overflow with no compactable candidates can
			// still retry on a larger-context model.
		...
		if (!compactionEnabled) return COMPACTION_CHECK_NONE;
	...
	#appendBashMessage(destination: BashAppendDestination, message: BashExecutionMessage): void {
		switch (destination.kind) {
			case "current": {
				// Persist a clone first, then stamp the runtime-only entry id onto the live message so
				// context extensions can disambiguate this result without writing the field to disk.
				const entryId = destination.manager.appendMessage({ ...message });
	...
	drainPendingIrcInboxMessages(agentId: string, opts?: { from?: string; limit?: number }): IrcMessage[] {
		...
		const remainingInterrupts: { record: CustomMessage; onWakeFailure?: (message: IrcMessage) => void }[] = [];
		const remainingAsides: { record: CustomMessage; onWakeFailure?: (message: IrcMessage) => void }[] = [];
		const queues = [
			{ entries: this.#pendingIrcInterrupts, remaining: remainingInterrupts },
			{ entries: this.#pendingIrcAsides, remaining: remainingAsides },
		];
		for (const queue of queues) {
			for (const entry of queue.entries) {
				const { record } = entry;
				if (record.customType !== "irc:incoming") {
					queue.remaining.push(entry);
					continue;
				}
	...
	async deliverIrcMessage(
		msg: IrcMessage,
		opts?: { expectsReply?: boolean; onWakeFailure?: (message: IrcMessage) => void },
		...
		const record: CustomMessage = {
			...
			display: true,
			// Store routing fields with the displayed irc:incoming record so a later
			// failed wake can rebuild the exact bus message without parsing content.
			details: {
				id: msg.id,
				from: msg.from,
				to: msg.to,
		...
		if (this.isStreaming) {
			...
			if (recipientParentId === msg.from) {
				...
				this.#pendingIrcInterrupts.push({ record, onWakeFailure: opts?.onWakeFailure });
			}
		...
		this.#wakeForIrc([record], { onWakeFailure: opts?.onWakeFailure });
		return "woken";
	}
	...
	async navigateTree(
		...
		await this.#restoreMCPSelectionsForSessionContext(stateContext);
	...
	getContextBreakdown(options?: {
		...
		let lastUsageIndexForAdjustment: number | null = null;
		if (useAnchor && anchorAssistant) {
			lastUsageIndexForAdjustment = resolvedAnchorIndex;
			const promptTokens =
			...
			lastUsageIndexForAdjustment = pending.cutoffCount - 1;
			anchored = true;
			let tailTokens = 0;
		...
		const effectiveUsedTokens = this.#estimateEffectiveContextTokens(usedTokens, lastUsageIndexForAdjustment);
		const messagesTokens = Math.max(0, effectiveUsedTokens - categoryNonMessageTokens);
	...
	async duoHandoffToExecutor(resolution: string, scope?: DuoExecutionScope): Promise<DuoHandoffResult> {
		return (await this.#duoController?.handoffToExecutor(resolution, scope)) ?? "no-controller";
	}
	...
	/**
	 * Lightweight advisor status for the status line without computing token or
	 * cost breakdowns on every render frame.
	 */
