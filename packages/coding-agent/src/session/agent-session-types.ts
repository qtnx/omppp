...
import type { ReasoningSlide } from "./model-controls";
import type { SessionManager } from "./session-manager";

import type { SystemPromptRebuildContext } from "./session-tools";
...
export interface AgentSessionConfig {
	...
	/** Returns the local calendar date used in system-prompt reminders. */
	...
	/** Reloads the capability-derived SSH tool after credential/config changes. */
	reloadSshTool?: () => Promise<AgentTool | null>;
	/** Restricts activation to a preselected tool set when present. */
	requestedToolNames?: ReadonlySet<string>;
	/** Tool names explicitly requested by the caller and pinned during selection. */
	explicitlyRequestedToolNames?: ReadonlySet<string>;
	/** Resolved tool-discovery mode after applying the auto policy. */
	effectiveDiscoveryMode?: EffectiveToolDiscoveryMode;
	/** Whether MCP tools begin in discovery mode. */
	mcpDiscoveryEnabled?: boolean;
	/** Whether MCP is enabled for this session. */
	mcpEnabled?: boolean;
	/** MCP tools selected from a restored session snapshot. */
	...
	/** Context-GC SQLite database path supplied by the SDK for this session. */
...
/**
 * Whether the persisted conversation ends with model-visible work awaiting an
 * assistant response. Non-turn-bearing custom/developer messages are ignored.
 */
