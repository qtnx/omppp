...
function resolveGlobScopePattern(
	...
	if (maxSuffix.level !== undefined) {
		...
		const baseMatches = matchingGlobModels(maxSuffix.base, availableModels);
		let models = baseMatches;
		if (options?.preserveLiteralMaxSuffix === false) {
			const literalMatches = new Set(matchingGlobModels(pattern, availableModels));
			models = baseMatches.filter(
				model =>
					literalMatches.has(model) ||
					splitThinkingSuffix(model.id, -1, MAX_THINKING_SUFFIX_OPTIONS).level === undefined,
			);
		}
		return { models, thinkingLevel, explicitThinkingLevel: thinkingLevel !== undefined };
	}
...
function matchModel(
	...
	options?: { modelRegistry?: CanonicalModelRegistry; exactOnly?: boolean },
): Model<Api> | undefined {
...
export function selectHeadroomAwareModelPatterns(
	...
	for (const pattern of patterns) {
		try {
			...
			if (!model || deps.authStorage.usage.headroom(model, { utilizationMax, windowMode }).hasRoom) {
...
export function extractExplicitThinkingSelector(
	...
	while (!visited.has(current)) {
		...
		const strictSelector = isLiteralModelSelector(current, options)
			? undefined
			: splitThinkingSuffix(current, rolePrefixLength).level;
		if (strictSelector) {
...
function resolveExactCanonicalScopePattern(
	pattern: string,
	modelRegistry: Required<Pick<CanonicalModelRegistry, "getCanonicalVariants">>,
	availableModels: Model<Api>[],
	...
	const variants = modelRegistry
		...
		.map((variant: { model: Model<Api> }) => variant.model);
	if (variants.length === 0) {
		return undefined;
	}

	return { models: variants, thinkingLevel: concreteThinkingLevel(thinkingLevel), explicitThinkingLevel };
}
...
export async function resolveModelScope(
	...
	const canonicalRegistry =
		typeof modelRegistry.getCanonicalVariants === "function"
			? ({ getCanonicalVariants: modelRegistry.getCanonicalVariants.bind(modelRegistry) } satisfies Required<
					Pick<CanonicalModelRegistry, "getCanonicalVariants">
				>)
			: undefined;
	...
	for (const pattern of patterns) {
		...
		const canonicalScope = canonicalRegistry
			? resolveExactCanonicalScopePattern(pattern, canonicalRegistry, availableModels)
			: undefined;
		if (canonicalScope) {
			for (const model of canonicalScope.models) {
				addScopedModel(model, canonicalScope.thinkingLevel, canonicalScope.explicitThinkingLevel);
			}
			continue;
		}

		// Role aliases resolve to the role's concrete model, not its fallback chain.
 ...
 */
function hasCanonicalModelRegistry(value: unknown): value is Partial<CanonicalModelRegistry> {
	return (
		typeof value === "object" &&
		value !== null &&
		"getCanonicalVariants" in value &&
		typeof value.getCanonicalVariants === "function"
	);
}
...
export function filterAvailableModelsByEnabledPatterns(
	available: Model<Api>[],
	patterns: readonly string[],
	modelRegistryOrSettings?: Partial<CanonicalModelRegistry> | Settings,
	settings?: Settings,
): Model<Api>[] {
	const modelRegistry = hasCanonicalModelRegistry(modelRegistryOrSettings) ? modelRegistryOrSettings : undefined;
	const effectiveSettings: Settings | undefined = modelRegistry
		? settings
		: (modelRegistryOrSettings as Settings | undefined);
	...
	for (const pattern of patterns) {
		if (pattern.includes("*") || pattern.includes("?") || pattern.includes("[")) {
			for (const model of resolveGlobScopePattern(pattern, available, {
				preserveLiteralMaxSuffix: canonicalRegistry === undefined,
			}).models) {
				addAllowed(model);
			}
			continue;
		...
		const exactCanonical = canonicalRegistry
			? resolveExactCanonicalScopePattern(pattern, canonicalRegistry, available)
			: undefined;
		if (exactCanonical) {
		...
		if (effectiveSettings && modelRoleAliasPrefixLength(pattern) !== undefined) {
			const { model } = resolveModelRoleValue(pattern, available, { settings: effectiveSettings });
			if (model) addAllowed(model);
			continue;
		}

		const { model } = parseModelPatternWithContext(pattern, available, context, { modelRegistry });
		if (model) {
			addAllowed(model);
		}
	}
