...
interface ModelRegistryOptions {
	...
	settings?: Settings;
	/** SQLite cache location; defaults beside an explicit models config. */
	cacheDbPath?: string;
	/**
...
export class ModelRegistry {
	...
	#loadBuiltInModels(overrides: Map<string, ProviderOverride>, providerFilter?: ReadonlySet<string>): Model<Api>[] {
		...
		if (providerFilter && !providerFilter.has("tnx")) return bundledModels;
	 ...
	 * (`authStorage.keys.source`) and the disabled-provider set are resolved once
	 ...
	 * tokens are NOT refreshed (`authStorage.keys.source`). This is what keeps the
	...
	hasProvider(providerId: string): boolean {
		...
		if (this.#disabledProviderIds().has(providerId)) return false;
		return (
