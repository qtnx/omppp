...
describe("InteractiveMode resume mode restoration", () => {
	...
	async function writeLoopSession(
		name: string,
		defaultModel: Model<Api>,
		options: { promptFilePath?: string; intervalMs?: number; paused?: boolean } = {},
	): Promise<string> {
		const timestamp = "2026-06-01T00:00:00.000Z";
		const defaultEntryId = `${name}-default-model`;
		return await writeSessionFile(name, [
			...
			},
			{
				...
				mode: "loop",
				data: {
					promptFilePath: options.promptFilePath ?? "local://LOOP_PROMPT.md",
	...
	it("restores active loop mode and prompt file from the active session during init", async () => {
		const registry = modelRegistry();
		const defaultModel = modelOrThrow(registry, "claude-sonnet-4-5");
		const loopSessionFile = await writeLoopSession("loop", defaultModel, { intervalMs: 1 });
		const created = await createHarness({ sessionFile: loopSessionFile, initialModel: defaultModel });
		const loopPromptPath = resolveLocalUrlToPath("local://LOOP_PROMPT.md", {
			getArtifactsDir: () => created.session.sessionManager.getArtifactsDir(),
			getSessionId: () => created.session.sessionManager.getSessionId(),
		});
		await Bun.write(loopPromptPath, "restored loop prompt");

		await created.mode.init({ suppressWelcomeIntro: true });

		expect(created.mode.loopModeEnabled).toBe(true);
		expect(created.mode.loopPromptFilePath).toBe("local://LOOP_PROMPT.md");
		expect(created.mode.loopPrompt).toBe("restored loop prompt");
		Object.defineProperty(created.session, "isCompacting", { configurable: true, get: () => false });
		Object.defineProperty(created.session, "isStreaming", { configurable: true, get: () => false });
		Object.defineProperty(created.session, "hasPostPromptWork", { configurable: true, get: () => false });
		expect(created.mode.loopRuntime?.intervalMs).toBe(1);

		const input = await created.mode.getUserInput();

		expect(input.text).toBe("restored loop prompt");
	});

	it("does not restore a loop prompt after loop mode was paused", async () => {
		const registry = modelRegistry();
		const defaultModel = modelOrThrow(registry, "claude-sonnet-4-5");
		const loopSessionFile = await writeLoopSession("paused-loop", defaultModel, { paused: true });
		const created = await createHarness({ sessionFile: loopSessionFile, initialModel: defaultModel });
		const loopPromptPath = resolveLocalUrlToPath("local://LOOP_PROMPT.md", {
			getArtifactsDir: () => created.session.sessionManager.getArtifactsDir(),
			getSessionId: () => created.session.sessionManager.getSessionId(),
		});
		await Bun.write(loopPromptPath, "old loop prompt");

		await created.mode.init({ suppressWelcomeIntro: true });

		expect(created.mode.loopModeEnabled).toBe(false);
		expect(created.mode.loopPromptFilePath).toBeUndefined();
		expect(created.mode.loopPrompt).toBeUndefined();
	});
});
