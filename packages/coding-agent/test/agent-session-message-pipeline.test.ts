...
describe("AgentSession message pipeline", () => {
	...
	it("injects a single workflow-tool notice for workflow requests", async () => {
		...
		const model = {
			...
			maxTokens: 1024,
			compat: undefined,
			identity: classifyModel("test-provider", "workflow-model", { lenient: true }),
		} satisfies Model;
	...
	it("reports Context GC projected tokens in context usage", async () => {
		...
		try {
			...
			const model = {
				...
				maxTokens: 4_096,
				compat: undefined,
				identity: classifyModel("anthropic", "test-model", { lenient: true }),
			} satisfies Model;
	...
	it("rejects an aborted enqueue after usage preflight without mutating either queue", async () => {
		const model = {
			...
			maxTokens: 1024,
			compat: undefined,
			identity: classifyModel("test-provider", "enqueue-preflight-model", { lenient: true }),
		} satisfies Model;
	...
	it("keeps parent memory once and places fresh fork recall before the new user turn", async () => {
		...
		const fakeBackend: MemoryBackend = {
			...
			async beforeAgentStartPrompt() {
				if (remembered) return undefined;
				return {
					context: contexts.length === 0 ? parentMemory : forkMemory,
					commit: () => {
						remembered = true;
						return true;
					},
