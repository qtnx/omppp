...
function createToolSession(mcpManager?: MCPManager): ToolSession {
	return {
		cwd: os.tmpdir(),
		hasUI: false,
...
describe("McpProtocolHandler", () => {
	...
	it("read tool resolves mcp:// URLs only through the caller-scoped MCP manager", async () => {
		...
		await expect(
			new ReadTool(createToolSession()).execute("read-restricted", { path: "mcp://test://doc" }),
		).rejects.toThrow("No MCP manager");
		const result = await new ReadTool(createToolSession(scopedManager)).execute("read-unrestricted", {
			path: "mcp://test://doc",
		});
	...
	it("preserves a literal semicolon in an exact MCP resource URI", async () => {
		...
		const result = await new ReadTool(createToolSession(manager)).execute("read-semicolon-resource", {
			path: `mcp://${uri}`,
		});
	...
	it("lets read consume a native URI advertised by an MCP server", async () => {
		...
		const result = await new ReadTool(createToolSession(manager)).execute("read-ags-resource", {
	...
	it("waits for the MCP resource catalog before rejecting a native URI", async () => {
		...
		const result = await new ReadTool(createToolSession(manager)).execute("read-delayed-ags-resource", {
			path: "ags://capabilities/current-host",
		});
		const output = result.content.find(block => block.type === "text");
	...
	it("resolves an opaque resource URI advertised by an MCP server", async () => {
		...
		const result = await new ReadTool(createToolSession(manager)).execute("read-opaque-resource", {
