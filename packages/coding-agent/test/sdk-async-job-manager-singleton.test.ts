...
describe("AsyncJobManager singleton across concurrent top-level sessions", () => {
	...
	async function spawnTopLevelSession(
		...
		optionsOrExtensions?: { parentTaskPrefix?: string; toolNames?: string[] } | ExtensionFactory[],
	) {
		const options = Array.isArray(optionsOrExtensions) ? undefined : optionsOrExtensions;
		const extensions = Array.isArray(optionsOrExtensions) ? optionsOrExtensions : [];
