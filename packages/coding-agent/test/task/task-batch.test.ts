...
describe("task.batch validation", () => {
	...
	it("requires shared context for agent-centric batch calls", async () => {
		const text = await executeText({ tasks: [{ task: "Work." }] }, { "task.batch": true });
		expect(text).toContain("Missing `context`");
	});

	it("accepts fork-legacy batches without shared context", async () => {
		let capturedContext: string | undefined = "not-called";
		...
		expect(text).not.toContain("Missing `context`");
...
describe("task.batch spawning", () => {
	...
	it("spawns one background job per task item with shared context, runtime caps, and independent schemas", async () => {
		mockDiscovery({
			...taskAgent,
		...
		const result = await tool.execute("tc-batch", {
			context: "# Goal\nShared background.",
			tasks: [
				{ name: "Alpha", task: "Do A.", max_runtime_seconds: 6, outputSchema: alphaSchema, schemaMode: "strict" },
				{ name: "Beta", task: "Do B.", max_runtime_seconds: 0, outputSchema: betaSchema, schemaMode: "permissive" },
			],
		} as TaskParams);
