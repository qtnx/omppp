...
describe("worktree isolation helpers", () => {
	...
	describe("git-backed worktree helpers", () => {
		...
		it("captures task deltas in an unborn repository with baseline untracked files", async () => {
			const unbornRepo = await fs.mkdtemp(path.join(os.tmpdir(), "omp-worktree-unborn-"));
			try {
				await runGit(unbornRepo, ["init"]);
				await fs.writeFile(
					path.join(unbornRepo, "stats_utils.py"),
					"from collections.abc import Sequence\n\n\ndef median(values: Sequence[float]) -> float:\n\treturn 0.0\n",
				);
				const baseline = await captureBaseline(unbornRepo);
				...
				const delta = await captureDeltaPatch(unbornRepo, baseline);
		...
		it("captures baseline untracked files whose paths begin with a dash", async () => {
			const dashRepo = await fs.mkdtemp(path.join(os.tmpdir(), "omp-worktree-dash-"));
			try {
				await runGit(dashRepo, ["init", "-q", "-b", BASE_BRANCH]);
				await runGit(dashRepo, ["config", "user.email", "test@example.com"]);
				await runGit(dashRepo, ["config", "user.name", "Test User"]);
				await fs.writeFile(path.join(dashRepo, "tracked.txt"), "base\n");
				await runGit(dashRepo, ["add", "."]);
				await runGit(dashRepo, ["commit", "-q", "-m", "initial"]);
				await fs.writeFile(path.join(dashRepo, "-stats.py"), "def median() -> float:\n\treturn 0.0\n");
				const baseline = await captureBaseline(dashRepo);

				await fs.writeFile(
					path.join(dashRepo, "-stats.py"),
					"def median() -> float:\n\treturn 0.0\n\n\ndef mean() -> float:\n\treturn 0.0\n",
				);

				const delta = await captureDeltaPatch(dashRepo, baseline);
				...
				await fs.rm(dashRepo, { recursive: true, force: true });
			}
		});
...
describe("getRepoRoot", () => {
	...
	it("preserves the generic git-not-found error for directories without any repo", async () => {
		const dir = await fs.mkdtemp(path.join(testTempRoot(), "omp-norepo-"));
		tempDirs.push(dir);
		await expect(getRepoRoot(dir)).rejects.toThrow("Git repository not found.");
	});
