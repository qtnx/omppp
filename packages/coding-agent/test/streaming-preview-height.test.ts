...
describe("streaming edit preview height (stable, full tail window)", () => {
	...
	test("real TUI finalization replaces streaming edit preview throughout native scrollback", async () => {
		...
		try {
			...
			tui.stop();
			await term.flush();
			if (savedMultiplexerEnv.TMUX === undefined) delete Bun.env.TMUX;
			else Bun.env.TMUX = savedMultiplexerEnv.TMUX;
			if (savedMultiplexerEnv.STY === undefined) delete Bun.env.STY;
