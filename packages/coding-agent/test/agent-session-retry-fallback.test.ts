...
describe("AgentSession retry fallback", () => {
	...
	it("switches to the configured chain after one wasted sibling wait on usage-limit exhaustion", async () => {
		...
		vi.spyOn(modelRegistry.authStorage.limits, "markReached").mockResolvedValue({
			switched: false,
	...
	it("re-checks context before a cooldown-expiry revert onto a smaller-window model in the auto-continue path", async () => {
		...
		let primaryAttempts = 0;
		let fallbackTurns = 0;
		const { promise: continuationDispatched, resolve: onContinuationDispatched } = Promise.withResolvers<void>();
		const agent = new Agent({
			getApiKey: model => `${model.provider}-test-key`,
			initialState: {
				model: primaryModel,
		...
		// instead of issuing the oversized request.
		now += 60_000;
