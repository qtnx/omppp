...
const codexCanonicalRegistry: CanonicalModelRegistry = {
	resolveCanonicalModel: (canonicalId: string, options?: { candidates?: readonly Model<Api>[] }) => {
...
const canonicalRegistry = {
	...
	getCanonicalVariants: (canonicalId: string, options?: { candidates?: readonly Model<Api>[] }) => {
		if (canonicalId !== "claude-sonnet-4-5") return [];
		const candidates = options?.candidates ?? canonicalVariantModels;
		return candidates.map(model => ({ canonicalId, selector: `${model.provider}/${model.id}`, model }));
	},
	getCanonicalId: () => "claude-sonnet-4-5",
	getAvailable: () => canonicalVariantModels,
	getAll: () => canonicalVariantModels,
} satisfies CanonicalModelRegistry & { getAvailable: () => Model<Api>[]; getAll: () => Model<Api>[] };
...
describe("resolveModelRoleValue canonical ids", () => {
...
describe("parseModelPattern", () => {
	...
	describe("patterns with valid thinking levels", () => {
		...
		test("max resolves to the exposed max thinking level after the literal pattern misses", () => {
...
describe("resolveModelScope", () => {
	...
	test("extracts :max as a scoped-model glob thinking selector", async () => {
		const registry = {
		...
		expect(scoped).toHaveLength(2);
		expect(scoped.map(entry => entry.model.id)).toEqual(["coding-router", "coding-router:max"]);
		expect(scoped.map(entry => entry.thinkingLevel)).toEqual([Effort.XHigh, Effort.Max]);
		expect(scoped.every(entry => entry.explicitThinkingLevel)).toBe(true);
	});
});
...
describe("parseModelString", () => {
	...
	describe("thinking level suffix extraction", () => {
		...
		test("extracts max as the exposed max thinking level for provider id selectors", () => {
...
describe("filterAvailableModelsByEnabledPatterns", () => {
	...
	const registry = {
		getCanonicalVariants: (_id: string, _opts?: { candidates?: readonly Model<Api>[] }) => [],
	} satisfies CanonicalModelRegistry;
	...
	test("extracts :max as an enabledModels glob thinking selector when a registry can expand the bare model", () => {
		const result = filterAvailableModelsByEnabledPatterns(mockMaxSuffixModels, ["nanogpt/*:max"], registry);
		expect(result.map(model => model.id)).toEqual(["coding-router", "coding-router:max"]);
	});

	test("preserves literal :max in enabledModels globs without registry expansion", () => {
