...
describe("createAgentSession MCP server instructions (deferred UI)", () => {
	...
	it("renders Bash guidance from candidate active names before publishing the tool set", async () => {
		const { session } = await createAgentSession({
			cwd: tempDir,
			agentDir: tempDir,
			modelRegistry,
			sessionManager: SessionManager.inMemory(),
			settings: Settings.isolated({ inlineToolDescriptors: "on", "advisor.enabled": false }),
			model: getBundledModel("openai", "gpt-4o-mini"),
			disableExtensionDiscovery: true,
			skills: [],
			contextFiles: [],
