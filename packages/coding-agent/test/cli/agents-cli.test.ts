...
describe("agents unpack", () => {
	test("preserves task's disabled review gate through unpack + reparse", async () => {
		await withTempDir(async dir => {
			vi.spyOn(process.stdout, "write").mockImplementation(() => true);

			await runAgentsCommand({ action: "unpack", flags: { dir, force: true, json: true } });

			const written = await fs.readFile(path.join(dir, "task.md"), "utf8");
			const { frontmatter } = parseFrontmatter(written, { source: "task.md" });
