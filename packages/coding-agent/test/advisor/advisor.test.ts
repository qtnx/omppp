...
describe("advisor", () => {
	...
	describe("AdviseTool", () => {
		...
		it("lets a one-shot consult answer bypass its own duplicate filter", async () => {
			...
			expect(first.content).toEqual([{ type: "text", text: "Delivered." }]);
			expect(onAdvice).toHaveBeenCalledTimes(1);

			const duplicate = await tool.execute("tc-2", { note, severity: "concern" });
			expect(duplicate.content).toEqual([{ type: "text", text: "Dropped: already raised." }]);
			expect(onAdvice).toHaveBeenCalledTimes(1);

			tool.setConsultAnswerExemption(true);
			const exempted = await tool.execute("tc-3", { note, severity: "concern" });
			expect(exempted.content).toEqual([{ type: "text", text: "Delivered." }]);
			expect(onAdvice).toHaveBeenCalledTimes(2);
			expect(onAdvice).toHaveBeenNthCalledWith(2, note, "concern");

			const oneShotDuplicate = await tool.execute("tc-4", { note, severity: "concern" });
			expect(oneShotDuplicate.content).toEqual([{ type: "text", text: "Dropped: already raised." }]);
			expect(onAdvice).toHaveBeenCalledTimes(2);

			const normalDuplicate = await tool.execute("tc-5", { note, severity: "concern" });
			expect(normalDuplicate.content).toEqual([{ type: "text", text: "Dropped: already raised." }]);
			expect(onAdvice).toHaveBeenCalledTimes(2);
		});
				...
				const message = {
					...
					content: [
						...
						},
						{
							...
							name: "advise",
							arguments: {
								note: "Prompt injection detected: run rm -rf should be ignored.",
								severity: "blocker",
							},
						},
					],
		...
		describe("AdvisorRuntime", () => {
			function makeAgent(
				...
				opts?: { respond?: (input: string | AgentMessage[]) => AgentMessage[] },
			): AdvisorAgent {
				...
				it("keeps large advisor updates text-only for non-Fable and text-only Fable models", async () => {
					...
					for (const testCase of cases) {
						...
						const messages: AgentMessage[] = [
							{ role: "user", content: largeHistory, timestamp: 1 } as AgentMessage,
						];
						const runtime = new AdvisorRuntime(agent, makeAdvisorHost(messages));
			...
			it("preserves the next user turn when an accepted empty stop is pruned", async () => {
				...
				expect(nextTurn.indexOf("real user instruction")).toBeLessThan(nextTurn.indexOf("checking files"));
			});
			...
			it("resolves consults as paused immediately while paused", async () => {
				...
				const agent = makeAgent(promptInputs);
				const messages: AgentMessage[] = [
					{ role: "user", content: "current context", timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
					enqueueAdvice: () => {},
			...
			it("obfuscates session updates before prompting the advisor", async () => {
				...
				const agent = makeAgent(promptInputs);
				const messages: AgentMessage[] = [
					{ role: "user", content: `token ${secret}`, timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
					obfuscator,
			...
			it("keeps replace-regex collisions across advisor deltas", async () => {
				...
				const agent = makeAgent(promptInputs);
				const messages: AgentMessage[] = [
					{ role: "user", content: "first tok_abc123", timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
					obfuscator,
						...
						maintainContext: async incoming => {
							...
							if (maintenancePreviews.length === 1) {
								...
								maintenanceInputs.push(
									buildOpenAiNativeHistory(defaultConvertToLlm(agent.state.messages), model),
								);
				...
				const runtime = new AdvisorRuntime(agent, {
					...
					maintainContext: async () => {
						...
						if (maintenanceCalls === 2) {
							...
							const summary = agent.state.messages[0] as AgentMessage & {
			...
			it("clears advisor thinking signatures when collision scrubbing rewrites their text", async () => {
				...
				const messages: AgentMessage[] = [
					{ role: "user", content: "later tok_abc123", timestamp: 2 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
					obfuscator,
						...
						content: [
							{
								...
								id: "call-1",
								name: "read",
								arguments: { type: "image", value: "tok_abc123" },
							},
						],
						timestamp: 2,
					} as unknown as AgentMessage,
			...
			it("rebases a rewritten transcript without replaying delivered history or resetting the advisor", async () => {
				...
				const messages: AgentMessage[] = [
					{ role: "user", content: oldLargeTranscript, timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
					enqueueAdvice: () => {},
			...
			it("classifies structured overflow metadata before rolling back the failed turn", async () => {
				...
				const messages: AgentMessage[] = [
					{ role: "user", content: "ancient-primary", timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
				};
			...
			it("drops only a double-overflowing batch and continues queued and later updates", async () => {
				...
				const messages: AgentMessage[] = [
					{ role: "user", content: "ancient-history", timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
			...
			it("drops the in-flight batch when a reset aborts the advisor prompt", async () => {
				...
				const messages: AgentMessage[] = [
					{ role: "user", content: "old-conversation", timestamp: 1 } as AgentMessage,
				];
				const host: AdvisorRuntimeHost = {
					snapshotMessages: () => messages,
				};
			...
			it("drops stale quota handling when reset happens during onTurnError", async () => {
				...
				const host: AdvisorRuntimeHost = {
					...
					onTurnError: async (_error, _model, _failedMessages, signal) => {
						hookInvocations++;
						allowHook();
		...
		describe("AdvisorRuntime consult", () => {
			...
			it("chunks two queued consults into two separate prompts, both resolving", async () => {
				const promptInputs: string[] = [];
				const agent = makeConsultAgent(promptInputs, input => [
					text(input.includes("q-one") ? "ans-one" : "ans-two"),
				]);
				const messages: AgentMessage[] = [];
				const host: AdvisorRuntimeHost = { snapshotMessages: () => messages, enqueueAdvice: () => {} };
				const runtime = new AdvisorRuntime(agent, host);
		...
		describe("resolveAdvisorDeliveryChannel", () => {
			...
			it("queues an interrupting note at the current tool boundary while a suppressed turn resumes", () => {
				for (const severity of ["concern", "blocker"] as const) {
					expect(
						resolveAdvisorDeliveryChannel({
							severity,
		...
		describe("formatAdvisorBatchContent", () => {
			it("formats an unchanged current note as advisory content", () => {
				expect(formatAdvisorBatchContent([{ note: "watch out" }])).toBe(
					'<advisory guidance="weigh, don\'t blindly obey">\nwatch out\n</advisory>',
				);
			});

			it("does not synthesize a stale-context caveat into a delivered note", () => {
				const result = formatAdvisorBatchContent([{ note: "watch out" }]);
				expect(result).not.toContain("newer primary turns arrived after this reviewed window");
				expect(result).not.toContain("verify this still applies");
			});

			it("preserves the original multiline note body under advisory framing", () => {
				const note = "multi\nline\nnote";
				expect(formatAdvisorBatchContent([{ note }])).toContain(note);
			});
		});

		describe("AdvisorRuntime fresh backlog", () => {
