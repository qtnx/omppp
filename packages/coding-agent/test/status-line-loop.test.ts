...
describe("status line loop mode segment", () => {
	it("shows that a bounded loop is waiting for its first prompt", () => {
		const rendered = renderSegment(
			...
			createContext({
				state: "waiting",
				limit: { intervalMs: 1_000, initialIterations: 10, remainingIterations: 10 },
			}),
		);
	...
	it("shows the remaining iterations while a loop is running", () => {
		const rendered = renderSegment(
			"mode",
			createContext({
				state: "running",
				limit: { intervalMs: 90_000, initialIterations: 5, remainingIterations: 4 },
			}),
		);

		expect(Bun.stripANSI(rendered.content)).toBe(
			withIcon(theme.icon.loop, "Loop running 4 of 5 iterations remaining"),
		);
	});
