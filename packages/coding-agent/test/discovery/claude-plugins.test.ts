...
import { isSameMCPConnection, type MCPServer, mcpCapability } from "@oh-my-pi/pi-coding-agent/capability/mcp";
...
describe("listClaudePluginRoots", () => {
	...
	test("uses manifest MCP servers without parsing an invalid fallback .mcp.json", async () => {
		const pluginsDir = path.join(tempDir, ".claude", "plugins");
		...
		expect(result.warnings.some(warning => warning.includes("Invalid JSON"))).toBe(false);
		expect(found).toBeDefined();
		expect(found?._source.path).toBe(path.join(pluginPath, ".claude-plugin", "plugin.json"));
	});
	...
	test("preserves a __proto__ env key through discovery and delivery", async () => {
		...
		try {
			...
			// biome-ignore lint/suspicious/noProto: prototype pollution regression fixture
	...
	test("loads OMP user skills without opting into foreign Claude skills", async () => {
		// This case asserts the foreign opt-in gate itself, so it drops the
		// blanket opt-in the surrounding suite applies for plugin discovery.
		disableUserSource("claude-plugins");
		disableUserSource("claude");
	...
	test("loadSkills surfaces omp-installed plugin skills without enabling the Claude source", async () => {
		...
		// With the claude/claude-plugins user sources opted out, the omp-origin
		// skill must still load; the claude-origin one must stay opt-in.
