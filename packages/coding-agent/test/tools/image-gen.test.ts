...
describe("imageGenTool catalog routing", () => {
	...
	it("does not let the active session provider shadow a connected Codex subscription", async () => {
		const openaiImage = catalogModel("openai", "gpt-image-1", "openai-responses");
		const codexImage = catalogModel("openai-codex", "gpt-image-1", "openai-codex-responses");
		const codexCarrier = catalogModel("openai-codex", "gpt-5.5", "openai-codex-responses", "chat");
		const active = catalogModel("openai", "gpt-5.5", "openai-responses", "chat");
		const urls: string[] = [];
		const fetchMock: FetchImpl = async input => {
			urls.push(input.toString());
			const event = {
		...
		const ctx = createContext({
			models: [openaiImage, codexImage, codexCarrier, active],
			settings: Settings.isolated(),
			fetch: fetchMock,
			activeModel: active,
		});

		const result = await imageGenTool.execute("codex-precedence", { subject: "codex first" }, undefined, ctx);
		collectPaths(result);
	...
	it("uses the prioritized gpt-6-astra model as the Codex hosted carrier", async () => {
		const codexImage = catalogModel("openai-codex", "gpt-image-1", "openai-codex-responses");
		const astra = catalogModel("openai-codex", "gpt-6-astra", "openai-codex-responses", "chat");
		const older = catalogModel("openai-codex", "gpt-5.5", "openai-codex-responses", "chat");
		let requestBody: Record<string, unknown> | undefined;
		const fetchMock: FetchImpl = async (_input, init) => {
			requestBody = JSON.parse(String(init?.body));
			const event = {
		...
		const ctx = createContext({ models: [codexImage, astra, older], settings, fetch: fetchMock, activeModel: older });

		const result = await imageGenTool.execute("codex-astra", { subject: "codex default" }, undefined, ctx);
		collectPaths(result);
