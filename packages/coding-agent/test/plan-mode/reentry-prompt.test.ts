...
describe("plan-mode-active tool availability", () => {
	it("omits ask-tool directives when ask is unavailable", () => {
		...
		expect(withoutAsk).not.toContain("via `ask` with 2–4 mutually exclusive options");
		expect(withoutAsk).not.toContain("`ask` only for preferences/tradeoffs");
		expect(withoutAsk).not.toContain("use `ask` to close any remaining preference questions");

		const withAsk = render({ askAvailable: true, iterative: true });
		expect(withAsk).toContain("via `ask` with 2–4 mutually exclusive options");
