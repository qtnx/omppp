...
describe("InteractiveMode orchestrator mode", () => {
	...
	it("retains canonical policies and context blocks across orchestrator activation and restoration", async () => {
		const created = createHarness({
			registryToolNames: [...ORCHESTRATOR_CONTROL_TOOLS, ...SAFE_ORCHESTRATOR_TOOLS],
		...
		expect(activePrompt).toEqual([`Test\n\n${orchestratorModeActivePrompt.trimEnd()}`, "CTX"]);

		await created.handleOrchestratorModeCommand();

		expect(session?.systemPrompt).toEqual(["Test", "CTX"]);
	});
});
