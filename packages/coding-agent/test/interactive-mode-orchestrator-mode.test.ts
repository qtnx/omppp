...
describe("InteractiveMode orchestrator mode", () => {
	...
	it("requires super_review critique and debate checkpoints in Safe Orchestrator Mode", async () => {
		const created = createHarness({
			registryToolNames: [...ORCHESTRATOR_CONTROL_TOOLS, ...SAFE_ORCHESTRATOR_TOOLS],
			activeToolNames: ["read"],
		});
		...
		const superReviewGuidance = modeContext.match(/`super_review`[\s\S]{0,2000}/)?.[0] ?? "";
		...
		if (exactPricePattern.test(modeContext)) {
