...
describe("model thinking runtime helpers", () => {
	...
	it("exposes max for GLM 5.2 without widening older or binary GLM models", () => {
		...
		expect(glm52Qwen.thinking?.efforts.at(-1)).toBe(Effort.Max);
		...
		expect(requireSupportedEffort(glm52Qwen, Effort.Max)).toBe(Effort.Max);
	});
	...
	it("does not expose xhigh on first-party Grok 4.5 Responses models", () => {
		...
		expect(paid.compat.reasoningEffortMap?.minimal).toBe("low");
		expect(oauth.compat.reasoningEffortMap?.minimal).toBe("low");
	});
