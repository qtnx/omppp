 ...
 * - Explicit spec thinking owns `mode` and `defaultLevel`; model-defined
 *   effort restrictions normalize stale cached capability surfaces. Wire facts
 *   (`effortMap`, `supportsDisplay`) are backfilled from identity when not
 *   explicitly set, so configs never need provider wire-tier tables.
