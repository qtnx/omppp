...
function resolveThinkingPolicy<TApi extends Api>(
	...
	if (facts.family("fable", "mythos") && facts.revGte("5.1")) config.prefixBinding = true;
	return config;
}
/**
...
function mergeEffortMap(
	...
	const merged = { ...detected, ...ruleMap, ...configured };
	return spec.reasoningMode === "pro" ? merged : filterEffortMap(merged, efforts);
}
...
export function resolveModelPolicy(spec: ModelSpec<Api>): ResolvedModelPolicy<Api> {
	...
	return {
		...
		catalog: applyForkCatalogOverrides(spec, axes.catalog),
	};
}
