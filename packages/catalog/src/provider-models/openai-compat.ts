...
/**
 * Providers whose generated pro aliases this pass owns and re-projects.
 * First-party OpenAI and Codex Responses both support the fork's pro aliases.
 */
 ...
 * on an owned provider that carries the generated `reasoningMode` marker.
 ...
 * first-party `openai` and `openai-codex` gpt-5.6 rows. Each alias inherits
 * its base row's metadata, requests the base wire id via `requestModelId`, and
 ...
 * sources merge: stale generated aliases are dropped and re-projected from
 * current base rows so alias metadata always tracks the base, while a real
 * upstream model that occupies an alias id wins and suppresses projection.
 */
export function projectOpenAIProReasoningAliases(models: readonly ModelSpec<Api>[]): ModelSpec<Api>[] {
	const priced = models.map(model => {
		if ((model.provider === "openai" || model.provider === "openai-codex") && model.id === "gpt-5.6-sol") {
			return {
				...model,
				cost: {
					...OPENAI_GPT_56_SOL_STANDARD_COST,
	...
	const kept = priced.filter(model => !isGeneratedOpenAIProReasoningAlias(model));
	const ids = new Set(kept.map(model => `${model.provider}/${model.id}`));
	const out = [...kept];
	for (const model of kept) {
		if (OPENAI_PRO_REASONING_PROVIDERS[model.provider] !== true) continue;
...
export function applyXaiCatalogPricing(models: readonly ModelSpec[]): ModelSpec[] {
	...
	return pricedModels.map(model => {
		if (model.provider !== "xai-oauth" || hasTokenPrice(model.cost) || model.id === "grok-composer-2.5-fast")
			return model;
...
function mergeCuratedIntoModel(
	...
	const cost =
		curated.id === "grok-composer-2.5-fast" ? { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 } : base.cost;
	return {
		...base,
...
/**
 * Meta's first-party coding-agent guide is authoritative for Muse Spark limits;
 * models.dev reports an approximation, so enrich discovered rows with the real
 * cost/context/output metadata and the developer-role compat flag.
 */
...
export function metaModelManagerOptions(config?: MetaModelManagerConfig): ModelManagerOptions<"openai-responses"> {
	return {
		...createOpenAICompatibleModelManagerOptions({
			...
			requireApiKey: true,
			mapModel: (entry, defaults, reference) =>
				applyMetaModelMetadata(mapWithBundledReference(entry, defaults, reference)),
		}),
