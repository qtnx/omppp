...
function applyGeneratedModelPolicy(model: ModelSpec<Api>): void {
	...
	// Kimi Code K3 uses the provider-native mandatory-thinking transport. The
	// credential-free models.dev fallback currently labels it as z.ai, while
	// live discovery maps the same model to `kimi`; pin generated bundles to
...
function applyOpenAICatalogPolicy(model: ModelSpec<Api>, parsedModel: OpenAIModel): void {
	...
	// GPT-5.6 luna/sol/terra on the Codex transport use the registry's 372K
	// input window. Pin bundled first-party Codex SKUs exactly so discovery
	// values cannot regress the generated catalog; API-key OpenAI models are
