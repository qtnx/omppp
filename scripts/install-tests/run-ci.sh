...
smoke_cli() {
   ...
   bun "$ROOT_DIR/scripts/install-tests/settings-session.ts" "$ompx_bin"
}
...
for pkg in utils wire omptype catalog ai mnemopi snapcompact agent tui stats context-gc-plugin system-context-reminder-plugin delegation-reminder-plugin collab-web; do
   (
...
#    `bin.ompx` from `src/cli.ts` to the prepack bundle `dist/cli.js`. The repo
...
(
   ...
   bun add "$utils_tgz" "$wire_tgz" "$omptype_tgz" "$natives_tgz" "$catalog_tgz" "$ai_tgz" "$mnemopi_tgz" "$snapcompact_tgz" "$agent_tgz" "$tui_tgz" "$stats_tgz" "$context_gc_tgz" "$system_context_tgz" "$delegation_reminder_tgz" "$coding_agent_tgz" "$collab_web_tgz"
