...
for pkg in utils wire omptype hashline catalog ai mnemopi snapcompact agent tui stats context-gc-plugin system-context-reminder-plugin delegation-reminder-plugin collab-web; do
   (
...
(
   ...
   bun add "$utils_tgz" "$wire_tgz" "$omptype_tgz" "$natives_tgz" "$hashline_tgz" "$catalog_tgz" "$ai_tgz" "$mnemopi_tgz" "$snapcompact_tgz" "$agent_tgz" "$tui_tgz" "$stats_tgz" "$context_gc_tgz" "$system_context_tgz" "$delegation_reminder_tgz" "$coding_agent_tgz" "$collab_web_tgz"
