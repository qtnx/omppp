...
async function watchCI(): Promise<boolean> {
	...
	const repository = githubRepositoryFromOriginUrl(originUrl);
	console.log(`  Commit: ${commitSha.slice(0, 8)}`);

	while (true) {
		const runsOutput =
			await $`gh run list --repo ${repository} --commit ${commitSha} --json databaseId,status,conclusion,name`.text();
		...
		for (const run of inProgressRuns) {
			const jobsOutput = await $`gh run view ${run.databaseId} --repo ${repository} --json jobs`
				.quiet()
				.nothrow()
				.text();
			try {
		...
		if (failedJobs.length > 0) {
			...
			for (const f of failedJobs) {
				...
				const log = await $`gh run view --repo ${repository} --job ${f.jobId} --log-failed`
					.quiet()
					.nothrow()
					.text();
				if (log.trim()) {
		...
		if (failed.length > 0) {
			...
			for (const r of failed) {
				...
				const jobsOutput = await $`gh run view ${r.databaseId} --repo ${repository} --json jobs`
					.quiet()
					.nothrow()
					.text();
				try {
					...
					for (const job of jobs) {
						if (job.conclusion !== "success" && job.conclusion !== "skipped") {
							const log = await $`gh run view --repo ${repository} --job ${job.databaseId} --log-failed`
								.quiet()
								.nothrow()
								.text();
							if (log.trim()) {
...
async function cmdRelease(versionOrBump: string): Promise<void> {
	...
	if (version === "major" || version === "minor" || version === "patch") {
		...
		console.log(`Bumping ${versionOrBump} version from ${currentVersion} -> ${version}`);
	} else if (version === "canary") {
		version = bumpCanaryVersion(currentVersion);
		console.log(`Bumping canary version from ${currentVersion} -> ${version}`);
	}
