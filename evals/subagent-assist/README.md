...
Running the repository's full TypeScript suite exposed a real defect the focused suites missed: an unavailable classifier consumed the one allowed audit and added a model turn to ordinary sessions, which changed existing session, retry, prewalk, and empty-stop behavior. Without a usable classification the gate is now completely inert, so the existing todo, plan, QA, and empty-stop machinery keeps owning the turn and no extra turn is spent. An audit is spent only on an answer that arrived but cannot be trusted.
