...
Two additional real-session regressions first failed with missing output files, then passed after preserving open obligations across an inconclusive verdict and an exhausted audit allowance. Immediate topic-switch checking now defaults to zero idle delay and zero context floor; its integration test verifies compaction precedes dispatch.

Running the repository's full TypeScript suite exposed defects the focused suites missed. An unavailable classifier consumed the audit allowance and added a model turn to ordinary sessions, changing existing session, retry, prewalk, and empty-stop behavior; without a usable judgment the gate is now inert, and the audit allowance is one per request rather than one per candidate, so a repeatedly clipped or unanswered stop cannot loop. A settle-ordering change also suppressed the existing QA reminder while a QA job was running; the original ordering is restored.

A print-mode ordering test was removed rather than kept red: `runPrintMode` never returns against this in-memory rig even with the completion gate uninvolved, so the test measured the harness. Suspended-versus-terminal ends stay covered by the end-notification assertions in the same file. The public JSON ordering claim is therefore **not verified** here.

Two latency defects followed. A configured but unreachable Jev charged a full connection timeout to every caller, including delegation routing and the pre-dispatch topic-switch check; both transports now park an unreachable endpoint for 60 seconds after a failed attempt, and agent routing is bounded to 2 seconds. Checking every request regardless of idle time or context size also put that check in front of ordinary back-to-back prompts, so the idle threshold is 5 minutes and the context floor is 4K tokens: an idle return to the session is still classified, a rapid follow-up is not.
...
Unavailable or inconclusive classification never becomes claimed approval. An answer that arrives but is untrusted receives one primary-context audit per genuine request; an unavailable answer, an unavailable service, and a disabled setting are all inert and add no model turn. Open local obligations still block a premature stop whenever a usable judgment exists and reports no external blocker. Disabling `autonomy.stopGate` disables the backstop. The service timeout and failure budget remain bounded. A classifier is advisory, not proof that arbitrary commands ran.
