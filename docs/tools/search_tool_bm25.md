...
## Notes
- `search_tool_bm25` remains the legacy activation surface for hidden built-ins and MCP tools. The additive `xd://` device transport can expose other tools without removing this discovery contract.
...
- On startup, `packages/coding-agent/src/sdk.ts` resolves `"auto"` after the full registry exists and injects `search_tool_bm25` when the count exceeds 40. It hides non-essential discoverable built-ins only in `tools.discoveryMode = "all"`. Tools whose class is marked as `loadMode === "essential"` are always active; they survive hiding regardless of configuration. The OMPx default essential override list remains `read`, `bash`, `edit`, and `task` unless `tools.essentialOverride` changes it; upstream defaults also include `write`, `glob`, and `eval`, and `tools.essentialOverride` can treat additional discoverable tools as essential or explicitly specify the active essential list.
