...
- Signatures:
  - JS: `await agent(prompt, { agent?, model?, label?, schema?, schemaMode?, isolated?, apply?, merge?, handle?, tools? })`; legacy positional `await agent(prompt, agent?, model?, label?, schema?)` remains implemented.
  - Python: keyword arguments `agent="task"`, `model=None`, `label=None`, `schema=None`, `schema_mode=None`, `isolated=None`, `apply=None`, `merge=None`, `handle=False`, `tools=None`.
...
- `agent` defaults from the current spawn policy (bundled `task` unless overridden) and resolves through normal agent discovery, so project and user agents work.
...
- `schema` overrides agent/session schemas; `schemaMode`/`schema_mode` chooses `permissive` or `strict`. Successful structured calls return parsed data.
