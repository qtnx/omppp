...
- Signatures:
  - JS: preferred `await agent(prompt, { agent?, model?, label?, schema?, schemaMode?, isolated?, apply?, merge?, handle? })`; legacy positional `await agent(prompt, agent?, model?, label?, schema?)` remains implemented.
  - Python/Ruby/Julia: keyword arguments `agent="task"`, `model=None`, `label=None`, `schema=None`, `schema_mode=None`, `isolated=None`, `apply=None`, `merge=None`, `handle=False`.
