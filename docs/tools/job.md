...
8. Otherwise the tool waits on `Promise.race(...)` across:
   ...
   - a pending-agent-context wake — `session.hasPendingAgentAsides?.()` is checked once before the wait and on the 500 ms interval; pending IRC asides, idle-flushable yield-queue entries (e.g. another job's async-result), or queued steering messages resolve the wake so the loop boundary can inject them,
   - a timeout promise only when `async.pollWaitDuration` is a fixed duration (`5s`–`5m`); the default `block` (and unknown/legacy values such as `smart`) arms no timeout,
...
10. A 500 ms interval always runs during the wait: it sends progress snapshots from `#snapshotJobs(...)` when `onUpdate` exists and re-checks `hasPendingAgentAsides`; one snapshot/check happens immediately before entering the race.
...
13. The final text groups jobs by non-running vs still-running state. A fixed-window timeout or aside wake is not an error path; it simply returns the current snapshot.
...
- Background work / cancellation
  - Waiting blocks until a watched job settles, pending agent context arrives, the tool-call abort signal fires, or — only with a fixed `async.pollWaitDuration` — the configured window elapses.
...
- Poll wait duration comes from `async.pollWaitDuration` ("Max Poll Time") in `packages/coding-agent/src/config/settings-schema.ts`:
  - allowed values: `5s`, `10s`, `30s`, `1m`, `5m`, `block`
  - default: `block`
  - fixed values cap the wait at exactly that duration; `block` arms no timeout and waits until a watched job settles, pending agent context arrives, or the call is aborted. Legacy `smart` configs are migrated to `block` in `packages/coding-agent/src/config/settings.ts`.
