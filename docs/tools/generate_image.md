...
2. Provider order is: concrete per-request `provider`; entries in `providers.imageOrder`; a connected OpenAI Codex subscription; the active session model's corresponding image provider; then the built-in order `openai`, `openai-codex`, `antigravity`, `xai`, `openrouter`, `gemini`, `deepinfra`. Duplicates are removed. `provider: "auto"` does not add a provider.
