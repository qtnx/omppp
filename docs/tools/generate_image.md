...
| `model` | `string` | No | Image-model selector for this request, for example `openrouter/google/gemini-3-pro-image` or `xai/grok-imagine-image` (xAI's quality tier is `xai/grok-imagine-image-quality`). When omitted, the configured `image` role chain is used. |
...
2. A request with `model` resolves that selector against available catalog models of kind `image` and attempts only the selected model. Without `model`, the tool resolves `modelRoles.image` followed by `retry.fallbackChains.image`; when no fallback chain is configured, the built-in image defaults apply, while `retry.fallbackChains.image: []` disables fallbacks. Among non-explicit candidates a connected Codex subscription is tried first, then the active session provider, then the remaining defaults; explicit configuration keeps its own order.
