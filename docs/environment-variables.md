...
| `PI_OMPX_SSH_AUTH_SOCK`      | macOS sandbox: override for the SSH agent socket exposed to sandboxed `git`/`ssh` as `SSH_AUTH_SOCK`. By default the running agent (1Password, Secretive, launchd, `ssh-agent`) is auto-discovered; set this (or the `sandbox.sshAuthSock` setting) to force a specific Unix socket, applied only when no socket is already inherited. Private keys stay blocked.                                                                                                                                                                                                                                       |
| `PI_OMPX_LINUX_SANDBOX`      | Linux startup sandbox opt-in. Set before startup to `podman`/`1`/`true` to re-exec top-level OMPx inside rootless Podman. `0`/`false`/`off` disables. Captured before `.env` overlays so project-local env files cannot enable it. Uses the default dev image `oh-my-pi/pi:dev` unless overridden by `PI_OMPX_PODMAN_IMAGE` or trusted `sandbox.podman.image`; source checkouts can build the default with `bun run pi:image`. |
| `PI_OMPX_PODMAN_IMAGE`       | Optional trusted OCI image override containing a matching `ompx` command for Linux Podman workspace sandboxing. Must be set before startup, because it is captured before `.env` overlays; config-based setup should use trusted user/global `sandbox.podman.image`. |
