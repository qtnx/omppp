...
| `providers.imageOrder` | array | `[]` | Image-generation provider IDs in priority order (`openai-codex`, `openai`, `antigravity`, `xai`, `gemini`, `openrouter`). Unlisted providers fall back to a connected Codex subscription, then the active session provider, then the built-in order. Replaces the removed `providers.image` enum (a legacy value migrates to the head of this list). |
...
| `providers.maxInFlightRequests` | record | `{}` | Positive per-provider concurrency limits for LLM HTTP requests, shared across local `ompx` processes using the same config root. Omitted providers are unlimited. `ompx config set` rejects non-positive or non-numeric values. |
...
| `providers.cacheRetention` | enum | `auto` | `auto`, `short`, `long`, `none`. Prompt-cache retention forwarded to providers that support it. `auto` keeps provider defaults and honors `PI_CACHE_RETENTION`; `short` forces 5m; `long` uses 1h TTLs where supported; `none` disables prompt caching and cache-affinity routing. |
