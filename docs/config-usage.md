...
3. Config overlays: `PI_CONFIG_FILES` (platform path-list), followed by repeated `ompx --config <path>` files; all are loaded as `config.yml`-style YAML for this process only
...
3. Write the merged result to `config.yml`
