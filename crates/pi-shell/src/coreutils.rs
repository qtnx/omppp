...
/// Registers an upstream utility name as disabled so brush resolves it through
/// `$PATH` instead of running an in-process copy. `enable` can flip the public
/// disabled bit on any registration, so these registrations intentionally do
/// not retain a callable uutils entry point; if one is force-enabled, it fails
/// closed instead of entering `spawn_blocking`.
macro_rules! external_only_uutil_builtin {
	($vis:vis fn $reg_fn:ident => $run:path) => {
		$vis fn $reg_fn<SE: ShellExtensions>() -> Registration<SE> {
			fn execute<SE: ShellExtensions>(
				...
				Box::pin(async { Ok(ExecutionResult::new(127)) })
			}
			Registration {
				execute_func:                   execute::<SE>,
				content_func:                   uutil_content,
...
external_only_uutil_builtin!(pub fn tail_builtin => uu_tail::run);
uutil_builtin!(pub fn ls_builtin => uu_ls::run);
external_only_uutil_builtin!(pub fn find_builtin => uu_find::run);
external_only_uutil_builtin!(pub fn grep_builtin => pi_uu_grep::run);
external_only_uutil_builtin!(pub fn rg_builtin => pi_uu_grep::run_rg);
