...
fn unbounded_tail<T: Read>(reader: &mut BufReader<T>, settings: &Settings) -> UResult<()> {
	...
	// SIGPIPE is not available on Windows. In embedded mode, a broken pipe must
	// not terminate the host shell process; report the conventional status.
	#[cfg(target_os = "windows")]
	match writer.flush() {
		Ok(()) => {},
		Err(err) if err.kind() == ErrorKind::BrokenPipe => {
			pi_uutils_ctx::set_exit_code(13);
			return Ok(());
		},
