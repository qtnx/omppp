		...
		if !options_done && arg.starts_with('-') && arg != "-" {
			...
			while short_index < chars.len() {
				...
				match option {
					...
					// `pgrep -q` is supported by procps-ng (Linux) as well as BSD/macOS, and
					// the long `--quiet` form above was never gated — restricting the short
					// form to macOS made `pgrep -q` fail with exit 2 on Linux.
