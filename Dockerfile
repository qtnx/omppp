...
# Legal payload for the reusable SDKs and OMPx product installed in this image.
COPY LICENSE THIRD-PARTY-NOTICES.txt /usr/share/doc/ompx/

# `ompx`/`omp` shims — run the coding-agent CLI against $PI_ROOT via Bun.
# Derived images override PI_ROOT to point at wherever their OMPx source lives.
RUN printf '%s\n' \
    '#!/usr/bin/env bash' \
    'set -euo pipefail' \
    ': "${PI_ROOT:=/work/pi}"' \
